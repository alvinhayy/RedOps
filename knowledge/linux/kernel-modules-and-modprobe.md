---
title: "Kernel Modules and modprobe Abuse"
source: hacktricks.wiki
source_url: https://hacktricks.wiki/linux-hardening/main-system-information/kernel-modules-and-modprobe.html
...
Kernel module support is a high-impact area during Linux privilege escalation review. Do not treat every unsigned-module message as exploitable by itself, but use it to answer practical questions.[\[1\]](#references)[\[2\]](#references)[\[3\]](#references)[\[8\]](#references)[\[9\]](#references)[\[10\]](#references)
...
Quick triage starts with the following module-status, signature, logging, and module-tree checks.[\[1\]](#references)[\[2\]](#references)[\[6\]](#references)[\[8\]](#references)
...
If you have legitimate permission to load a local module, `insmod` inserts the exact `.ko` file you provide. The module’s init function runs as part of the load, and messages written with `printk()` go to the kernel log buffer, which is normally read with `dmesg`.[\[3\]](#references)[\[4\]](#references)[\[5\]](#references)[\[6\]](#references)

A minimal review workflow uses `modinfo` to inspect metadata, `insmod` and `rmmod` to load and remove a module, `lsmod` to confirm loaded state, and `dmesg` to inspect kernel logs.[\[4\]](#references)[\[6\]](#references)[\[12\]](#references)[\[13\]](#references)[\[14\]](#references)
...
dmesg | tail -n 30
```
If `sudo -l` allows `insmod`, `modprobe`, or a wrapper around them, treat it as critical: `sudo -l` lists the invoking user’s privileges, and loading a kernel module requires `CAP_SYS_MODULE`. See [Linux capabilities](../interesting-files-permissions/linux-capabilities.html#cap_sys_module) for direct capability-based paths.[\[3\]](#references)[\[9\]](#references)[\[10\]](#references)
...
`insmod`
A sudo rule that allows a user to run `insmod` is not comparable to allowing a normal administrative helper. The module’s initialization code runs as part of insertion, so the practical review question is whether this user can choose or modify the module being loaded.[\[3\]](#references)

The following generic review flow repeats those inspection, load, state, log, and removal checks for a candidate module.[\[4\]](#references)[\[6\]](#references)[\[12\]](#references)[\[13\]](#references)[\[14\]](#references)
...
```
If the user can provide an arbitrary `.ko`, the rule should be treated as full system compromise in an authorized assessment. A safer operational pattern is to avoid delegating module loading through sudo; if it is unavoidable, restrict the exact path, ownership, permissions, signing policy, and removal workflow.[\[3\]](#references)[\[10\]](#references)

For a harmless module-building pattern in a controlled lab, a minimal source and Makefile are shown below; the `make -C /lib/modules/$(uname -r)/build M=$PWD` form follows the kernel’s documented kbuild workflow for external modules.[\[5\]](#references)[\[7\]](#references)
...
```
Build and load only in an authorized lab; kbuild builds the external module and the load/remove commands invoke the kernel module interfaces.[\[3\]](#references)[\[4\]](#references)[\[5\]](#references)[\[7\]](#references)
...
Check the current helper path through the kernel sysctl interface and inspect the target’s ownership and mode.[\[1\]](#references)
...
```
Check whether the sysctl, delegated sudo rules, or file capabilities can be influenced.[\[1\]](#references)[\[9\]](#references)[\[10\]](#references)[\[15\]](#references)
...
```
The following lab-only pattern changes the helper path and triggers a documented module-autoload request; use it only on an isolated, authorized system.[\[1\]](#references)

On current Linux kernels, do not use an unknown executable as a generic trigger: legacy custom binary-format module autoloading was removed in Linux 6.14, while the kernel documentation identifies an unknown filesystem type as a module-autoload request path.[\[1\]](#references)[\[11\]](#references)
...
```
An unrestricted sudo rule for `modprobe` is exploitable even when arbitrary `.ko` files cannot pass signature verification: `-C` selects an attacker-controlled configuration directory, from which an `install` command can be executed by the sudo-launched process.[\[8\]](#references)[\[16\]](#references)
...
```
For mitigation, do not grant argument-unrestricted `modprobe` through sudo, keep every configuration directory root-owned and non-writable, and review unexpected `install`/`remove` directives. When a trusted administrative workflow must bypass such directives for one module, `modprobe --ignore-install` ignores them for that named module, but dependencies can still have their own commands.[\[8\]](#references)[\[16\]](#references)

### Writable `/lib/modules` review

`/lib/modules` review
Writable module directories can allow module replacement, malicious module planting, or auto-load abuse depending on how `modprobe` is later invoked; `modprobe` searches `/lib/modules/$(uname -r)` and uses its dependency data when resolving modules.[\[8\]](#references)

Review writable module files and dependency/alias metadata under the active kernel release’s module tree.[\[8\]](#references)
...
```
If you find writable module content, inspect how `modprobe` resolves dependencies and how `modinfo` reports module metadata.[\[8\]](#references)[\[12\]](#references)
