...
source_url: https://hacktricks.wiki/linux-hardening/software-information/splunk-lpe-and-persistence.html
...
When you land on a host running Splunk or Splunk Universal Forwarder, these are usually the most interesting paths:[\[7\]](#references)[\[8\]](#references)[\[9\]](#references)[\[10\]](#references)[\[11\]](#references)
...
**Exploit overview:**
An exploit targeting the Splunk Universal Forwarder (UF) allows attackers with the **agent password** to execute arbitrary code on systems running the agent, potentially compromising a large portion of the environment.[\[1\]](#references)
...
The original report demonstrates the following loop for sending a payload to multiple forwarders.[\[1\]](#references)
...
└── default/inputs.conf
```
Minimal `inputs.conf`:[\[10\]](#references)
...
sourcetype = auditd
```
Quick Linux dropper (using that documented app layout):[\[10\]](#references)
...
If you can read Splunk’s local files, there are usually two good goals: recover **Splunk admin access** and recover **encrypted service credentials**.[\[8\]](#references)

### Password hashes and local users

Splunk stores local authentication data in `etc/passwd`. Depending on the deployment, cracking that file can recover working credentials for the web UI and the management API.[\[1\]](#references)[\[7\]](#references)

If you already have valid **admin** credentials and Splunk uses its **native** authentication backend, the CLI itself can be used for persistence.[\[13\]](#references)
...
`splunk.secret` and encrypted values
Splunk uses `etc/auth/splunk.secret` to protect sensitive values stored in multiple configuration files. If you can steal both the **secret** and the relevant **`.conf` files**, you can often recover or replay:[\[8\]](#references)
...
This can support **lateral movement** even when the Splunk admin password itself is not crackable.[\[8\]](#references)

### `user-seed.conf` abuse

`user-seed.conf` abuse
`user-seed.conf` is only consumed during first start or when `etc/passwd` does not exist. That makes it less useful on a live box, but very interesting in:[\[9\]](#references)
...
In those cases, planting a `HASHED_PASSWORD` generated with `splunk hash-passwd` gives you a quiet way to regain admin access after redeployment.[\[9\]](#references)
...
A useful recent technique is abusing **user-supplied XSLT** in vulnerable Splunk Enterprise versions to turn a low-privileged authenticated account into **OS command execution** as the `splunk` user.[\[3\]](#references)[\[4\]](#references)
...
The important offensive takeaway is that this path is **post-auth RCE without needing app upload**. On Linux it usually lands you in the **`splunk`** account, which is still valuable because that user often owns the application tree, can read secrets, and can plant persistent apps that survive shell loss.[\[3\]](#references)[\[4\]](#references)

A representative path used during exploitation is:[\[4\]](#references)
