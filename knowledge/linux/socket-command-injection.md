...
source_url: https://hacktricks.wiki/linux-hardening/network-information/socket-command-injection.html
...
Some privileged daemons expose a root-owned UNIX socket that accepts untrusted input and couples privileged actions to thread-IDs and signals. If the protocol lets an unprivileged client influence which native thread is targeted, you may be able to trigger a privileged code path and escalate.[\[1\]](#references)[\[2\]](#references)

The primary write-up and disclosure describe the following sequence.[\[1\]](#references)[\[2\]](#references)
...
```
To turn this into a root shell, a simple named-pipe + nc pattern can be used.[\[2\]](#references)
