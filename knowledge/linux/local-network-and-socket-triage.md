---
title: "Local Network and Socket Triage"
source: hacktricks.wiki
source_url: https://hacktricks.wiki/linux-hardening/network-information/local-network-and-socket-triage.html
...
Start by identifying listening services, their bind addresses, and the owning process when permissions allow it.[\[1\]](#references)[\[2\]](#references)
...
Map local ports with lightweight probes.[\[6\]](#references)[\[7\]](#references)
...
```
Use `nmap` locally when available.[\[8\]](#references)[\[9\]](#references)[\[10\]](#references)
...
Containerized or lab environments often expose services only on a bridge or veth subnet. Enumerate interfaces and routes before assuming a service is unreachable.[\[2\]](#references)
...
ip neigh
```
Find likely local subnets.[\[2\]](#references)

```
ip -o -4 addr show | awk '{print $2, $4}'
```
Probe a discovered subnet carefully.[\[8\]](#references)[\[9\]](#references)[\[10\]](#references)
...
Forward a local-only HTTP service with SSH.[\[11\]](#references)

```
ssh -L 8080:127.0.0.1:8080 user@target
```
Bridge a local port with `socat` when you already have shell access.[\[12\]](#references)

```
socat TCP-LISTEN:18080,fork,reuseaddr TCP:127.0.0.1:8080
```
Forward a Unix socket to TCP for local testing.[\[5\]](#references)[\[12\]](#references)
...
printf 'version\n' | nc -nv 127.0.0.1 9000
```
HTTP check without a browser.[\[13\]](#references)[\[14\]](#references)
...
Capture loopback HTTP traffic.[\[16\]](#references)

```
sudo tcpdump -i lo -A -s0 'tcp port 80 or tcp port 8080'
```
Capture a specific local service.[\[16\]](#references)

```
sudo tcpdump -i lo -w /tmp/loopback.pcap 'tcp port 8080'
```
Decode Basic Auth from a captured or logged header.[\[17\]](#references)[\[18\]](#references)
...
Run a client with key logging enabled.[\[19\]](#references)[\[20\]](#references)
...
```
Capture the traffic at the same time.[\[16\]](#references)

```
sudo tcpdump -i lo -w /tmp/tls.pcap 'tcp port 8443'
```
Then load `/tmp/tls.pcap` and `/tmp/sslkeys.log` into Wireshark. This only works when the client library supports NSS-style key logging and you can set the environment before the connection is made.[\[20\]](#references)[\[21\]](#references)

## Unix Socket Interaction and Command Injection

Unix sockets are local IPC endpoints.<sup>[\[5\]](#references)</sup> They may expose HTTP APIs, custom protocols, or unsafe command handlers.[\[12\]](#references)[\[14\]](#references)

```
ss -lnx
find /run /var/run /tmp -type s -ls 2>/dev/null
```
Interact with HTTP over a Unix socket.[\[14\]](#references)

```
curl --unix-socket /run/app/app.sock http://localhost/
curl --unix-socket /run/app/app.sock -i http://localhost/admin
```
Interact with a raw socket.[\[12\]](#references)[\[13\]](#references)

```
printf 'status\n' | socat - UNIX-CONNECT:/run/app/app.sock
printf 'help\n' | nc -U /run/app/app.sock
```
If user-controlled socket input is passed to a shell or privileged helper, it can become command injection.<sup>[\[26\]](#references)</sup> For a focused example, see [Socket Command Injection](socket-command-injection.html).

## nftables Review and Authorized Rule Changes

Local firewall rules may explain why a service is visible locally but blocked remotely, or why a high port appears unreachable from one interface.[\[22\]](#references)

Review rules.[\[22\]](#references)
...
```
Look for drops affecting a target port.[\[22\]](#references)

```
sudo nft list ruleset | grep -Ei 'drop|reject|dport|tcp|udp'
```
In an authorized lab, remove a specific blocking rule by handle.[\[22\]](#references)

```
sudo nft -a list chain inet filter input
sudo nft delete rule inet filter input handle <handle>
```
Prefer deleting the exact handle over flushing full tables. The technique is to identify the precise filter causing the behavior and change only that rule.[\[22\]](#references)
