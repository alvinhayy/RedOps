...
source_url: https://hacktricks.wiki/linux-hardening/user-information/euid-ruid-suid.html
...
An unprivileged process can only modify its `euid` to match the current `ruid`, `euid`, or `suid`.[\[3\]](#references)
...
These functionalities are designed not as a security mechanism but to facilitate the intended operational flow, such as when a program adopts another user’s identity by altering its effective user ID.[\[1\]](#references)[\[3\]](#references)[\[4\]](#references)

Notably, a privileged call to `setuid` can assign all three IDs, whereas `setreuid` and `setresuid` expose different controls; differentiating these functions is crucial for understanding user-ID transitions.[\[1\]](#references)[\[2\]](#references)[\[3\]](#references)[\[4\]](#references)
...
Examples taken from https://0xdf.gitlab.io/2022/05/31/setuid-rabbithole.html#testing-on-jail, check it for further information.[\[1\]](#references)
