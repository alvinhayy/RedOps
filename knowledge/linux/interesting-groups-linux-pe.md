...
source_url: https://hacktricks.wiki/linux-hardening/user-information/interesting-groups-linux-pe/index.html
...
**Sometimes**, a system’s **/etc/sudoers** policy (or a file included from it) contains entries such as:[\[3\]](#references)
...
```
This means that any user matched by either entry may run any command as any target user through `sudo` (subject to the rest of the policy).[\[3\]](#references)
...
```
The relevant group names and defaults vary by distribution; a group is useful here only if the local policy names it.[\[5\]](#references)
...
```
On an SSH session without a registered authentication agent, `pkexec` may fail with this error even when the policy would otherwise allow the action; polkit documents `pkttyagent` as a text authentication agent for non-desktop sessions. The exact behavior is version- and distribution-dependent, so verify the local policy and agent setup. One workaround reported for affected NixOS versions uses **2 different SSH sessions**.[\[1\]](#references)[\[4\]](#references)[\[5\]](#references)
...
```
This means that any user matched by the entry may run any command as any target user through `sudo` (subject to the rest of the policy).[\[3\]](#references)
...
On systems whose permissions grant it, users in the **shadow** group can **read** **/etc/shadow**; verify the actual mode and ACLs on the target:[\[6\]](#references)[\[7\]](#references)
...
**staff**: Allows users to add local modifications to the system (`/usr/local`) without needing root privileges (note that executables in `/usr/local/bin` are in the PATH variable of any user, and they may “override” the executables in `/bin` and `/usr/bin` with the same name). Compare with group “adm”, which is more related to monitoring/security.[\[2\]](#references)[\[7\]](#references)
...
On Ubuntu systems, `pam_motd` runs executable scripts via `run-parts --lsbsysinit` as root at login; cron jobs may also use `run-parts`, but this is distribution- and configuration-specific.[\[10\]](#references)[\[11\]](#references)
...
```
On a new SSH login, `pspy` can help confirm whether this path is actually invoked on the target; it can observe process command lines without root.[\[10\]](#references)[\[12\]](#references)
...
Membership in the **disk** group may grant raw access to block devices and is often **close to root access**; Debian describes it as mostly equivalent to root, but verify the actual device permissions and storage layout on the target.[\[7\]](#references)
...
```
The `-w` option opens the filesystem read-write, and the `write` command copies a native file into the opened filesystem. Avoid using it on a mounted live filesystem because direct edits can corrupt the filesystem; work from an offline image when possible.[\[8\]](#references)
...
Using the command `w` you can find **who is logged on the system** and it will show an output like the following one.[\[20\]](#references)
...
```
The **tty1** entry identifies the first Linux virtual console; it does not by itself prove that a user is physically present at the machine, especially in containers or other environments.[\[21\]](#references)
...
```
If the installed **GIMP** version exposes a raw-data importer, open **`screen.raw`** with that importer; support and controls vary by version and plug-in.[\[22\]](#references)

Set the image Width and Height to match the framebuffer geometry; try the available pixel formats/Image Types until the output is legible.[\[9\]](#references)
...
Membership in the `docker` group grants root-level access to the Docker daemon on standard rootful installs. Because bind mounts are read-write by default, a user who can control that daemon can mount the host’s `/` into a container and alter host files; this effectively gives root on the host.[\[13\]](#references)[\[14\]](#references)[\[15\]](#references)
...
[Privilege escalation via Docker - Chris Foster](https://fosterelli.co/privilege-escalation-via-docker.html)

## lxc/lxd Group

[Interesting Groups - Linux Privesc](./index.html)

## Adm Group

Usually **members** of the group **`adm`** have permissions to **read log** files located inside */var/log/*.

Therefore, if you have compromised a user inside this group you should definitely take a **look to the logs**.[\[7\]](#references)

## Backup / Operator / lp / Mail groups

These groups have service- and distribution-specific meanings. Debian documents `backup` for delegated backup/restore, `lp` for printer daemons, and `mail` for `/var/mail`, so check local permissions before treating membership as a privilege path.[\[7\]](#references)
