...
source_url: https://hacktricks.wiki/linux-hardening/software-information/pam-pluggable-authentication-modules.html
...
When analyzing or modifying PAM, the **location of an inserted rule** determines which stack sees it:[\[1\]](#references)[\[13\]](#references)
...
```
Compile and stealth-replace (the replacement/timestomp pattern is documented by Unit 42). Adjust both the backup path hard-coded in the wrapper and the commands below to the target’s actual PAM module directory:[\[2\]](#references)
...
For package-integrity checks, RPM verifies installed-file metadata, `debsums -s` reports checksum errors, and `dpkg -S` in the triage block queries package ownership; the audit watch syntax records writes and attribute changes to a path.[\[6\]](#references)[\[7\]](#references)[\[8\]](#references)[\[9\]](#references)
...
`pam_exec` for persistence
Instead of replacing `pam_unix.so`, a lighter touch is to append a `pam_exec` line in `/etc/pam.d/sshd` so an invocation that reaches that PAM line runs a helper while leaving the normal stack intact.[\[4\]](#references)
...
Practical notes follow the module types and `type=` filter documented for `pam_exec`:[\[4\]](#references)
...
`authselect`
On RHEL and Fedora-family systems that use `authselect`, direct edits to generated files such as `/etc/pam.d/system-auth` or `/etc/pam.d/password-auth` may be **overwritten by `authselect`**. For persistence, operators often patch the active custom profile under `/etc/authselect/custom/<profile>/` and then re-select it.[\[5\]](#references)[\[19\]](#references)

Typical workflow when you have root:[\[5\]](#references)
...
```
This matters for both offense and triage: if `/etc/pam.d/system-auth` contains the banner `Generated by authselect` and `Do not modify this file manually`, then the real persistence point may live under `/etc/authselect/custom/` rather than in `/etc/pam.d/`.[\[5\]](#references)
