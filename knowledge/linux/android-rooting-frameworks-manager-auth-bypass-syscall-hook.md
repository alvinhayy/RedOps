...
source_url: https://hacktricks.wiki/linux-hardening/software-information/android-rooting-frameworks-manager-auth-bypass-syscall-hook.html
...
When userspace calls prctl(0xDEADBEEF, CMD_BECOME_MANAGER, data_dir_path, …), KernelSU verifies:[\[1\]](#references)[\[2\]](#references)[\[11\]](#references)

1. Path prefix check

- The provided path must start with an expected prefix for the caller UID, e.g. /data/data/or /data/user/ / .
...
1. APK signature check via FD table scan
...
If all checks pass, the kernel caches the manager’s UID temporarily; manager-only commands then accept that UID, while other commands retain their own UID or allowlist checks.[\[2\]](#references)[\[3\]](#references)

## Vulnerability class: trusting path-derived APK selection

KernelSU v0.5.7 does not bind the signature result to PackageManager’s installed package identity. In `manager.c`, the package test is only a path substring check (`strstr(cwd, pkg)`); the first candidate that passes that test is then signature-checked. An attacker can therefore place a genuine manager APK under a `/data/app/` path that also contains the attacker’s package name and arrange for it to be selected first.[\[1\]](#references)[\[3\]](#references)[\[4\]](#references)

This trust-by-indirection lets an unprivileged app impersonate the manager without owning the manager’s signing key.[\[1\]](#references)

Key properties exploited:[\[1\]](#references)[\[3\]](#references)
...
The concrete KernelSU v0.5.7 case requires:[\[1\]](#references)[\[3\]](#references)
...
High-level steps (the cited demo video shows the public proof of concept in operation):[\[1\]](#references)[\[2\]](#references)[\[10\]](#references)
...
Practical notes on step 2 (FD ordering):[\[1\]](#references)
...
```
Manager authentication via the KernelSU v0.5.7 `prctl` hook:[\[1\]](#references)[\[2\]](#references)[\[11\]](#references)
...
```
After success, privileged commands (examples):[\[2\]](#references)[\[11\]](#references)
...
Limitations of the attack:[\[1\]](#references)[\[2\]](#references)
