...
source_url: https://hacktricks.wiki/linux-hardening/user-information/interesting-groups-linux-pe/lxd-privilege-escalation.html
...
Just grab both **lxd.tar.xz** and **rootfs.squashfs** from the newest build (the directory name is the date).[\[8\]](#references)

Alternatively, you can install distrobuilder on your machine by following the [project instructions](https://github.com/lxc/distrobuilder).[\[4\]](#references)[\[5\]](#references)[\[6\]](#references)
...
```
Upload **incus.tar.xz** (**lxd.tar.xz** if you downloaded from the Canonical image server) and **rootfs.squashfs**, then import the image and create a container.[\[2\]](#references)[\[3\]](#references)[\[5\]](#references)[\[8\]](#references)[\[9\]](#references)
...
Run **`lxd init`**, set up a default storage pool, then **repeat** the previous chunk of commands.[\[2\]](#references)

Finally, start the container and open a root shell on the host filesystem:[\[1\]](#references)[\[2\]](#references)
...
Build an Alpine image and start it with the flag `security.privileged=true`, which maps container root to host root; mounting `/` then exposes the host filesystem inside the container.[\[1\]](#references)[\[7\]](#references)[\[9\]](#references)
