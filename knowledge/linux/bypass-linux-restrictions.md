...
source_url: https://hacktricks.wiki/linux-hardening/linux-basics/bypass-linux-restrictions/index.html
...
The command-injection and WAF-evasion collections in PayloadsAllTheThings, Bo0oM’s cheat sheet, and the two linked Secjuice articles provide background for the shell-syntax variations in this section.[\[1\]](#references)[\[2\]](#references)[\[3\]](#references)[\[4\]](#references)
...
For out-of-band callbacks, a collaborator-style service such as Burp Collaborator can induce a target application to interact with an external server; the existing [**pingb**](http://pingb.in) link is retained as historical navigation, not a current availability claim.[\[6\]](#references)
...
Start with the existing [**shell builtins**](https://www.gnu.org/software/bash/manual/html_node/Shell-Builtin-Commands.html) navigation, then try the following Bash-specific techniques:[\[7\]](#references)
...
The following invocation uses Bashfuscator, an open-source Bash obfuscation framework; the repository link in the code comment is retained as navigation.[\[8\]](#references)
...
If you are inside a filesystem with **read-only and noexec protections**, or in a **distroless image**, the environment imposes execution constraints documented by Linux `mount(8)` and the Distroless project; the linked page collects techniques for working within them.[\[11\]](#references)[\[12\]](#references)

[Bypass FS protections: read-only / no-exec / Distroless](bypass-fs-protections-read-only-no-exec-distroless/index.html)

## Chroot & other Jails Bypass

## Space-Based Bash NOP Sled (“Bashsledding”)

When a vulnerability lets you partially control an argument that ultimately reaches `system()` or another shell, the payload offset may be uncertain. Alan Cao and Will Tan describe a constrained embedded-device case where a shell payload was sprayed into memory-mapped NVRAM and prefixed with spaces.[\[5\]](#references)

Therefore you can create a *NOP sled for Bash* by prefixing your real command with a long sequence of spaces or tab characters; Bash defines spaces and tabs as blanks that separate words in a simple command.[\[5\]](#references)[\[7\]](#references)
...
```
If a ROP chain (or another memory-corruption primitive) passes a command-string pointer that begins anywhere within the space block, Bash can parse the remaining leading blanks until it reaches the command; in the cited router exploit, this made uncertain string offsets usable.[\[5\]](#references)[\[7\]](#references)

Practical use cases in constrained embedded targets include:[\[5\]](#references)
...
🛠️  Combine this technique with ROP gadgets that call `system()` in a controlled lab; the cited router research demonstrates this combination on constrained hardware.[\[5\]](#references)
