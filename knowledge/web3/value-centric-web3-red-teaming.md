...
source_url: https://hacktricks.wiki/blockchain/blockchain-and-crypto-currencies/value-centric-web3-red-teaming.html
...
Build a map of everything that can influence value state, even if it is off-chain.[\[2\]](#references)
...
Translate the AADAPT taxonomy into concrete attack candidates per component.[\[2\]](#references)
...
Work the queue like an adversary: start with the operational footholds that could succeed today, then progress into deep protocol/economic manipulation paths.[\[2\]](#references)
...
Instrument telemetry streams so every scenario produces actionable detection data.[\[2\]](#references)
...
Track program maturity on three axes:[\[2\]](#references)
...
Typical milestones: (1) completed value inventory + AADAPT mapping, (2) first end-to-end scenario with detections implemented, (3) quarterly purple-team cycles expanding coverage and driving down MTTD/MTTC.[\[2\]](#references)

## 7. Scenario templates

Use these repeatable blueprints to design simulations that map directly to AADAPT behaviors.[\[2\]](#references)
