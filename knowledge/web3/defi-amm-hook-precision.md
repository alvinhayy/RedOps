...
source_url: https://hacktricks.wiki/blockchain/blockchain-and-crypto-currencies/defi-amm-hook-precision.html
...
1. Model the hook’s math and thresholds
...
1. Calibrate exact‑input swaps to cross boundaries
...
In v4, the swap loop must run from a PoolManager unlock callback; negative `amountSpecified` denotes exact input, and `sqrtPriceLimitX96` must be strictly inside the valid range. A zero price limit reverts, so the pseudocode below uses the lower bound for a zero-for-one swap.[\[9\]](#references)[\[10\]](#references)[\[11\]](#references)
...
```
1. Exit and cross‑chain replication
...
Simplified vulnerable line (and post‑mortem fix).[\[1\]](#references)
