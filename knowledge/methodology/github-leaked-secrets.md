...
source_url: https://hacktricks.wiki/generic-methodologies-and-resources/external-recon-methodology/github-leaked-secrets.html
...
`trufflehog github --org <ORG> --results=verified`.<sup>[\[2\]](#references)[\[13\]](#references)</sup>- Gitleaks scans Git repositories, directories, and archives. Use
`gitleaks git -v --log-opts="--all" <repo>` for history, `gitleaks dir -v <path>` for directories, and `--max-archive-depth 1` to inspect archives.<sup>[\[6\]](#references)</sup>- Nosey Parker is archived and replaced by Titus. Existing installations still support
`noseyparker scan --datastore np.db <path|repo>` followed by `noseyparker report --datastore np.db`.<sup>[\[7\]](#references)[\[8\]](#references)</sup>- ggshield (GitGuardian CLI) scans files, repositories, and Docker images and integrates with local or CI workflows:
`ggshield secret scan repo <path-or-url>`.<sup>[\[9\]](#references)</sup>
...
- GitHub’s Code Search UI supports regex, while the REST/API path (including
`gh search code`) uses the legacy engine and does not expose regex features. Prefer the UI for regex queries.<sup>[\[3\]](#references)[\[5\]](#references)</sup>- GitHub search excludes files above its documented size limit and is not exhaustive. To be thorough, clone and scan locally with a secrets scanner.
...
```
Tip: For git history, prefer scanners that parse `git log -p --all` to catch removed secrets.[\[6\]](#references)
...
```
For additional code-search workflows, see [Wide Source Code Search](wide-source-code-search.html).
