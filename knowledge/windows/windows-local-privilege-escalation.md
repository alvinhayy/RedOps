...
[Uiaccess Admin Protection Bypass](uiaccess-admin-protection-bypass.html)

Secure Desktop accessibility registry propagation can be abused for an arbitrary SYSTEM registry write (RegPwn):[\[18\]](#references)

[Secure Desktop Accessibility Registry Propagation LPE (RegPwn)](secure-desktop-accessibility-registry-propagation-regpwn.html)
...
[Local NTLM Reflection via SMB Arbitrary Port](local-ntlm-reflection-via-smb-arbitrary-port.html)
...
[CTX_WSUSpect_White_Paper (1).pdf](</files/CTX_WSUSpect_White_Paper (1).pdf>)

**WSUS CVE-2020-1013**

[**Read the complete report here**](https://www.gosecure.net/blog/2020/09/08/wsus-attacks-part-2-cve-2020-1013-a-windows-10-local-privilege-escalation-1-day/).[\[33\]](#references)
...
Veeam B&R < `11.0.1.1261` exposes a localhost service on **TCP/9401** that processes attacker-controlled messages, allowing arbitrary commands as **NT AUTHORITY\SYSTEM**.[\[12\]](#references)
...
For more information about the flow of the attack check [https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/](https://research.nccgroup.com/2019/08/20/kerberos-resource-based-constrained-delegation-when-an-image-change-leads-to-a-privilege-escalation/)[\[36\]](#references)
...
If active, **plain-text passwords are stored in LSASS** (Local Security Authority Subsystem Service).[**More info about WDigest in this page**](../stealing-credentials/credentials-protections.html#wdigest).
...
Starting with **Windows 8.1**, Microsoft introduced enhanced protection for the Local Security Authority (LSA) to **block** attempts by untrusted processes to **read its memory** or inject code, further securing the system.[**More info about LSA Protection here**](../stealing-credentials/credentials-protections.html#lsa-protection).
...
**Domain credentials** are authenticated by the **Local Security Authority** (LSA) and utilized by operating system components. When a user’s logon data is authenticated by a registered security package, domain credentials for the user are typically established.[**More info about Cached Credentials here**](../stealing-credentials/credentials-protections.html#cached-credentials).
...
```
Always check for possible [**electron/cef/chromium debuggers** running, you could abuse it to escalate privileges](../../linux-hardening/software-information/electron-cef-chromium-debugger-abuse.html).
...
```
Look for dangerous ACLs granted to **`Everyone`**, **`BUILTIN\Users`**, or **`Authenticated Users`**, especially **`(F)`**, **`(M)`**, or **`(W)`** on the service executable or on the directory containing it. A practical abuse flow is:[\[27\]](#references)
...
Useful automated checks:[\[28\]](#references)
...
Some Windows Accessibility features create per-user **ATConfig** keys that are later copied by a **SYSTEM** process into an HKLM session key. A registry **symbolic link race** can redirect that privileged write into **any HKLM path**, giving an arbitrary HKLM **value write** primitive.[\[18\]](#references)
...
Example tooling (RegPwn BOF / standalone):[\[19\]](#references)
...
[AppendData/AddSubdirectory permission over service registry](appenddata-addsubdirectory-permission-over-service-registry.html)
...
[Notepad Plus Plus Plugin Autoload Persistence](notepad-plus-plus-plugin-autoload-persistence.html)
...
[Privilege Escalation with Autoruns](privilege-escalation-with-autorun-binaries.html)
...
[Arbitrary Kernel Rw Token Theft](arbitrary-kernel-rw-token-theft.html)
...
[Kernel Race Condition Object Manager Slowdown](kernel-race-condition-object-manager-slowdown.html)

#### Cancel-safe queue UAFs, paged-pool disclosures, and I/O ring pivots

Some Windows kernel LPE chains can be built from two individually weak bugs: a **cancel-safe queue lifetime race** that frees a request/CBD while the queue lock is still held, and a **lock-release-before-copy** disclosure that leaks a freed paged-pool allocation during `RtlCopyToUser`.[\[29\]](#references)
...
[Windows Registry Hive Exploitation](windows-registry-hive-exploitation.html)
...
This creates two useful primitives:[\[24\]](#references)[\[25\]](#references)
...
If you have **write permissions inside a folder present on PATH** you could be able to hijack a DLL loaded by a process and **escalate privileges**.[\[2\]](#references)
...
[Writable Sys Path +DLL Hijacking Privesc](dll-hijacking/writable-sys-path-dll-hijacking-privesc.html)

## Node.js / Electron module resolution hijacking via `C:\node_modules`

`C:\node_modules`
This is a **Windows uncontrolled search path** variant that affects **Node.js** and **Electron** applications when they perform a bare import such as `require("foo")` and the expected module is **missing**.[\[20\]](#references)

Node resolves packages by walking up the directory tree and checking `node_modules` folders on each parent. On Windows, that walk can reach the drive root, so an application launched from `C:\Users\Administrator\project\app.js` may end up probing:[\[21\]](#references)
...
Use **Procmon** to prove the resolution path:[\[23\]](#references)
...
mkdir C:\node_modules
```
1. Drop a module with the exact expected name:
...
module.exports = {}
```
1. Trigger the victim application. If the application attempts `require("foo")` and the legitimate module is absent, Node may load`C:\node_modules\foo.js` .
...
More [commands for network enumeration here](../basic-cmd-for-pentesters.html#network)
...
From [https://www.neowin.net/news/windows-7-exploring-credential-manager-and-windows-vault](https://www.neowin.net/news/windows-7-exploring-credential-manager-and-windows-vault)[\[34\]](#references)
...
**Note that to recover passwords from AppCmd.exe you need to be Administrator and run under a High Integrity level.****AppCmd.exe** is located in the `%systemroot%\system32\inetsrv\` directory.
...
More information about this technique here: [https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent/](https://blog.ropnop.com/extracting-ssh-private-keys-from-windows-10-ssh-agent/)[\[37\]](#references)
...
**MSF-Credentials Plugin****is a msf** plugin I have created this plugin to **automatically execute every metasploit POST module that searches for credentials** inside the victim.[**Winpeas**](https://github.com/carlospolop/privilege-escalation-awesome-scripts-suite) automatically search for all the files containing passwords mentioned in this page.[**Lazagne**](https://github.com/AlessandroZ/LaZagne) is another great tool to extract password from a system.
...
Then, if you have **full access to the low privileged process**, you can grab the **open handle to the privileged process created** with `OpenProcess()` and **inject a shellcode**.[Read this example for more information about **how to detect and exploit this vulnerability**.](leaked-handle-exploitation.html)[Read this **other post for a more complete explanation on how to test and abuse more open handlers of processes and threads inherited with different levels of permissions (not only full access)**](http://dronesec.pw/blog/2019/08/22/exploiting-leaked-process-and-thread-handles/).
...
When data is sent through a pipe by a **client**, the **server** that set up the pipe has the ability to **take on the identity** of the **client**, assuming it has the necessary **SeImpersonate** rights. Identifying a **privileged process** that communicates via a pipe you can mimic provides an opportunity to **gain higher privileges** by adopting the identity of that process once it interacts with the pipe you established. For instructions on executing such an attack, helpful guides can be found [**here**](named-pipe-client-impersonation.html) and [**here**](#from-high-integrity-to-system).
...
[Telephony Tapsrv Arbitrary Dword Write To Rce](telephony-tapsrv-arbitrary-dword-write-to-rce.html)
...
[Protocol Handler Shell Execute Abuse](../protocol-handler-shell-execute-abuse.html)
...
https://github.com/jas502n/CVE-2019-1388[\[35\]](#references)
...
The technique described [**in this blog post**](https://www.zerodayinitiative.com/blog/2022/3/16/abusing-arbitrary-file-deletes-to-escalate-privilege-and-other-great-tricks) with a exploit code [**available here**](https://github.com/thezdi/PoC/tree/main/FilesystemEoPs).[\[31\]](#references)[\[32\]](#references)
...
```
1. Step 3: Trigger SYSTEM process (e.g., `SilentCleanup` )
...
1. Step 4: Inside the oplock callback – redirect the deletion
...
When a **privileged service** writes logs/exports to a path read from a **writable config**, redirect that path with **Object Manager symlinks + NTFS mount points** to turn the privileged write into an arbitrary overwrite (even **without** SeCreateSymbolicLinkPrivilege).[\[15\]](#references)
...
```
1. Wait for the privileged component to write the log (e.g., admin triggers “send test SMS”). The write now lands in `C:\Windows\System32\cng.sys` .
2. Inspect the overwritten target (hex/PE parser) to confirm corruption; reboot forces Windows to load the tampered driver path → **boot loop DoS** . This also generalizes to any protected file a privileged service will open for write.
...
From a High Integrity process you could try to **enable the AlwaysInstallElevated registry entries** and **install** a reverse shell using a ***.msi*** wrapper.[More information about the registry keys involved and how to install a *.msi* package here.](#alwaysinstallelevated)
...
Using this technique is usually **selected any process running as SYSTEM with all the token privileges** (*yes, you can find SYSTEM processes without all the token privileges*).**You can find an** **example of code executing the proposed technique here****.**
...
If you want to read an example of [**how to go from high integrity to System using name pipes you should read this**](from-high-integrity-to-system-with-name-pipes.html).
...
[**Windows-Exploit-Suggester**](https://github.com/GDSSecurity/Windows-Exploit-Suggester) – Reads the output of **systeminfo** and recommends working exploits (local python)[**Windows Exploit Suggester Next Generation**](https://github.com/bitsadmin/wesng) – Reads the output of **systeminfo** and recommends working exploits (local Python)
