...
source_url: https://hacktricks.wiki/windows-hardening/windows-local-privilege-escalation/dll-hijacking/index.html
...
Several methods are employed for DLL hijacking, each with its effectiveness depending on the application’s DLL loading strategy:[\[4\]](#references)
...
[Windows CPython Build-Landmark and sys.path Hijacking](windows-cpython-build-landmark-sys-path-hijacking.html)

### AppDomainManager hijacking (`<exe>.config` + attacker assembly)

`<exe>.config` + attacker assembly)
Classic DLL sideloading is not the only way to make a trusted **.NET Framework** process load attacker code. If the target executable is a **managed** application, the CLR also consults an **application configuration file** named after the executable (for example `Setup.exe.config`). That file can define a custom **AppDomainManager**. If the config points to an attacker-controlled assembly placed next to the EXE, the CLR loads it **before the application’s normal code path** and runs inside the trusted process.[\[24\]](#references)

Per Microsoft’s .NET Framework configuration schema, both `<appDomainManagerAssembly>` and `<appDomainManagerType>` must be present for the custom manager to be used.[\[16\]](#references)[\[17\]](#references)
...
A practical intrusion pattern is to pair the trusted managed EXE with both a malicious `*.config` and a malicious AppDomainManager DLL that acts only as a **small bootstrapper**:[\[25\]](#references)
...
[Advanced Html Staged DLL Sideloading](advanced-html-staged-dll-sideloading.html)
...
If you are looking for a **missing DLL inside a specific executable**, set another filter such as **“Process Name” “contains” `<exec name>`**, execute it, and stop capturing events.[\[9\]](#references)
...
2. The system directory. Use the [**GetSystemDirectory**](https://docs.microsoft.com/en-us/windows/desktop/api/sysinfoapi/nf-sysinfoapi-getsystemdirectorya) function to get the path of this directory.(*C:\Windows\System32* )
...
**Related technique:** [oplock-gated mount-point switching against privileged remediation](../kernel-race-condition-object-manager-slowdown.html#applied-chain-oplock-gated-mount-point-switch-against-privileged-remediation).
...
```
1. Deliver the archive to the watched inbox/share; when the scheduled task re-launches the process it loads the malicious DLL and executes your code as the service account.
...
This technique has been observed in-the-wild to drive multi-stage sideloading chains: an initial launcher drops a helper DLL, which then spawns a Microsoft-signed, hijackable binary with a custom DllPath to force loading of the attacker’s DLL from a staging directory.[\[6\]](#references)
...
Note that you can **check your permissions in a folder** doing:[\[5\]](#references)
...
[Writable Sys Path +DLL Hijacking Privesc](writable-sys-path-dll-hijacking-privesc.html)
...
Windows Narrator.exe still probes a predictable, language-specific localization DLL on start that can be hijacked for arbitrary code execution and persistence.[\[7\]](#references)
...
This case demonstrates **Phantom DLL Hijacking** in Lenovo’s TrackPoint Quick Menu (`TPQMAssistant.exe`), tracked as **CVE-2025-1729**.[\[2\]](#references)[\[3\]](#references)
...
Threat actors frequently pair MSI-based droppers with DLL side-loading to execute payloads under a trusted, signed process.[\[10\]](#references)
...
Check Point described how Ink Dragon deploys ShadowPad using a **three-file triad** to blend in with legitimate software while keeping the core payload encrypted on disk:[\[12\]](#references)
...
Operators pair DLL sideloading with LOLBAS so the only custom artifact on disk is the malicious DLL next to the trusted EXE:[\[1\]](#references)
...
- Works with binaries that prefer local DLLs (e.g.,
-
**Hunting:** Alert on`forfiles` when`/p` ,`/m` , and`/c` appear together; uncommon outside admin scripts.

## [Case Study: NSIS dropper + Bitdefender Submission Wizard sideload (Chrysalis)](#case-study-nsis-dropper--bitdefender-submission-wizard-sideload-chrysalis)

A recent Lotus Blossom intrusion abused a trusted update chain to deliver an NSIS-packed dropper that staged a DLL sideload plus fully in-memory payloads.[\[13\]](#references)
...
## [Signed-host sideloading with export proxying + host thread parking](#signed-host-sideloading-with-export-proxying--host-thread-parking)

Some DLL sideloading chains add **stability engineering** so the legitimate host stays alive long enough to load later stages cleanly instead of crashing after the malicious DLL is loaded.[\[11\]](#references)
