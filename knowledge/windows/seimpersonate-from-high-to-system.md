...
source_url: https://hacktricks.wiki/windows-hardening/windows-local-privilege-escalation/seimpersonate-from-high-to-system.html
...
The following code comes from [this access-token article](https://medium.com/@seemant.bisht24/understanding-and-abusing-access-tokens-part-ii-b9069f432962). It accepts a **process ID as an argument** and starts a command shell **as the user** of that process.[\[3\]](#references)

Running in a High Integrity process you can **indicate the PID of a process running as System** (like `winlogon`, `wininit`) and execute a `cmd.exe` as SYSTEM.[\[3\]](#references)
