...
source_url: https://hacktricks.wiki/windows-hardening/windows-local-privilege-escalation/access-tokens.html
...
`runas /netonly` details
`runas /netonly` (and C2 helpers such as `make_token`) creates a **`LOGON32_LOGON_NEW_CREDENTIALS`** token. This is very useful to understand during lateral movement because:[\[3\]](#references)
...
Some practical notes that are easy to forget while operating:[\[1\]](#references)
...
If you already have a **service** or **SYSTEM** context and a **privileged user is logged on**, stealing or duplicating that user’s token is often quieter than dumping **LSASS**. In many real intrusions this is enough to:[\[2\]](#references)
...
For examples of **session/user token hijacking** from a privileged context, check [**WTS Impersonator**](../stealing-credentials/wts-impersonator.html). Remember that APIs such as **`WTSQueryUserToken`** are meant for **highly trusted services** and normally require **`LocalSystem` + `SeTcbPrivilege`**, so they are primarily useful once you already control a service-level context. For privilege-specific ways to obtain **SYSTEM** first, check the pages below.
