...
source_url: https://hacktricks.wiki/windows-hardening/stealing-credentials/index.html
...
Utilization of the **comsvcs.dll** is feasible for dumping the lsass process, thereby eliminating the need to upload and execute procdump. This method is described in detail at [https://en.hackndo.com/remote-lsass-dump-passwords/](https://en.hackndo.com/remote-lsass-dump-passwords).[\[9\]](#references)
...
Ink Dragon ships a three-stage dumper dubbed **LalsDumper** that never calls `MiniDumpWriteDump`, so EDR hooks on that API never fire:[\[3\]](#references)
...
```
Code from the book: [https://0xword.com/es/libros/99-hacking-windows-ataques-a-sistemas-y-redes-microsoft.html](https://0xword.com/es/libros/99-hacking-windows-ataques-a-sistemas-y-redes-microsoft.html)[\[7\]](#references)
...
Christoffer Andersson’s database-layer research documents these tables and their version-specific behavior in more detail.[\[8\]](#references)
...
Ink Dragon’s FinalDraft RAT includes a `DumpRDPHistory` tasker whose techniques are handy for any red-teamer:[\[3\]](#references)
...
The same implant also tampers with several registry keys to make credential theft easier:[\[3\]](#references)
...
hMailServer stores its DB password in `C:\Program Files (x86)\hMailServer\Bin\hMailServer.ini` under `[Database] Password=`. The value is Blowfish-encrypted with the static key `THIS_KEY_IS_NOT_SECRET` and 4-byte word endianness swaps. Use the hex string from the INI with this Python snippet:[\[2\]](#references)
...
SQL Server Management Studio (SSMS) stores saved connection information in a per-user `sqlstudio.bin` file. Dedicated dumpers can parse the file and recover saved SQL credentials. In shells that only return command output, the file is often exfiltrated by encoding it as Base64 and printing it to stdout.[\[1\]](#references)
...
If code execution is obtained as the **victim user** on a Windows host using **Chrome + Google Password Manager synced passkeys**, passkeys become an interesting post-exploitation target even **without admin/SYSTEM**.[\[4\]](#references)
...
1. **Pass-ta-key / device-identity relay**<sup>[\[4\]](#references)</sup>  - Enumerate `WebauthnCredentialSpecifics` from Chrome’s LevelDB.
...
2. Enumerate
3. **Pending UV-key hijack**<sup>[\[4\]](#references)</sup>  - Force re-onboarding by deleting `passkey_enclave_state` or by sending a valid signed`device/forget` operation.
...
4. Force re-onboarding by deleting
5. **Master-secret / SDS recovery theft**<sup>[\[4\]](#references)</sup>  - Force recovery or rejoin so Chrome fetches the synced-passkey master secret.
