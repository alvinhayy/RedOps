...
source_url: https://hacktricks.wiki/windows-hardening/windows-local-privilege-escalation/telephony-tapsrv-arbitrary-dword-write-to-rce.html
...
```
`pszDomainUser` is a mailslot path (`\\*\MAILSLOT\...` ). Any**existing filesystem path** writable by`NETWORK SERVICE` is accepted.
...
## [Forcing Deterministic Writes](#forcing-deterministic-writes)
...
## [From DWORD Write to RCE inside TapiSrv](#from-dword-write-to-rce-inside-tapisrv)
...
3. Works with UNC paths to a real Windows SMB share; some attacker SMB servers fail with
4. **Payload** : the export executes under`NETWORK SERVICE` . A minimal DLL can run`cmd.exe /c whoami /all > C:\Windows\Temp\poc.txt` and return a non-zero value (e.g.,`0x1337` ) so the service unloads the DLL, confirming execution.<sup>[\[1\]](#references)</sup>

## [Hardening / Detection Notes](#hardening--detection-notes)
