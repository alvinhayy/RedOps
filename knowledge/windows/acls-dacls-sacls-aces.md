...
source_url: https://hacktricks.wiki/windows-hardening/windows-local-privilege-escalation/acls-dacls-sacls-aces.html
...
The process of accessing a file involves the system checking the object’s security descriptor against the user’s access token to determine if access should be granted and the extent of that access, based on the ACEs.[\[1\]](#references)
...
There arey **three main types of Access Control Entries (ACEs)**:[\[1\]](#references)
...
Each ACE has **four critical components**:[\[1\]](#references)
...
Access determination is conducted by sequentially examining each ACE until:[\[1\]](#references)
...
The way **ACEs** (rules that say who can or cannot access something) are put in a list called **DACL** is very important. This is because once the system gives or denies access based on these rules, it stops looking at the rest.[\[1\]](#references)
...
When managing access to resources, like a folder, we use lists and rules known as Access Control Lists (ACLs) and Access Control Entries (ACEs). These define who can or cannot access certain data.[\[1\]](#references)
