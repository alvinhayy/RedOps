...
source_url: https://hacktricks.wiki/windows-hardening/stealing-credentials/credentials-mimikatz.html
...
On modern workstations this matters even more because **Credential Guard is enabled by default on many Windows 11 22H2+ and Windows Server 2025 domain-joined, non-DC systems**, while **LSASS-as-PPL is enabled by default on fresh Windows 11 22H2+ installs**. In practice, this means `sekurlsa::logonpasswords` often yields less material than older tradecraft expected and operators increasingly pivot to **offline minidumps**, **Kerberos key extraction (`sekurlsa::ekeys`)**, or **CloudAP/PRT-oriented modules**. For the protection side, check [Windows credentials protections](credentials-protections.html).
...
Use the commands below as quick syntax reminders. The dedicated pages for [golden tickets](../active-directory-methodology/golden-ticket.html), [silver tickets](../active-directory-methodology/silver-ticket.html), [diamond tickets](../active-directory-methodology/diamond-ticket.html), and [over-pass-the-hash / pass-the-key](../active-directory-methodology/over-pass-the-hash-pass-the-key.html) contain the up-to-date AES/PAC/opsec nuances.
...
If `RC4` is disabled or unreliable, Mimikatz can patch **AES128/AES256 Kerberos keys** into the current logon session instead of only using an NT hash. This is usually a better fit for modern domains than treating `sekurlsa::pth` as NTLM-only.[\[1\]](#references)
...
-
**DCShadow** : Temporarily make a machine act as a DC for AD object manipulation. See[DCShadow](../active-directory-methodology/dcshadow.html) .
  - `mimikatz "lsadump::dcshadow /object:targetObject /attribute:attributeName /value:newValue" exit`
-
**DCSync** : Mimic a DC to request password data. See[DCSync](../active-directory-methodology/dcsync.html) .
...
On **Entra ID** or **hybrid-joined** hosts, `sekurlsa::cloudap` can expose cached **Primary Refresh Token (PRT)** material from LSASS. If the associated Proof-of-Possession key is software-protected, `dpapi::cloudapkd` can derive the clear/derived key material needed for follow-on **Pass-the-PRT** workflows.[\[1\]](#references)
