...
source_url: https://hacktricks.wiki/windows-hardening/windows-local-privilege-escalation/privilege-escalation-with-autorun-binaries.html
...
All the binaries located in the **Startup folders are going to be executed on startup**. The common startup folders are the ones listed a continuation, but the startup folder is indicated in the registry. [Read this to learn where.](privilege-escalation-with-autorun-binaries.html#startup-path)
...
[Archive Extraction Path Traversal](../../generic-hacking/archive-extraction-path-traversal.html)
...
Registry keys known as **Run** and **RunOnce** are designed to automatically execute programs every time a user logs into the system. The command line assigned as a key’s data value is limited to 260 characters or less.[\[2\]](#references)
...
On Windows Vista and later versions, the **Run** and **RunOnce** registry keys are not automatically generated. Entries in these keys can either directly start programs or specify them as dependencies. For instance, to load a DLL file at logon, one could use the **RunOnceEx** registry key along with a “Depend” key. This is demonstrated by adding a registry entry to execute “C:\temp\evil.dll” during the system start-up:[\[2\]](#references)
...
This per-user registry value can point to a script or command that is executed when that user logs on. It is mainly a **persistence** primitive because it only runs in the context of the affected user, but it is still worth checking during post-exploitation and autoruns reviews.[\[3\]](#references)[\[6\]](#references)[\[7\]](#references)
...
Typically, the **Userinit** key is set to **userinit.exe**. However, if this key is modified, the specified executable will also be launched by **Winlogon** upon user logon. Similarly, the **Shell** key is intended to point to **explorer.exe**, which is the default shell for Windows.[\[1\]](#references)
...
Steps to create a boot option for automatically starting in “Safe Mode with Command Prompt”:[\[5\]](#references)
...
- **IsInstalled:**  - `0` indicates the component’s command will not execute.
...
**Find more Autoruns like registries in** **https://www.microsoftpressstore.com/articles/article.aspx?p=2762082&seqNum=2**[\[4\]](#references)
