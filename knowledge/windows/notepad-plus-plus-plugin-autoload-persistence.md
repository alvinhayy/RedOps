...
source_url: https://hacktricks.wiki/windows-hardening/windows-local-privilege-escalation/notepad-plus-plus-plugin-autoload-persistence.html
...
- Writable options for low-privileged operators:<sup>[\[1\]](#references)</sup>  - Use the **portable Notepad++ build** in a user-writable folder.
...
Notepad++ expects specific **exported functions**. These are all called during initialization, giving multiple execution surfaces:[\[1\]](#references)
...
Compile a DLL with the expected exports and place it in `plugins\\MyNewPlugin\\MyNewPlugin.dll` under a writable Notepad++ folder:[\[1\]](#references)
...
A weaponized plugin can turn Notepad++ into a **reflective DLL loader**:[\[1\]](#references)
