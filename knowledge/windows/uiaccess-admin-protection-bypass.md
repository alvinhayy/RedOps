...
source_url: https://hacktricks.wiki/windows-hardening/windows-local-privilege-escalation/uiaccess-admin-protection-bypass.html
...
- Limited admin user (user is in Administrators but running filtered) ➜
...
`AiCheckSecureApplicationDirectory`)
...
Run the PowerShell helper to discover writable/overwritable objects inside nominally secure roots from the perspective of a chosen token:[\[1\]](#references)
