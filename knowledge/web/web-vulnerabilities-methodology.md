...
source_url: https://hacktricks.wiki/pentesting-web/web-vulnerabilities-methodology.html
...
[Reflecting Techniques - PoCs and Polygloths CheatSheet](pocs-and-polygloths-cheatsheet/index.html)
...
For deeper exploitation ideas, check [Abusing Service Workers](xss-cross-site-scripting/abusing-service-workers.html), [WebAssembly linear memory corruption to DOM XSS](xss-cross-site-scripting/wasm-linear-memory-template-overwrite-xss.html), and [Code Review Tooling](../network-services-pentesting/pentesting-web/code-review-tools.html).

#### File System Access API: browser-native file read/write abuse

Chromium-family browsers expose **`showOpenFilePicker()`**, **`showSaveFilePicker()`**, and **`showDirectoryPicker()`** to trusted pages in a **secure context** and after a **user gesture**.[\[6\]](#references)<sup>[\[7\]](#references)</sup> If a target web app, phishing lure, or malicious dependency can convince the user to approve a directory with **read-write** access, the page can operate on the selected files **without dropping a native payload**.[\[5\]](#references)
...
-  [**Cross Site Request Forgery**](csrf-cross-site-request-forgery.html)
-  [**Cross-site WebSocket hijacking (CSWSH)**](websocket-attacks.html)
-  [**Phone Number Injections**](phone-number-injections.html)
-  [**PostMessage Vulnerabilities**](postmessage-vulnerabilities/index.html)

#### Cross-site WebSocket hijacking & localhost abuse

WebSocket upgrades automatically forward cookies and do not block `ws://127.0.0.1`, so **any web origin can drive desktop IPC endpoints** that skip `Origin` validation. When you spot a launcher exposing a JSON-RPC-like API through a local agent:[\[1\]](#references)
...
Passkeys are **origin-bound**, so the usual bug is not “steal the secret” but **abuse the workflow around the ceremony**:[\[4\]](#references)
...
See [Account Takeover](account-takeover.html#qr--cross-device-login-flows) and [Clickjacking](clickjacking.html#browser-extensions-dom-based-autofill-clickjacking) for concrete attack patterns.
...
```
Useful follow-up reading: [Code Review Tooling](../network-services-pentesting/pentesting-web/code-review-tools.html) and [Next.js](../network-services-pentesting/pentesting-web/nextjs.html#nextjs-server-actions-enumeration-hash-to-function-name-via-source-maps).
