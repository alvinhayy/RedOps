...
source_url: https://hacktricks.wiki/pentesting-web/ssti-server-side-template-injection/jinja2-ssti.html
...
```
Source: [https://jinja.palletsprojects.com/en/2.11.x/templates/#debug-statement](https://jinja.palletsprojects.com/en/2.11.x/templates/#debug-statement)[\[1\]](#references)[\[6\]](#references)
...
First of all, in a Jinja injection you need to **find a way to escape from the sandbox** and recover access the regular python execution flow. To do so, you need to **abuse objects** that are **from** the **non-sandboxed environment but are accessible from the sandbox**.[\[2\]](#references)[\[3\]](#references)
...
`{% ... %}`
Sometimes `{{ ... }}` is blocked, sanitized or the injection lands inside a statement-friendly context. In those cases you can still abuse Jinja statement tags such as `{% with %}`, `{% if %}`, `{% for %}`, `{% set %}` and, in newer versions, `{% print %}` to execute code, leak data through the block body, or trigger blind side effects.[\[1\]](#references)
...
{% endraw %}
```
If the target filters some chars but still allows statement tags, combine this idea with the [filter bypasses](jinja2-ssti.html#filter-bypasses) and the [no-`{{` / no-`.` / no-`_` example](jinja2-ssti.html#without-several-chars). Also remember that `{% print %}` is not mandatory: on targets where it is unavailable, `{% with %}`, `{% if %}`, `{% set %}` and `{% for %}` are usually enough to keep exploiting the template.
...
Some of these bypasses appeared in the preceding examples; the following list summarizes them:[\[3\]](#references)[\[5\]](#references)
...
{% endraw %}
```
- [**Return here for more options to access a global object**](jinja2-ssti.html#accessing-global-objects)
- [**Return here for more options to access the object class**](jinja2-ssti.html#recovering-less-than-class-object-greater-than)
- [**Read this to get RCE without the object class**](jinja2-ssti.html#jinja-injection-without-less-than-class-object-greater-than)

**Avoiding HTML encoding**

By default Flask HTML encode all the inside a template for security reasons:[\[3\]](#references)
...
From the [**global objects**](jinja2-ssti.html#accessing-global-objects) there is another way to get to **RCE without using that class.**
