...
source_url: https://hacktricks.wiki/pentesting-web/file-inclusion/lfi2rce-via-nginx-temp-files.html
...
The original nginx-assisted LFI research showed that even the following one-liner is enough when PHP runs behind an nginx reverse proxy that buffers request bodies to disk:[\[1\]](#references)
...
- PHP’s include/require resolves those `/proc/.../fd/...` symlinks, so an attacker with LFI can hop through procfs to execute the buffered temp file even after nginx deletes it.<sup>[\[1\]](#references)</sup>
...
- The original downloadable lab archive.<sup>[\[4\]](#references)</sup>
- Two 2021 challenge instances preserved as historical lab targets.<sup>[\[5\]](#references)</sup><sup>[\[6\]](#references)</sup>
