...
source_url: https://hacktricks.wiki/pentesting-web/file-inclusion/lfi2rce-via-temp-file-uploads.html
...
When PHP receives a `multipart/form-data` POST containing a file upload, it stores the body in a temporary file. Application code can persist it with `move_uploaded_file()`; otherwise PHP removes it at the end of the request. The exploit races an LFI against that cleanup.[\[1\]](#references)[\[2\]](#references)
...
On GNU/Linux, temporary names have substantially more entropy, so direct name brute force is generally impractical; other disclosure or race primitives may still expose the pathname.[\[1\]](#references)
