...
source_url: https://hacktricks.wiki/pentesting-web/orm-injection.html
...
[**This post**](https://www.elttam.com/blog/plormbing-your-django-orm/) explains how directly expanding attacker-controlled data into Django ORM filters can expose an application, for example:[\[1\]](#references)
...
`where` filters (operator injection)
Prisma’s query API accepts either primitive values or operator objects. When handlers assume the request body contains plain strings but pass them directly to `where`, attackers can smuggle operators into authentication flows and bypass token checks.[\[3\]](#references)
...
These tricks where **found in this post****.**[\[4\]](#references)
...
String comparisons inherit the database collation, so leak oracles must be designed around how the backend orders characters:[\[3\]](#references)
