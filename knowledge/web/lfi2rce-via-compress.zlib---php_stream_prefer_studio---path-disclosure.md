...
source_url: https://hacktricks.wiki/pentesting-web/file-inclusion/lfi2rce-via-compress.zlib-+-php_stream_prefer_studio-+-path-disclosure.html
...
`compress.zlib://` and `PHP_STREAM_PREFER_STDIO`
The interesting file is the **seekability backing file**, not a file containing decompressed output. In current php-src, `php_stream_gzopen()` opens the resource nested after `compress.zlib://` with `STREAM_MUST_SEEK | STREAM_WILL_CAST`. When that inner resource is a non-seekable HTTP stream, PHP selects `PHP_STREAM_PREFER_STDIO`, creates a real temporary file with `php_stream_fopen_tmpfile()`, and copies the raw HTTP response body into it before zlib reads from it.[\[1\]](#references)[\[3\]](#references)
...
If only a plain LFI is available and there is no wrapper-controlled fetch or path leak, use a technique matching the available primitive, such as [phpinfo() temporary uploads](lfi2rce-via-phpinfo.html), [temporary file upload races](lfi2rce-via-temp-file-uploads.html), or [eternal waiting](lfi2rce-via-eternal-waiting.html).

### Race condition to RCE

The hxp 36C3 CTF `includer` challenge demonstrated the complete chain.[\[1\]](#references)
...
```
The large declared `Content-Length` is a synchronization primitive: it prevents PHP from observing end-of-body after the safe prefix. Do not close the connection until the payload has been copied and the competing local include has run.[\[1\]](#references)
