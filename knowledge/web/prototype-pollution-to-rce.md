...
source_url: https://hacktricks.wiki/pentesting-web/deserialization/nodejs-proto-prototype-pollution/prototype-pollution-to-rce.html
...
According to this [**archived writeup**](https://web.archive.org/web/20260000000000id_/https://research.securitum.com/prototype-pollution-rce-kibana-cve-2019-7609/), when a **process is spawned** through **`child_process`** (for example, with `fork` or `spawn`), Node calls `normalizeSpawnArguments`, which historically exposed a **prototype-pollution gadget for creating environment variables**:[\[1\]](#references)
...
A similar payload to the previous one with some changes was proposed in **this writeup****.**<sup>[\[2\]](#references)</sup> The main differences are:
...
// This should create the file /tmp/pp2rec
```
## [Filesystem-less PP2RCE via `--import` (Node ≥ 19)](#filesystem-less-pp2rce-via---import-node--19)
...
The attack is conceptually identical to the `--require /proc/self/*` tricks shown above, but instead of pointing to a file we embed the payload directly in a base64-encoded `data:` URL:[\[6\]](#references)
...
```
### [Why `--import` helps](#why---import-helps)
...
`--import` support in `NODE_OPTIONS` is present in Node 22.2.0. The Node core team has discussed restricting data URIs; test the exact target version because command-line option handling continues to evolve.[\[6\]](#references)[\[7\]](#references)

## [DNS Interaction](#dns-interaction)

Using the following payloads it’s possible to abuse the NODE_OPTIONS env var we have discussed previously and detect if it worked with a DNS interaction:[\[3\]](#references)
...
```
## [PP2RCE vuln child_process functions](#pp2rce-vuln-child_process-functions)
...
In the previous examples, triggering the gadget requires functionality that **calls `spawn`** (the command-execution methods in **`child_process`** ultimately call it). The previous example contained such a call, but the target application may not.

### Controlling a require file path

In this [**other writeup**](https://blog.sonarsource.com/blitzjs-prototype-pollution/), the user can control the file path on which **`require`** will operate.<sup>[\[2\]](#references)</sup> In that scenario, the attacker needs to **find a `.js` file on the system** that will **execute a spawn method when imported.**
...
If the code executes `require` after the prototype pollution, you may be able to **force a different path through prototype pollution** even when you do not directly control the requested path. Thus, code such as `require("./a_file.js")` or `require("bytes")` may load the path selected through the polluted package metadata.

Therefore, if a require is executed after your prototype pollution and no spawn function, this is the attack:

- Find a **`.js` file inside the system** that when**required** will**execute something using `child_process`**  - If you can upload files to the platform you are attacking you might upload a file like that
...
Similar to the previous one, this was found in [**this writeup**](https://blog.huli.tw/2022/12/26/en/ctf-2022-web-js-summary/#balsn-ctf-2022-2linenodejs).[\[4\]](#references)
...
In the paper [https://arxiv.org/pdf/2207.11171.pdf](https://arxiv.org/pdf/2207.11171.pdf) is also indicated that the control of **`contextExtensions`** from some methods of the **`vm`** library could be used as a gadget.[\[5\]](#references)
...
[!INFO]
**Node 20 and later releases** shipped further hardening: several `child_process` helpers copy user-supplied `options` with **`CopyOptions()`** instead of using them by reference. This blocks pollution of nested objects such as `stdio`, but **does not by itself protect against the `NODE_OPTIONS` / `--import` tricks** described above because those flags may still arrive through environment variables.<sup>[\[7\]](#references)</sup>
...
Additional gadget collections and CTF examples can help identify application-specific sinks beyond `child_process`.[\[8\]](#references)
