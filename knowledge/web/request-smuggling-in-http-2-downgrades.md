...
source_url: https://hacktricks.wiki/pentesting-web/http-request-smuggling/request-smuggling-in-http-2-downgrades.html
...
5. connection-specific headers such as
...
```
`* Using HTTP2` appears, the edge speaks H2.
...
If you can only poison requests on your **own** client-mapped upstream connection, the bug can still be useful for cache poisoning, internal-header leaks, or front-end control bypass. For those reuse-locked cases, pivot to [HTTP Connection Request Smuggling](../http-connection-request-smuggling.html).

## Modern downgrade-only injection gadgets

Many modern front-ends already strip a literal `transfer-encoding: chunked` header. Recent findings often work by **manufacturing dangerous HTTP/1.1 bytes during the downgrade itself** rather than sending them as a normal header.[\[1\]](#references)
...
For proxy-specific quirks and tunnel-focused payloads, see [Upgrade Header Smuggling](../h2c-smuggling.html).
