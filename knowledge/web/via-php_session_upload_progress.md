...
source_url: https://hacktricks.wiki/pentesting-web/file-inclusion/via-php_session_upload_progress.html
...
PHP defaults `session.upload_progress.cleanup` to `On`, so progress data is removed as soon as the upload is processed. Exploitation is therefore a race unless cleanup has been disabled.[\[4\]](#references)

### The CTF

In the [**original CTF**](https://blog.orange.tw/posts/2018-10-hitcon-ctf-2018-one-line-php-challenge/) where this technique is described, winning the race was not enough: the loaded content also needed to start with the string `@<?php`.[\[1\]](#references)
...
More information is available in the original writeup and the final exploit.[\[1\]](#references)[\[2\]](#references)
