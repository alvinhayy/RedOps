...
source_url: https://hacktricks.wiki/pentesting-web/cache-deception/cache-poisoning-via-url-discrepancies.html
...
Different HTTP servers and proxies like Nginx, Node, and CloudFront decode delimiters differently, leading to inconsistencies across CDNs and origin servers that could be exploited. For example, if the web server perform this transformation `/myAccount%3Fparam` → `/myAccount?param` but the cache server keeps as key the path `/myAccount%3Fparam`, there is an inconsistency.[\[1\]](#references)

A way to check for these inconsistencies is to send requests URL encoding different chars after loading the path without any encoding and check if the encoded path response came from the cached response.[\[1\]](#references)

### Dot segment

Dot-segment normalization is another source of discrepancies. For `/static/../home/index` or `/aaa..\home/index`, one component may key the literal path while another resolves dot segments or backslashes. Compare a cache-busted baseline with the normalized and non-normalized variants, and use only non-sensitive test accounts because a successful cache-deception probe may publish a response.[\[1\]](#references)
