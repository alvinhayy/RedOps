...
source_url: https://hacktricks.wiki/pentesting-web/xpath-injection.html
...
An attack technique known as XPath Injection is utilized to take advantage of applications that form XPath (XML Path Language) queries based on user input to query or navigate XML documents.[\[2\]](#references)

### Nodes Described

Expressions are used to select various nodes in an XML document. These expressions and their descriptions are summarized below:[\[3\]](#references)
...
IMPORTANT: Notice that the **“and” is the first operation made**.[\[1\]](#references)
...
The output contains strings and the user can manipulate the values to search:[\[1\]](#references)
...
- [2] [OWASP Testing Guide - Testing for XPath Injection (OTG-INPVAL-010)](<https://wiki.owasp.org/index.php/Testing_for_XPath_Injection_(OTG-INPVAL-010)>)
