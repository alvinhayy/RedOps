...
source_url: https://hacktricks.wiki/pentesting-web/sql-injection/index.html
...
### [Identifying with PortSwigger](#identifying-with-portswigger)
...
First, determine the **number of columns** returned by the **original query**, because both sides of the `UNION` must return the same number.
...
## [Exploiting Error based](#exploiting-error-based)
...
The preceding sections cover generic SQL injection exploitation. The following pages contain additional DBMS-specific techniques:

Or you will find **a lot of tricks regarding: MySQL, PostgreSQL, Oracle, MSSQL, SQLite and HQL in** **https://github.com/swisskyrepo/PayloadsAllTheThings/tree/master/SQL%20Injection**[\[1\]](#references)

## [Authentication bypass](#authentication-bypass)

List to try to bypass the login functionality:

### [Raw hash authentication Bypass](#raw-hash-authentication-bypass)
...
```
### [Injected hash authentication Bypass](#injected-hash-authentication-bypass)
...
data = {"login": chr(0xbf) + chr(0x27) + "OR 1=1 #", "password":"test"}
r = requests.post(url, data=data, cookies=cookies, headers={'referrer':url})
print r.text
```
...
### [Modify password of existing object/user](#modify-password-of-existing-objectuser)
...
*Note: This attack will no longer work as described above in latest MySQL installations. While comparisons still ignore trailing whitespace by default, attempting to insert a string that is longer than the length of a field will result in an error, and the insertion will fail. For more information about about this check:* *https://heinosass.gitbook.io/leet-sheet/web-app-hacking/exploitation/interesting-outdated-attacks/sql-truncation*[\[5\]](#references)
...
You can find a more in-depth explanation of this trick in the [GoSecure blog](https://www.gosecure.net/blog/2021/10/19/a-scientific-notation-bug-in-mysql-left-aws-waf-clients-vulnerable-to-sql-injection/).[\[8\]](#references)
...
### [Column/tablename injection in SELECT list via subqueries](#columntablename-injection-in-select-list-via-subqueries)
...
### [SQLi via AST/filter-to-SQL converters (JSON_VALUE predicates)](#sqli-via-astfilter-to-sql-converters-json_value-predicates)
...
[GitHub - m4ll0k/Atlas: Quick SQLMap Tamper Suggester \xc2\xb7 GitHub](https://github.com/m4ll0k/Atlas)

## [Other Guides](#other-guides)

- [https://sqlwiki.netspi.com/](https://sqlwiki.netspi.com)
- [https://github.com/swisskyrepo/PayloadsAllTheThings/tree/master/SQL%20Injection](https://github.com/swisskyrepo/PayloadsAllTheThings/tree/master/SQL%20Injection)<sup>[\[1\]](#references)</sup>

## [Brute-Force Detection List](#brute-force-detection-list)

[Auto_Wordlists/wordlists/sqli.txt at main \xc2\xb7 carlospolop/Auto_Wordlists \xc2\xb7 GitHub](https://github.com/carlospolop/Auto_Wordlists/blob/main/wordlists/sqli.txt)
