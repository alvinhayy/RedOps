...
source_url: https://hacktricks.wiki/pentesting-web/content-security-policy-csp-bypass/index.html
...
- If you have JS limited execution it’s possible to get a used nonce inside the page with
...
```
Working payload: `"/><script>alert(1);</script>`[\[4\]](#references)

#### self + ‘unsafe-inline’ via Iframes

[CSP bypass: self + ‘unsafe-inline’ with Iframes](csp-bypass-self-+-unsafe-inline-with-iframes.html)
...
```
1. If the victim has credentials saved for that origin, the browser password manager may **autofill the injected fields** .
2. Because the form has no `method` , HTML defaults to**`GET`** , so clicking submit moves the credentials into the URL, such as`/?email=victim%40mail.com&password=Secret123` .
3. If the injection is reflected again, a second-stage payload can force a navigation that leaks that credential-bearing URL in the **`Referer`** header:
...
>
```
#### [Payloads using Angular + a library with functions that return the `window` object (](#payloads-using-angular--a-library-with-functions-that-return-the-window-object-check-out-this-post)[check out this post](https://blog.huli.tw/2022/09/01/en/angularjs-csp-bypass-cdnjs/)):
...
```
You should be able to exfiltrate data, similarly as it has always be done with [Google Analytics](https://www.humansecurity.com/tech-engineering-blog/exfiltrating-users-private-data-using-google-analytics-to-bypass-csp)/[Google Tag Manager](https://blog.deteact.com/csp-bypass/).[\[14\]](#references)<sup>[\[15\]](#references)</sup> In this case, you follow these general steps:
...
However, according to the description in [CSP spec 4.2.2.3. Paths and Redirects](https://www.w3.org/TR/CSP2/#source-list-paths-and-redirects), if the redirection leads to a different path, it can bypass the original restrictions.[\[18\]](#references)
...
You can bypass this CSP by exfiltrating the data via images (in this occasion the XSS abuses a CSRF where a page accessible by the bot contains an SQLi, and extract the flag via an image):[\[19\]](#references)
...
**Research:** **https://portswigger.net/research/bypassing-csp-with-policy-injection**[\[21\]](#references)
...
This time a **flag** is going to be extracted, whenever a **char is correctly guessed** via SQLi the **response** takes **more time** due to the sleep function. Then, you will be able to extract the flag:[\[19\]](#references)
...
For more information [**check the original report here**](https://socradar.io/csp-bypass-unveiled-the-hidden-threat-of-bookmarklets/).[\[22\]](#references)
...
```
In [**this CTF writeup**](https://github.com/aszx87410/ctf-writeups/issues/48), it was possible via **HTML injection** to **restrict** more a **CSP** so a script preventing CSTI was disabled and therefore the **vulnerability became exploitable.**[\[24\]](#references)
...
For an example [**check this CTF writeup**](https://github.com/maple3142/My-CTF-Challenges/tree/master/TSJ%20CTF%202022/Nim%20Notes).[\[25\]](#references)
...
From [**this writeup**](https://blog.ssrf.kr/69) it looks like it was possible to bypass a CSP protection by loading an error page (potentially without CSP) and rewriting its content.[\[30\]](#references)[\[34\]](#references)
...
[SOME - Same Origin Method Execution](../xss-cross-site-scripting/some-same-origin-method-execution.html)
...
The csper.io documentation describes generating a candidate policy from observed resources. A separate video walkthrough is available in reference 33.[\[33\]](#references)
