...
source_url: https://hacktricks.wiki/pentesting-web/client-side-template-injection-csti.html
...
OWASP’s current WSTG recommends identifying the framework first and then checking whether your reflection is re-parsed as a template rather than only inserted as inert text/HTML.[\[1\]](#references)
...
```
You can find a very **basic online example** of the vulnerability in **AngularJS** in [http://jsfiddle.net/2zs2yv7o/](http://jsfiddle.net/2zs2yv7o/) and in **Burp Suite Academy**
...
```
A really good post on CSTI in VUE can be found in [https://portswigger.net/research/evading-defences-using-vuejs-script-gadgets](https://portswigger.net/research/evading-defences-using-vuejs-script-gadgets)[\[2\]](#references)
...
```
The exact helper name exposed in the rendered template can vary depending on the Vue version / build output, so once you confirm Vue 3 CSTI, enumerate nearby helpers instead of assuming `_openBlock` is always present.[\[2\]](#references)
...
[self.alert(1)mod1]
```
**More payloads in** **https://portswigger.net/research/abusing-javascript-frameworks-to-bypass-xss-mitigations**[\[3\]](#references)
...
If the target uses AngularJS heavily, [**ACSTIS / angularjs-csti-scanner**](https://github.com/tijme/angularjs-csti-scanner) can help with **crawling**, **version-aware payload selection**, and optional **payload verification** to reduce false positives.
...
[Auto_Wordlists/wordlists/ssti.txt at main \xc2\xb7 carlospolop/Auto_Wordlists \xc2\xb7 GitHub](https://github.com/carlospolop/Auto_Wordlists/blob/main/wordlists/ssti.txt)
