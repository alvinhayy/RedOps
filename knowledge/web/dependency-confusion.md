...
source_url: https://hacktricks.wiki/pentesting-web/dependency-confusion.html
...
Dependency confusion is not the only way to get install-time execution. If an attacker compromises the maintainer account or publishing token of a legitimate package, they can publish a malicious version of the real package and obtain code execution on every machine that installs it.[\[5\]](#references)
...
`npx` binary-to-package confusion
`npx <name>` is a **different supply-chain primitive** from classic dependency confusion. If no explicit `--package` is given, npm first tries to resolve `<name>` as an executable from the local `node_modules/.bin`, global bins, installed package trees, and the `npx` cache. If none of those checks succeed, npm treats the same string as a **package name**, installs it into `~/.npm/_npx/<hash>/`, prepends `node_modules/.bin` from that cache entry to `PATH`, and executes it.[\[9\]](#references)[\[10\]](#references)[\[11\]](#references)

That means an attacker can win with **binary-name takeover** even when there is no public/private registry precedence issue:

1. Discover scripts, READMEs, CI jobs, bundles, or leaked `package.json` files containing`npx <binary_name>` .
2. Confirm the real package exposing that binary has a different package name (very common with scoped packages such as `@company/tool` ).
3. Check whether the public package `<binary_name>` is unclaimed.
4. Publish `<binary_name>` with a matching`bin` entry.
5. Wait for a developer, CI runner, automation job, or agent to execute `npx <binary_name>`**outside the correct dependency context** (wrong cwd, fresh workspace, dependencies not installed, minimal container, etc.).
...
- Enumerate `bin` entries from internal packages and compare the executable names against the actual package names.
- Inspect `~/.npm/_npx/` on build agents or developer workstations for unexpected cached packages and`_npx` metadata.
- Review CI logs for messages like `The following package was not found and will be installed` .

Practical hardening:

- Prefer `npx --package <expected-package> <binary>` (or`npm exec --package=<expected-package> -- <binary>` ) so the binary is bound to the intended package.
- Reserve public names matching internal binaries, especially names exported by scoped/private packages.
- In sensitive runners, use `--no` ,`offline` , or preinstall the expected package instead of allowing implicit remote fetches.
- Run automation from the intended project root and fail closed when `node_modules/.bin/<binary>` is absent.

## AWS Fix

This vulnerability was found in AWS CodeArtifact (read the details in this blog post). AWS added controls to mark dependencies/feeds as internal vs external so the client won’t fetch “internal” names from upstream public registries.[\[2\]](#references)

## Finding Vulnerable Libraries

In the original post about dependency confusion the author looked for thousands of exposed manifests (e.g., `package.json`, `requirements.txt`, lockfiles) to infer internal package names and then published higher-versioned packages to public registries.[\[1\]](#references)
...
- Use lockfiles that record the resolved registry URLs (npm/yarn/pnpm) or use hash/attestation pinning (pip
...
Use Package Source Mapping to tie package ID patterns to explicit sources and prevent resolution from unexpected feeds.[\[3\]](#references)
...
- Package provenance / attestations: when publishing public packages, enable provenance/attestations to make tampering more detectable downstream.<sup>[\[7\]](#references)</sup>
