...
source_url: https://hacktricks.wiki/pentesting-web/csrf-cross-site-request-forgery.html
...
When the action’s response is not visible to the attacker, test for **blind CSRF** by observing side effects through a separate authenticated session, audit trail, or callback.[\[12\]](#references)
...
- [**Cross-origin resource sharing**](cors-bypass.html) : The CORS policy of the victim site can influence the feasibility of the attack, especially if the attack requires reading the response from the victim site.[Learn about CORS bypass](cors-bypass.html) .
...
Some applications only enforce CSRF validation on POST while skipping it for other verbs. A common anti-pattern in PHP looks like:[\[5\]](#references)
...
Example (from [here](https://brycec.me/posts/corctf_2021_challenges)) of sending JSON data as text/plain:[\[14\]](#references)
...
```
This ensures the `Referer` header is omitted, potentially bypassing validation checks in some applications.[\[3\]](#references)[\[4\]](#references)

**Regexp bypasses**

To place the trusted domain string inside a query parameter that will appear in the `Referer`, use a URL such as the following:[\[3\]](#references)[\[4\]](#references)
...
Classic CSRF normally abuses ambient credentials, but an **unauthenticated** loopback or internal API is still exploitable: the victim browser contributes its **network position** instead of a cookie. The Same-Origin Policy may prevent the attacker from reading the response, but it does not stop a CORS-safelisted request from reaching the service and triggering a side effect.[\[16\]](#references)[\[17\]](#references)[\[18\]](#references)

During source review, correlate the following patterns across routes and server initialization rather than checking handlers in isolation:[\[17\]](#references)[\[18\]](#references)
...
- `Origin` validation that reflects arbitrary origins, accepts`null` , or uses substring/regex matching instead of parsing the origin and comparing it against an exact allowlist.
...
- Inconsistent treatment of loopback names and addresses. Test the exact destinations the client may use, such as `localhost` ,`127.0.0.1` , other`127.0.0.0/8` addresses, and`[::1]` ; application checks and browser local-network protections may not classify every spelling identically.

Use an auto-submitting form for a **blind**, preflight-free probe and choose a harmless action whose side effect can be confirmed in service or audit logs:[\[16\]](#references)[\[17\]](#references)
...
```
The same primitive can be tested with `fetch()` while deliberately keeping the request CORS-safelisted. The response will be opaque, so confirm the side effect separately:

```
fetch("http://127.0.0.1:PORT/SENSITIVE_ROUTE", {
  method: "POST",
  mode: "no-cors",
  headers: { "Content-Type": "application/x-www-form-urlencoded" },
  body: new URLSearchParams({ PARAMETER: "HARMLESS_ACTION" })
})
```
Do not change that probe to `Content-Type: application/json`: JSON is not a CORS-safelisted request content type and normally causes a preflight, which can prevent the state-changing request from being sent.

Do not assume that preflight-free means universally deliverable. Supporting browsers can gate public-to-local and public-to-loopback requests behind **Local Network Access** permissions (including form submissions and subframe navigation), and enterprise policy or an earlier permission decision can change the result. Test the exact victim browser and policy state.[\[19\]](#references)

Validate the primitive with a real or headless browser: record requests to the target origin, assert that the `POST` arrives, count any `OPTIONS` requests, and verify the harmless server-side effect independently. A successful action does not require response access, and direct network isolation is insufficient when an operator’s browser can route to the service.[\[17\]](#references)[\[18\]](#references)

For another real-world case study, check [this page about browser-to-localhost abuse](../windows-hardening/windows-local-privilege-escalation/abusing-auto-updaters-and-ipc.html).
...
Use purpose-built tooling and hands-on labs to validate PoCs without losing browser-specific behavior.[\[13\]](#references)
