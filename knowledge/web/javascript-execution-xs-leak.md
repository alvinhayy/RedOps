...
source_url: https://hacktricks.wiki/pentesting-web/xs-search/javascript-execution-xs-leak.html
...
```
The attacker logic is:[\[2\]](#references)
...
In many real targets, an iframe is not required. A direct script inclusion is enough:[\[1\]](#references)
...
`nosniff` decide whether the payload executes
...
If the endpoint is unstable across browsers, mix the execution oracle with the generic script load events already covered in the section index:[\[1\]](#references)
