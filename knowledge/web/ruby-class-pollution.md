...
source_url: https://hacktricks.wiki/pentesting-web/deserialization/ruby-class-pollution.html
...
3. This is only possible because there is a
4. **Impact Limitation** : This vulnerability only affects individual instances, leaving other instances of`User` and`Admin` unaffected, thus limiting the scope of exploitation.
...
`deep_merge`
`Hash#deep_merge` is not vulnerable by itself because it only merges hashes. It becomes dangerous when application code subsequently turns every merged key into an accessor or writes it into an object, as in the following pattern.[\[1\]](#references)
...
`deep_merge`
Hashie’s `deep_merge` method operates directly on object attributes rather than plain hashes. It **prevents replacement of methods** with attributes during a merge, with some **exceptions**: attributes ending in `_`, `!`, or `?` can still be merged into the object.[\[1\]](#references)

A special case is the standalone **`_`** attribute, which normally returns a `Mash` object. Because it is one of the exceptions, an attacker can modify it.[\[1\]](#references)
...
```
For the sample application, enumerate offsets and observe the signing endpoint (or another side channel) to identify success.[\[2\]](#references)
