...
source_url: https://hacktricks.wiki/pentesting-web/xss-cross-site-scripting/server-side-xss-dynamic-pdf.html
...
The best conformable way to exploit this vulnerability is to abuse the vulnerability to make the bot load a script you control locally. Then, you will be able to change the payload locally and make the bot load it with the same code every time.[\[2\]](#references)[\[1\]](#references)
...
If SSRF is allowed, but you **cannot reach** an interesting domain or IP, [check this page for potential bypasses](../ssrf-server-side-request-forgery/url-format-bypass.html).[\[3\]](#references)[\[4\]](#references)
...
This vulnerability can be transformed very easily in a SSRF (as you can make the script load external resources). So just try to exploit it (read some metadata?).[\[5\]](#references)
