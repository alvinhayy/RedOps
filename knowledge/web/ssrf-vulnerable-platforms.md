...
source_url: https://hacktricks.wiki/pentesting-web/ssrf-server-side-request-forgery/ssrf-vulnerable-platforms.html
...
- HTML-to-PDF APIs such as [**Gotenberg**](https://gotenberg.dev/docs/convert-with-chromium/convert-html-to-pdf) or wrappers around**Chromium** /**wkhtmltopdf**
...
For a bigger PDF-specific discussion, also check the HTML-to-PDF notes already present in the main [SSRF page](README.html#html-to-pdf-renderers-as-blind-ssrf-gadgets).
...
A recent high-value example is **Next.js**:[\[5\]](#references)
...
**who**really fetches the URL

A URL parameter that eventually influences OCR or image generation is **not automatically a useful SSRF against the target environment**. First map the fetch origin:[\[7\]](#references)
...
If the server fetches the URL but does **not** return the body, inspect every field that controls how the fetched bytes are handled afterwards:[\[7\]](#references)
...
High-signal candidates taken from the Assetnote blind SSRF chains research:[\[8\]](#references)
...
This page is intentionally keeping the list short. For a much larger chain catalog, check the original Blind SSRF Chains research linked below.[\[8\]](#references)
...
6. If you get any sign of internal reachability, pivot into:
  - [cloud-ssrf.md](cloud-ssrf.html) for metadata endpoints
  - [url-format-bypass.md](url-format-bypass.html) for allowlist bypasses
  - protocol-specific exploitation from the main [SSRF page](README.html)
