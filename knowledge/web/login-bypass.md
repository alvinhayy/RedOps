...
source_url: https://hacktricks.wiki/pentesting-web/login-bypass/index.html
...
See [Proxmox VE](../../network-services-pentesting/pentesting-web/proxmox-ve.html#root-cause-chain) for a product-specific example of authentication state confusion.

If you find a login page, test the following authentication and authorization failure modes.[\[3\]](#references)
...
- Check for a Node.js object-parsing issue with payloads such as `password[password]=1` .<sup>[\[1\]](#references)</sup>  - In a vulnerable `mysqljs/mysql` construction, the object can produce a condition similar to:`SELECT id, username, LEFT(password, 8) AS snipped_password, email FROM accounts WHERE username = 'admin' AND password = password = 1;` . The chained comparison may evaluate as true.
...
- In a vulnerable
- Check credentials:
  - [**Default credentials**](../../generic-hacking/brute-force.html#default-credentials) of the technology/platform used
  ...
  - **Brute-force** using a bigger**dictionary (**[**Brute force**](../../generic-hacking/brute-force.html#http-post-form)**)**
...
[Here you can find several tricks to bypass the login via **No SQL Injections**](../nosql-injection.html#basic-authentication-bypass)**.**

As the NoSQL Injections requires to change the parameters value, you will need to test them manually.

### XPath Injection authentication bypass

[Here you can find several tricks to bypass the login via **XPath Injection.**](../xpath-injection.html#authentication-bypass)
...
[Here you can find several tricks to bypass the login via **LDAP Injection.**](../ldap-injection.html#login-bypass)
...
Pages usually redirect users after login. Check whether the destination can be altered to cause an [**Open Redirect**](../open-redirect.html), especially when authorization codes or other secrets could be sent through the redirect flow.
