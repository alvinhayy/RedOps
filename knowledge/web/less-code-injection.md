...
source_url: https://hacktricks.wiki/pentesting-web/xs-search/css-injection/less-code-injection.html
...
```
The contents of `/etc/passwd` will appear between the markers. Try absolute paths, traversal from the compiler’s working directory, and `file://` URLs separately: supported path syntax differs among Less.js, less.php, and application-specific file managers.[\[4\]](#references)[\[6\]](#references)

#### Traversal and parser-error disclosure

On Node.js Less, `(inline)` accepts normal absolute or relative filesystem paths, so `file://` is not required. If `inline` is filtered, a plain import still makes the compiler parse the target as Less. When parsing fails, a verbose application error can disclose the offending line and adjacent lines through fields such as `extract`.[\[5\]](#references)[\[6\]](#references)
...
curl -sk "${TARGET}rest/v10/css/preview?baseUrl=1¶m=${INJ}" | \
...
```
A safe probe during an authorized test is to make the plugin perform an out-of-band DNS/HTTP callback rather than run a destructive command. Test local and HTTP(S) plugin locations separately because the configured file managers decide which schemes are supported.[\[5\]](#references)[\[6\]](#references)

#### Legacy inline JavaScript

Backtick JavaScript is another RCE primitive when the application uses Less before 3.0.0 or explicitly passes `{ javascriptEnabled: true }` (equivalent to `lessc --js`). It is disabled by default in modern Less.js, but wrappers may turn it back on. On recent Node.js versions, a direct execution probe is:[\[6\]](#references)
