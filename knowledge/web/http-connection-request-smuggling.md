...
source_url: https://hacktricks.wiki/pentesting-web/http-connection-request-smuggling.html
...
```
This turns connection reuse into an SSRF-like primitive against **internal virtual hosts**, admin panels, debug routes, and alternate tenants sharing the same edge.[\[1\]](#references)
...
The most practical modern variant is **browser-powered** exploitation. A victim first opens a legitimate connection to an attacker-controlled or attacker-triggered origin, and the browser later **reuses or coalesces** that connection for a different authority.[\[1\]](#references)
...
- **Client-side desync** and browser-assisted queue poisoning techniques from[Browser HTTP Request Smuggling](http-request-smuggling/browser-http-request-smuggling.html) .
- **HTTP/2 downgrade gadgets** from[Request Smuggling in HTTP/2 Downgrades](http-request-smuggling/request-smuggling-in-http-2-downgrades.html) when you need a second primitive to poison a shared back-end connection.

## Related State-Transition Abuse: `h2c` Upgrade Tunnelling

`h2c` Upgrade Tunnelling
