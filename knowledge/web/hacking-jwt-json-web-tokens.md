...
source_url: https://hacktricks.wiki/pentesting-web/hacking-jwt-json-web-tokens.html
...
```
1. Drop the signed token into the session cookie (e.g., `n8n-auth` ) to impersonate the user/admin account even if the password hash is salted.
...
1. Recover the PAN-OS build from the embedded JWT inside `<saml-request>` :
...
EOF
```
1. Compare the extracted version against the vendor hotfix matrix instead of relying on banners or TLS metadata. Examples from the PAN-OS advisory:
...
### [Kid Issues Overview](#kid-issues-overview)
...
#### [Revealing Key through “kid”](#revealing-key-through-kid)
...
#### [Path Traversal with “kid”](#path-traversal-with-kid)
...
#### [OS Injection through “kid”](#os-injection-through-kid)
...
```
### [Embedded Public Key (CVE-2018-0114)](#embedded-public-key-cve-2018-0114)
...
### [ES256: Revealing the private key with same nonce](#es256-revealing-the-private-key-with-same-nonce)
