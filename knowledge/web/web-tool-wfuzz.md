...
source_url: https://hacktricks.wiki/pentesting-web/web-tool-wfuzz.html
...
If `pip install wfuzz` breaks in recent Python environments, upgrade to the latest upstream release first (`pip install -U wfuzz`).[\[1\]](#references)
...
# Examples: urlencode, md5, base64, hexlify, uri_hex, double urlencode
```
In order to use an encoder, you have to indicate it in the **`-w`** or **`-z`** option.[\[2\]](#references)
...
```
#### [**JSON API login / OTP bruteforce**](#json-api-login--otp-bruteforce)
...
```
### [Bruteforce Directory/RESTful bruteforce](#bruteforce-directoryrestful-bruteforce)

```
wfuzz -c -w /tmp/tmp/params.txt --hc 404 https://domain.com/api/FUZZ
```
### [Path Parameters BF](#path-parameters-bf)

```
wfuzz -c -w ~/git/Arjun/db/params.txt --hw 11 'http://example.com/path%3BFUZZ=FUZZ'
```
### [Fuzz all parameters automatically](#fuzz-all-parameters-automatically)
...
```
#### [**NTLM, 2 lists, filter string (show), proxy**](#ntlm-2-lists-filter-string-show-proxy)
...
```
### [Cookie/Header bruteforce (vhost brute)](#cookieheader-bruteforce-vhost-brute)

#### [**Cookie, filter code (show), proxy**](#cookie-filter-code-show-proxy)
...
```
#### [**User-Agent, filter code (hide), proxy**](#user-agent-filter-code-hide-proxy)
...
```
#### [**Host + explicit IP (reverse proxy / misrouting checks)**](#host--explicit-ip-reverse-proxy--misrouting-checks)

```
wfuzz -c -w vhosts.txt --ip 10.10.10.10 \
  --hc BBB -H 'Host: FUZZ.example.com' http://example.com/
```
For more methodology around wildcard vhosts and related recon edge-cases, check [this page](../generic-methodologies-and-resources/external-recon-methodology/README.html).

### [HTTP Verbs (methods) bruteforce](#http-verbs-methods-bruteforce)
...
```
#### [**Using inline list**](#using-inline-list)
...
```
### [Recursive discovery](#recursive-discovery)
...
```
### [Save / restore long fuzzing sessions](#save--restore-long-fuzzing-sessions)
...
```
## [Tool to bypass Webs](#tool-to-bypass-webs)
