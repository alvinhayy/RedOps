...
source_url: https://hacktricks.wiki/windows-hardening/active-directory-methodology/ad-certificates/domain-escalation.html
...
It is noted that creating a certificate with this option triggers a warning, which is not the case when an existing certificate template (such as the `WebServer` template, which has `CT_FLAG_ENROLLEE_SUPPLIES_SUBJECT` enabled) is duplicated and then modified to include an authentication OID.[\[6\]](#references)
...
```
Then you can transform the generated **certificate to `.pfx`** format and use it to **authenticate using Rubeus or certipy** again:[\[5\]](#references)
...
You can use [**Certify**](https://github.com/GhostPack/Certify) or [**Certipy**](https://github.com/ly4k/Certipy) to abuse this scenario:[\[4\]](#references)
...
```
The scripts print the Base64 of the **PFX**, which includes the private key, for direct use with Rubeus. Do not replace it with `[Convert]::ToBase64String($cert.RawData)`: `RawData` encodes only the public certificate and cannot sign the PKINIT request.[\[5\]](#references)[\[27\]](#references)
...
Notable permissions applicable to certificate templates include:[\[6\]](#references)
...
```
Tools like [**Certify**](https://github.com/GhostPack/Certify) and [**Certipy**](https://github.com/ly4k/Certipy) are capable of detecting this misconfiguration and exploiting it:[\[4\]](#references)
...
```
This provides insights into the primary rights, namely **`ManageCA`** and **`ManageCertificates`**, correlating to the roles of “CA administrator” and “Certificate Manager” respectively.[\[6\]](#references)
...
In addition to the classic ESC7 abuses (enabling EDITF attributes or approving pending requests), **Certify 2.0** revealed a brand-new primitive that only requires the *Manage Certificates* (a.k.a. **Certificate Manager / Officer**) role on the Enterprise CA.[\[3\]](#references)
...
1.
**Submit a certificate request that will remain *pending*.** This can be forced with a template that requires manager approval:```
Certify.exe request --ca SERVER\\CA-NAME --template SecureUser --subject "CN=User" --manager-approval
# Take note of the returned Request ID
```
2.
**Append a custom extension to the pending request** using the new`manage-ca` command:```
...
3.
**Issue the request** (if your role also has*Manage Certificates* approval rights) or wait for an operator to approve it.  Once issued, download the certificate:```
Certify.exe request-download --ca SERVER\\CA-NAME --id 1337
```
4.
The resulting certificate now contains the malicious issuance-policy OID and can be used in subsequent attacks (e.g. ESC13, domain escalation, etc.).
...
[**Certify**](https://github.com/GhostPack/Certify)’s `cas` enumerates **enabled HTTP AD CS endpoints**:[\[4\]](#references)
...
```
#### [Abuse with Certify](#abuse-with-certify)
...
```
#### [Abuse with](#abuse-with-certipy) [Certipy](https://github.com/ly4k/Certipy)
...
[*] Exiting...
```
## [No Security Extension - ESC9](#no-security-extension---esc9)
...
## [Relaying NTLM to ICPR - ESC11](#relaying-ntlm-to-icpr---esc11)

### [Explanation](#explanation-12)

If CA Server Do not configured with `IF_ENFORCEENCRYPTICERTREQUEST`, it can be makes NTLM relay attacks without signing via RPC service. [Reference in here](https://blog.compass-security.com/2022/11/relaying-to-ad-certificate-services-over-rpc/).[\[10\]](#references)
...
```
### [Abuse Scenario](#abuse-scenario-1)
...
```
## [Shell access to ADCS CA with YubiHSM - ESC12](#shell-access-to-adcs-ca-with-yubihsm---esc12)
...
### [Abuse Scenario](#abuse-scenario-2)
...
------------------------
```
### [Abuse Scenario](#abuse-scenario-3)
...
```
## [Vulnerable Certificate Renewal Configuration- ESC14](#vulnerable-certificate-renewal-configuration--esc14)

### [Explanation](#explanation-15)

The description at https://github.com/ly4k/Certipy/wiki/06-%E2%80%90-Privilege-Escalation#esc14-weak-explicit-certificate-mapping is remarkably thorough. Below is a quotation of the original text.[\[14\]](#references)
...
### [Abuse Scenario](#abuse-scenario-4)
...
#### [Scenario A: Attacker Can Write to `altSecurityIdentities`](#scenario-a-attacker-can-write-to-altsecurityidentities)
...
#### [Scenario B: Target Has Weak Mapping via X509RFC822 (Email)](#scenario-b-target-has-weak-mapping-via-x509rfc822-email)
...
#### [Scenario C: Target Has X509IssuerSubject Mapping](#scenario-c-target-has-x509issuersubject-mapping)
...
#### [Scenario D: Target Has X509SubjectOnly Mapping](#scenario-d-target-has-x509subjectonly-mapping)
...
```
For more specific attack methods in various attack scenarios, please refer to the following: [adcs-esc14-abuse-technique](https://posts.specterops.io/adcs-esc14-abuse-technique-333a004dc2b9#aca0).[\[13\]](#references)

## [EKUwu Application Policies(CVE-2024-49019) - ESC15](#ekuwu-application-policiescve-2024-49019---esc15)

### [Explanation](#explanation-16)

The description at https://trustedsec.com/blog/ekuwu-not-just-another-ad-cs-esc is remarkably thorough. Below is a quotation of the original text.[\[15\]](#references)
...
The [Certipy privilege-escalation documentation](https://github.com/ly4k/Certipy/wiki/06-%E2%80%90-Privilege-Escalation#esc15-arbitrary-application-policy-injection-in-v1-templates-cve-2024-49019-ekuwu) contains more detailed usage examples.[\[14\]](#references)
...
```
#### [Scenario A: Direct Impersonation via Schannel](#scenario-a-direct-impersonation-via-schannel)
...
```
#### [Scenario B: PKINIT/Kerberos Impersonation via Enrollment Agent Abuse](#scenario-b-pkinitkerberos-impersonation-via-enrollment-agent-abuse)
...
```
## [Security Extension Disabled on CA (Globally)-ESC16](#security-extension-disabled-on-ca-globally-esc16)
...
1.
Requesting a certificate **without SID binding** .
2.
Using this certificate **for authentication as any account** , such as impersonating a high-privilege account (e.g., a Domain Administrator).

You can also refer to this article to learn more about the detailed principle:https://medium.com/@muneebnawaz3849/ad-cs-esc16-misconfiguration-and-exploitation-9264e022a8c6[\[16\]](#references)

### [Abuse](#abuse-8)

The following is referenced to [this link](https://github.com/ly4k/Certipy/wiki/06-%E2%80%90-Privilege-Escalation#esc16-security-extension-disabled-on-ca-globally),Click to see more detailed usage methods.[\[14\]](#references)
...
```
## [Rogue LDAP/LSA chase callback identity substitution (Certighost / CVE-2026-54121)](#rogue-ldaplsa-chase-callback-identity-substitution-certighost--cve-2026-54121)

### [Explanation](#explanation-18)

**Certighost** abuses an **AD CS enrollment chase / callback path** where the CA trusts requester-supplied request attributes to resolve the identity that should be placed in the issued certificate. In the public PoC, the crafted request includes:[\[1\]](#references)[\[2\]](#references)
...
The public PoC automates the full chain:[\[1\]](#references)
...
## [IIS AppPool machine enrollment to same-host Administrator](#iis-apppool-machine-enrollment-to-same-host-administrator)

An IIS pool running as `ApplicationPoolIdentity` uses its host’s **computer account** for outbound access to network resources. Therefore, code execution as `IIS AppPool\<POOL>` stays low-privileged in the local token but can submit an AD CS request that the CA authenticates as `HOST$`; this is an outbound identity transition, not token impersonation or a Potato-style local elevation.[\[19\]](#references)[\[20\]](#references)

This chain requires a domain-joined IIS host, an Enterprise CA reachable through RPC, a published machine-authentication template for which the computer has enrollment rights, PKINIT support, and KDC/SMB reachability. A custom pool identity changes the outbound principal, so confirm the pool really uses `ApplicationPoolIdentity` before assuming `HOST$`.[\[19\]](#references)[\[20\]](#references)

### [Attacker-controlled-key enrollment](#attacker-controlled-key-enrollment)

Generate the key pair and CSR away from the IIS server and retain the private key. Submit **only the CSR** from the compromised worker. The [Certi-Bhai ASPX PoC](https://github.com/incredibleindishell/Certi-Bhai/blob/main/IIS_Privilege_escalation/cert.aspx) instantiates `CertificateAuthority.Request`, sets `CertificateTemplate:Machine`, calls `ICertRequest::Submit`, and returns the issued certificate. Use the CA configuration string `CAHOST\CA-NAME`; a normal `Machine` template builds the subject from AD, so requester-supplied subject/SAN data is not required.[\[18\]](#references)[\[19\]](#references)[\[21\]](#references)

Combine the returned certificate with the **matching retained key**. `certutil -MergePFX machine_cert.cer machine_cert.pfx` only works when Windows can already associate the certificate with an accessible private key; for separate PEM files, create PKCS#12 explicitly:[\[19\]](#references)[\[23\]](#references)

```
openssl pkcs12 -export -in machine_cert.cer -inkey machine_cert.key \
  -out machine_cert.pfx -name 'HOST$'
```
Use the PFX for PKINIT and keep the returned computer TGT as base64 rather than injecting it immediately:[\[5\]](#references)[\[19\]](#references)

```
Rubeus.exe asktgt /user:HOST$ /domain:DOMAIN /certificate:machine_cert.pfx `
  /password:PFX_PASSWORD /nowrap
```
### [S4U2Self plus same-host service substitution](#s4u2self-plus-same-host-service-substitution)

S4U2Self lets a service obtain a ticket **to itself** containing another user’s authorization data. With the computer TGT, Rubeus can request that ticket for a privileged user, rewrite the service name in the returned KRB-CRED to CIFS, and inject it. This is the local “delegate to thyself” primitive: it does not require S4U2Proxy or an `msDS-AllowedToDelegateTo` entry.[\[5\]](#references)[\[17\]](#references)[\[22\]](#references)
...
## [Compromising Forests with Certificates Explained in Passive Voice](#compromising-forests-with-certificates-explained-in-passive-voice)

### [Breaking of Forest Trusts by Compromised CAs](#breaking-of-forest-trusts-by-compromised-cas)

The configuration for **cross-forest enrollment** is made relatively straightforward. The **root CA certificate** from the resource forest is **published to the account forests** by administrators, and the **enterprise CA** certificates from the resource forest are **added to the `NTAuthCertificates` and AIA containers in each account forest**. To clarify, this arrangement grants the **CA in the resource forest complete control** over all other forests for which it manages PKI. Should this CA be **compromised by attackers**, certificates for all users in both the resource and account forests could be **forged by them**, thereby breaking the security boundary of the forest.[\[6\]](#references)

### [Enrollment Privileges Granted to Foreign Principals](#enrollment-privileges-granted-to-foreign-principals)
...
Both scenarios lead to an **increase in the attack surface** from one forest to another. The settings of the certificate template could be exploited by an attacker to obtain additional privileges in a foreign domain.[\[6\]](#references)
