...
source_url: https://hacktricks.wiki/windows-hardening/active-directory-methodology/ad-certificates/index.html
...
AD CS acknowledges CA certificates in an AD forest through designated containers, each serving unique roles:[\[1\]](#references)
...
Defined within AD, these templates outline the settings and permissions for issuing certificates, including permitted EKUs and enrollment or modification rights, critical for managing access to certificate services.[\[1\]](#references)

## Certificate Enrollment

The enrollment process for certificates is initiated by an administrator who **creates a certificate template**, which is then **published** by an Enterprise Certificate Authority (CA). This makes the template available for client enrollment, a step achieved by adding the template’s name to the `certificatetemplates` field of an Active Directory object.[\[1\]](#references)

For a client to request a certificate, **enrollment rights** must be granted. These rights are defined by security descriptors on the certificate template and the Enterprise CA itself. Permissions must be granted in both locations for a request to be successful.[\[1\]](#references)

### Template Enrollment Rights

These rights are specified through Access Control Entries (ACEs), detailing permissions like:[\[1\]](#references)
...
The CA’s rights are outlined in its security descriptor, accessible via the Certificate Authority management console. Some settings even allow low-privileged users remote access, which could be a security concern.[\[1\]](#references)

### Additional Issuance Controls

Certain controls may apply, such as:[\[1\]](#references)
...
Certificates can be requested through:[\[1\]](#references)
...
Active Directory (AD) supports certificate authentication, primarily utilizing **Kerberos** and **Secure Channel (Schannel)** protocols.[\[1\]](#references)
...
```
Rubeus can also use a password-protected PFX certificate for PKINIT authentication and request a TGT. The optional `/getcredentials` switch requests a U2U service ticket and attempts to recover the account NT hash:[\[4\]](#references)
