...
source_url: https://hacktricks.wiki/windows-hardening/active-directory-methodology/laps.html
...
In the domain’s computer objects, the implementation of legacy Microsoft LAPS results in the addition of two attributes:[\[1\]](#references)
...
Native Windows LAPS adds several new attributes to computer objects:[\[2\]](#references)
...
When **`msLAPS-Password`** is readable, the value is a JSON object containing the account name, update time and clear-text password, for example:[\[2\]](#references)
...
```
A few operational details matter here:[\[3\]](#references)
...
$laps.p
```
That **`n`** field matters on newer deployments because **Windows LAPS automatic account management** can target a **custom account** instead of the built-in **`Administrator`**, and newer **Windows 11 24H2 / Windows Server 2025** systems can even **randomize** that account name.[\[4\]](#references)
...
In practice, this means snapshot-based persistence or attempts to resurrect an older known local admin password can burn quickly instead of surviving until the next normal expiration.[\[2\]](#references)
...
When **automatic account management** is enabled, Windows LAPS owns the lifecycle of the managed local admin account. Unexpected attempts to rename, reconfigure, or otherwise tamper with that account can be rejected with **`STATUS_POLICY_CONTROLLED_ACCOUNT`** / **`ERROR_POLICY_CONTROLLED_ACCOUNT`**, so persistence that depends on silently modifying the managed LAPS account is less reliable on newer endpoints.[\[4\]](#references)

### Recovering historical passwords from AD backups

When **Windows LAPS encryption + password history** is enabled, mounted AD backups can become an additional source of secrets. If you can access a mounted AD snapshot and use **recovery mode**, you can query older stored passwords without talking to a live DC.[\[3\]](#references)
