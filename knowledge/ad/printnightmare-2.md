...
source_url: https://hacktricks.wiki/windows-hardening/active-directory-methodology/printnightmare.html
...
On Windows 11 22H2+ and newer client builds, remote printing defaults to **RPC over TCP** and **RPC over named pipes** (`\PIPE\spoolss`) is disabled unless explicitly re-enabled. Some older PoCs and lab notes still assume the named pipe is reachable.[\[4\]](#references)
...
Microsoft’s 2021 patches blocked remote driver loading but **did not harden directory permissions**. SpoolFool abuses the `SpoolDirectory` parameter to create an arbitrary directory under `C:\Windows\System32\spool\drivers\`, drops a payload DLL, and forces the spooler to load it:[\[2\]](#references)
...
```
The exploit works on fully-patched Windows 7 → Windows 11 and Server 2012R2 → 2022 before February 2022 updates[\[2\]](#references)
...
If you want to **coerce authentication** via the spooler instead of loading a driver, jump to [printer spooler service abuse](printers-spooler-service-abuse.html).
