...
source_url: https://hacktricks.wiki/windows-hardening/lateral-movement/wmiexec.html
...
```
For attackers, WMI is a potent tool for enumerating sensitive data about systems or domains.[\[1\]](#references)
...
Stealthy identification of local admins on a remote machine and logged-on users can be achieved through specific WMI queries. `wmic` also supports reading from a text file to execute commands on multiple nodes simultaneously.[\[1\]](#references)

To remotely execute a process over WMI, such as deploying an Empire agent, the following command structure is employed, with successful execution indicated by a return value of “0”:[\[1\]](#references)
