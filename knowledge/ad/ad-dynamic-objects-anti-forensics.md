...
source_url: https://hacktricks.wiki/windows-hardening/active-directory-methodology/ad-dynamic-objects-anti-forensics.html
...
## [Stealth Primary Group Membership](#stealth-primary-group-membership)
...
- Reporting is tool-dependent: **`Get-ADGroupMember` / `net group`** usually resolve primary-group-derived membership, while**`memberOf`** and**`Get-ADGroup -Properties member`** do not. For broader`primaryGroupID` tradecraft, see[this other page about DCShadow and PGID abuse](dcshadow.html) .
...
## [AdminSDHolder Orphan-SID Pollution](#adminsdholder-orphan-sid-pollution)
...
## [Dynamic GPO Execution with Self-Destructing Evidence](#dynamic-gpo-execution-with-self-destructing-evidence)
...
- This is operationally cleaner than classic **GPODDITY-style** cleanup: instead of restoring the original`gPCFileSysPath` yourself, AD removes the malicious GPC automatically once the timer expires.<sup>[\[1\]](#references)</sup> See[ACL persistence abuse](acl-persistence-abuse/README.html#gpcfilesyspath-poisoning-with-gpoddity) for the protocol and tooling details rather than duplicating them here.

## [Ephemeral AD-Integrated DNS Redirection](#ephemeral-ad-integrated-dns-redirection)
...
## [Hybrid Entra ID Delta-Sync Gap (Note)](#hybrid-entra-id-delta-sync-gap-note)
