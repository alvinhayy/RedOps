...
source_url: https://hacktricks.wiki/windows-hardening/active-directory-methodology/unconstrained-delegation.html
...
More info: [https://www.harmj0y.net/blog/activedirectory/s4u2pwnage/](https://www.harmj0y.net/blog/activedirectory/s4u2pwnage/)[\[2\]](#references)**More information about Unconstrained delegation in ired.team.**[\[2\]](#references)[\[3\]](#references)
...
```
If the TGT if from a domain controller, you could perform a [**DCSync attack**](acl-persistence-abuse/index.html#dcsync) and obtain all the hashes from the DC.**More info about this attack in ired.team.**[\[10\]](#references)
...
```
1. Make the fake hostname resolvable inside the domain
...
```
1. Enable Unconstrained Delegation on the attacker-controlled computer
...
1. Start krbrelayx in export mode and prepare the Kerberos material
...
```
1. Coerce authentication from the DC/servers to your fake host
...
```
1. Use the captured DC machine TGT to perform DCSync
