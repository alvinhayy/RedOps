...
source_url: https://hacktricks.wiki/windows-hardening/active-directory-methodology/sccm-management-point-relay-sql-policy-secrets.html
...
4. Through the SOCKS proxy connect with `mssqlclient.py -windows-auth` as the relayed\ $
5. Execute:
  - `use CM_<SiteCode>`
...
Secrets such as `OSDJoinAccount/OSDJoinPassword`, `NetworkAccessUsername/Password`, etc. are recovered without touching PXE or clients.[\[1\]](#references)[\[3\]](#references)

## 1. Enumerating unauthenticated MP endpoints

The MP ISAPI extension **GetAuth.dll** exposes several parameters that don’t require authentication (unless the site is PKI-only):[\[1\]](#references)
...
Connect through the SOCKS proxy (port 1080 by default):[\[1\]](#references)

```
proxychains mssqlclient.py CONTOSO/MP01$@10.10.10.15 -windows-auth
```
Switch to the **CM_** DB (use the 3-digit site code, e.g.
...
Upon relay the login is mapped to:[\[1\]](#references)
...
- If the response includes the decryption key, feed it to
