...
source_url: https://hacktricks.wiki/windows-hardening/ntlm/index.html
...
If you are using `responder` you could try to **use the flag `--lm`** to try to **downgrade** the **authentication**.*Note that for this technique the authentication must be performed using NTLMv1 (NTLMv2 is not valid).*
...
NTLMv1 can also be attacked with [NTLMv1 Multi Tool](https://github.com/evilmog/ntlmv1-multi), which converts captured NTLMv1 messages into formats suitable for Hashcat.[\[1\]](#references)
...
If you have a **PCAP containing a successful authentication exchange**, extract the domain, username, server challenge, and NTLMv2 response, format the capture for Hashcat, and use mode `5600` to attempt password recovery. The archived practical walkthrough retains the packet-field extraction procedure, while Hashcat’s examples define the current accepted format.[\[2\]](#references)[\[7\]](#references)
...
You can obtain code execution in Windows machines using Pass-the-Hash from Linux.**See practical Pass-the-Hash execution examples.**
...
You can get the powershell scripts from here: [https://github.com/Kevin-Robertson/Invoke-TheHash](https://github.com/Kevin-Robertson/Invoke-TheHash)[\[3\]](#references)
...
This function combines the preceding modes. You can pass **several hosts**, exclude selected targets, and choose *SMBExec, WMIExec, SMBClient,* or *SMBEnum*. If you select **SMBExec** or **WMIExec** without a ***Command*** parameter, it only checks whether you have sufficient permissions.

```
Invoke-TheHash -Type WMIExec -Target 192.168.100.0/24 -TargetExclude 192.168.100.50 -Username Administ -ty    h F6F38B793DB6A94BA04A52F1D3EE92F0
```
### [Evil-WinRM Pass the Hash](../../network-services-pentesting/5985-5986-pentesting-winrm.html#using-evil-winrm)
...
The PoC can be found in **[https://github.com/eladshamir/Internal-Monologue](https://github.com/eladshamir/Internal-Monologue)**.[\[4\]](#references)
...
*Reflection*via Serialized SPNs (CVE-2025-33073)
...
[Local NTLM Reflection via SMB Arbitrary Port](../windows-local-privilege-escalation/local-ntlm-reflection-via-smb-arbitrary-port.html)
