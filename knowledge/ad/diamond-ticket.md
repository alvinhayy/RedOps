...
source_url: https://hacktricks.wiki/windows-hardening/active-directory-methodology/diamond-ticket.html
...
**Like a golden ticket**, a diamond ticket is a TGT which can be used to **access any service as any user**. A golden ticket is forged completely offline, encrypted with the krbtgt hash of that domain, and then passed into a logon session for use. Because domain controllers don’t track TGTs it (or they) have legitimately issued, they will happily accept TGTs that are encrypted with its own krbtgt hash.[\[1\]](#references)
...
./Rubeus.exe diamond \
  ...
  /ldap /opsec /nowrap
```
This workflow is ideal when you already control a service account key (e.g., dumped with `lsadump::lsa /inject` or `secretsdump.py`) and want to cut a one-off TGS that perfectly matches AD policy, timelines, and PAC data without issuing any new AS/TGS traffic.[\[3\]](#references)

### Sapphire-style PAC swaps (2025)

A newer twist sometimes called a **sapphire ticket** combines Diamond’s “real TGT” base with **S4U2self+U2U** to steal a privileged PAC and drop it into your own TGT. Instead of inventing extra SIDs, you request a U2U S4U2self ticket for a high-privilege user where the `sname` targets the low-priv requester; the KRB_TGS_REQ carries the requester’s TGT in `additional-tickets` and sets `ENC-TKT-IN-SKEY`, allowing the service ticket to be decrypted with that user’s key. You then extract the privileged PAC and splice it into your legitimate TGT before re-signing with the krbtgt key.[\[2\]](#references)[\[5\]](#references)

Impacket’s `ticketer.py` now ships sapphire support via `-impersonate` + `-request` (live KDC exchange):[\[2\]](#references)[\[5\]](#references)
...
Key OPSEC tells when using this variant:[\[5\]](#references)
