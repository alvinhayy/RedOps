...
source_url: https://hacktricks.wiki/windows-hardening/active-directory-methodology/badsuccessor-dmsa-migration-abuse.html
...
Unit 42 released a PowerShell helper script that parses security descriptors of each OU and highlights the required ACEs:[\[1\]](#references)
...
Once a writable OU is identified the attack is only 3 LDAP writes away:[\[1\]](#references)
...
```
After replication the attacker can simply **logon** as `attacker_dMSA$` or request a Kerberos TGT – Windows will build the token of the *superseded* account.[\[1\]](#references)
...
Enable **Object Auditing** on OUs and monitor for the following Windows Security Events:[\[1\]](#references)[\[2\]](#references)
...
Correlating `4662` (attribute modification), `4741` (creation of a computer/service account) and `4624` (subsequent logon) quickly highlights BadSuccessor activity.  XDR solutions such as **XSIAM** ship with ready-to-use queries (see references).[\[2\]](#references)
