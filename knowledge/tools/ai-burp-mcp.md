...
source_url: https://hacktricks.wiki/AI/AI-Burp-MCP.html
...
Install **MCP Server** from the Burp BApp Store and verify it is listening on `127.0.0.1:9876`.[\[1\]](#references)[\[2\]](#references)
...
If the MCP handshake fails due to strict `Origin` checks or extra headers, use a local reverse proxy to normalize headers (this matches the workaround for the Burp MCP strict validation issue).[\[1\]](#references)[\[3\]](#references)
...
The **burp-mcp-agents** repo provides launcher helpers:[\[4\]](#references)
...
To tag Burp/LLM traffic in logs, add a header rewrite (proxy or Burp Match/Replace):[\[1\]](#references)
