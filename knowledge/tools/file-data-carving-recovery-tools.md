...
source_url: https://hacktricks.wiki/generic-methodologies-and-resources/basic-forensic-methodology/partitions-file-systems-carving/file-data-carving-recovery-tools.html
...
Always carve a **verified copy**, not the original device. See [Image Acquisition & Mount](../image-acquisition-and-mount.html) for read-only acquisition and hashing workflows.
...
```
The legacy v2 `--dd='.*'` recipe is **not** the v3 equivalent of `-c`; first check `binwalk --version` when following old CTF/write-up commands.[\[11\]](#references)

⚠️  **Security note** – Versions **2.1.2b through 2.3.3** are affected by a **Path Traversal** vulnerability (CVE-2022-4510); the advisory lists no patched pip version. Avoid extracting untrusted samples with affected releases, or isolate the tool with a container/non-privileged UID.[\[4\]](#references)
...
The v2.1.1 release documents an Autotools build and the `-S jpeg_carve_mode=2` setting for carving all contiguous JPEGs.[\[2\]](#references)
...
```
The bundled `bulk_diff.py` compares two bulk_extractor runs, while `bulk_extractor_reader.py` reads the report and feature files.[\[3\]](#references)
...
`tsk_recover` (metadata-first)
Before raw signature carving, try filesystem-aware recovery when the volume metadata is still parseable. `tsk_recover` exports only unallocated files by default; `-a` selects allocated files and `-e` exports both. For a whole-disk image, pass the partition’s **start sector** from `mmls` to `-o` (do not convert it to bytes). If the input is already a partition image, omit `-o`.[\[12\]](#references)
...
```
This pass can preserve filesystem-derived names and paths that header/footer carving cannot; run Foremost, Scalpel or PhotoRec afterwards for entries whose metadata is missing or unusable.[\[12\]](#references)
...
```
The **`--cluster-size`** option controls how many sectors are copied at a time; smaller values can help with slow drives.[\[7\]](#references)

### Extundelete / Ext4magic (EXT 3/4 undelete)

If the source file system is Linux EXT-based you may be able to recover recently deleted files **without full carving**; these journal-based tools work on an unmounted filesystem or a read-only image.[\[8\]](#references)[\[9\]](#references)
...
```
**Compatibility note** – ext4magic is abandoned; its project page warns that current filesystems are no longer compatible with it.[\[10\]](#references)
