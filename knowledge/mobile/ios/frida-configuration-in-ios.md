...
source_url: https://hacktricks.wiki/mobile-pentesting/ios-pentesting/frida-configuration-in-ios.html
...
**Steps to install Frida on a Jailbroken device:**[\[2\]](#references)
...
Check this blog post about how to use Frida in non-jailbroken devices without patching the app: [https://mrbypass.medium.com/unlocking-potential-exploring-frida-objection-on-non-jailbroken-devices-without-application-ed0367a84f07](https://mrbypass.medium.com/unlocking-potential-exploring-frida-objection-on-non-jailbroken-devices-without-application-ed0367a84f07)[\[4\]](#references)
...
If at some point you need a training on reversing iOS / Frida check [https://reversing.training/](https://reversing.training/)[\[1\]](#references)
...
Penetration tests against production-like builds often require keeping jailbreak protections enabled while still attaching Frida. A reliable workflow is to pair Apple’s `debugserver` with LLDB over USB multiplexing:[\[3\]](#references)

1.
Forward SSH so the jailbroken phone is reachable even without Wi-Fi: `iproxy 2222 22 &` followed by`ssh root@localhost -p 2222` .
2.
On the device, spawn the debugger stub and make it wait for the target process: `debugserver *:5678 --waitfor <BundleName>` and then launch the app from the SpringBoard.
3.
Forward the debugging port and attach LLDB from macOS: ```
iproxy 1234 5678 &
lldb
(lldb) process connect connect://localhost:1234
```
4.
Use `finish` a few times so constructors return and LLDB can resolve every Swift/ObjC image before you start patching symbols.
...
### [Patching Swift jailbreak / Frida checks](#patching-swift-jailbreak--frida-checks)
...
### [Discovering Swift targets for Frida](#discovering-swift-targets-for-frida)
...
### [Hooking Swift `String` arguments](#hooking-swift-string-arguments)
...
## [Frida Fuzzing](#frida-fuzzing)

### [Frida Stalker](#frida-stalker)

[From the docs](https://frida.re/docs/stalker/): Stalker is Frida’s code **tracing engine**. It allows threads to be **followed**, **capturing** every function, **every block**, even every instruction which is executed.[\[5\]](#references)
...
#### [Advanced Fuzzing with Crash Monitoring](#advanced-fuzzing-with-crash-monitoring)
...
```
#### [Simple Standalone Fuzzer (Without fpicker)](#simple-standalone-fuzzer-without-fpicker)
