...
source_url: https://hacktricks.wiki/mobile-pentesting/ios-pentesting/ios-protocol-handlers.html
...
This page focuses on **WebView / browser-driven scheme abuse**. For app registration, deeplink hijacking, and callback stealing, see [iOS Custom URI Handlers / Deeplinks / Custom Schemes](ios-custom-uri-handlers-deeplinks-custom-schemes.html). For the file-origin / `loadFileURL:allowingReadAccessTo:` angle, see [iOS WebViews](ios-webviews.html). For claimed `https` handlers, see [iOS Universal Links](ios-universal-links.html).
...
`canOpenURL` used as if it were validation
...
`WKNavigationDelegate` or JS bridges open arbitrary URLs
Look for:
...
`WKURLSchemeHandler` turns native code into a private web server
