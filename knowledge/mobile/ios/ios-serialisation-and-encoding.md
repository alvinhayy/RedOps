...
source_url: https://hacktricks.wiki/mobile-pentesting/ios-pentesting/ios-serialisation-and-encoding.html
...
`NSKeyedArchiver`
`NSKeyedArchiver` and `NSKeyedUnarchiver` encode and decode object graphs. The following legacy APIs illustrate older code that may still appear during review, but Apple deprecated them; new code should use secure archiving/unarchiving APIs that specify the expected root class.[\[3\]](#references)
...
`Codable` for Simplified Serialization
...
```
This approach supports straightforward serialization to and from property lists and JSON.[\[4\]](#references)
