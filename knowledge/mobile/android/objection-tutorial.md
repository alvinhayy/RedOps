...
source_url: https://hacktricks.wiki/mobile-pentesting/android-app-pentesting/frida-tutorial/objection-tutorial.html
...
[**Objection**](https://github.com/sensepost/objection) is a runtime mobile exploration toolkit, powered by [Frida](https://www.frida.re). It was built with the aim of helping assess mobile applications and their security posture without the need for a jailbroken or rooted mobile device.[\[1\]](#references)
...
```
If you are using a **non-rooted device**, a common workflow is to patch the APK to embed **Frida Gadget**, reinstall it, and then connect to `Gadget`:[\[3\]](#references)
...
If the application still crashes or exits as soon as Objection/Frida attaches, continue with [this anti-instrumentation workflow](../android-anti-instrumentation-and-ssl-pinning-bypass.html).
...
If the app runs root checks, SSL pinning or anti-tampering **during startup**, run the hook before the REPL finishes collecting environment data:[\[2\]](#references)
