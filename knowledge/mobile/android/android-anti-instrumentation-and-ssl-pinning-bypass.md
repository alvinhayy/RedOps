...
source_url: https://hacktricks.wiki/mobile-pentesting/android-app-pentesting/android-anti-instrumentation-and-ssl-pinning-bypass.html
...
Medusa provides 90+ ready-made modules for SSL unpinning, root/emulator detection bypass, HTTP comms logging, crypto key interception, and more.[\[10\]](#references)
...
Auto-Frida is a Frida automation toolkit that focuses on repeatable setup plus **auto-detection** of protections and **consolidated bypass script generation**. It is useful when apps run checks very early or when multiple bypass modules would otherwise double-hook the same APIs.[\[11\]](#references)
...
Override custom guards to return safe values without repacking:[\[1\]](#references)
...
When LSPosed is already available on the test device, [SSL Kill Switch](https://github.com/0xdad0/ssl-kill-switch-lsposed) can apply persistent hooks without repacking the APK or attaching Frida. Install and activate the module, restrict its **LSPosed scope** to the authorized packages, restart those processes, install/trust the interception CA as required, and route traffic through the proxy. Only its optional transparent-routing feature invokes `su` for `iptables`.[\[20\]](#references)

The Java bypass is applied twice: from `initZygote()` for boot-classpath implementations and again from `handleLoadPackage()` with the target application’s class loader. This lets one module cover the following layers while preserving each hooked method’s return contract.[\[20\]](#references)
...
Current implementation caveats are important during testing:[\[20\]](#references)
...
Trace JNI entry points to locate native loaders and detection init:[\[1\]](#references)
...
```
See also:
[Reversing Native Libraries](reversing-native-libraries.html)
...
[Frida Tutorial — Self-contained agent + Gadget embedding](frida-tutorial/README.html)
...
If instrumentation is blocked, you can still inspect traffic by removing pinning statically:[\[1\]](#references)
...
-
For network config CA‑trust tricks (and Android 7+ user CA trust), see: {{#ref}} make-apk-accept-ca-certificate.md {{#endref}} {{#ref}} install-burp-certificate.md {{#endref}}

## LSPosed/Xposed Hooking Abuse (Telephony/SMS)

On rooted devices, LSPosed/Xposed modules can hook Java telephony/SMS APIs at runtime, keeping the APK unmodified on disk while fully controlling what the app sees. This is commonly abused to bypass SIM‑binding flows that trust local telephony APIs or local SMS provider state.[\[16\]](#references)
...
- [19] [NVISOsecurity disable-flutter-tls-verification](https://github.com/NVISOsecurity/disable-flutter-tls-verification)
- [20] [SSL Kill Switch - LSPosed Module](https://github.com/0xdad0/ssl-kill-switch-lsposed)
