...
source_url: https://hacktricks.wiki/mobile-pentesting/android-app-pentesting/intent-injection.html
...
```
and the receiving Activity forwards the `url` query parameter into a WebView, you can force the app to render arbitrary remote content in its own WebView context.[\[2\]](#references)[\[3\]](#references)[\[4\]](#references)
...
```
1. Launch the victim’s Unity activity with the CLI pre-init flag in the `unity` extra. Example ADB PoC:
...
Minimal payload (Kotlin)[\[25\]](#references)
...
```
ADB PoC (no special permissions)[\[26\]](#references)
...
When exported components expect specific extras, guessing payload shape causes time waste and false negatives. You can automate discovery of keys/types directly from Smali and emit ready-to-run adb commands.[\[5\]](#references)[\[6\]](#references)
...
`Intent.parseUri(..., URI_ALLOW_UNSAFE)` + provider grant abuse
A high-impact variant appears when a **third-party SDK adds an exported proxy Activity** in the **merged manifest** and that Activity turns attacker-controlled input into a new `Intent` that the victim app launches.[\[21\]](#references)
...
When you control the sender, add `Intent.FLAG_DEBUG_LOG_RESOLUTION` to an implicit Intent to make Android log how resolution happens and which component will be selected.[\[17\]](#references)[\[18\]](#references)
