...
source_url: https://hacktricks.wiki/mobile-pentesting/android-app-pentesting/smali-changes.html
...
Another **script** that facilitates this task a lot is **https://github.com/ax/apk.sh**[\[6\]](#references)
...
```
If the target is a split package, either rebuild the whole set or use tooling that **joins the APKs first**. [**apk.sh**](https://github.com/ax/apk.sh) is handy here because it can combine split APKs into a single patchable APK and fix public resource identifiers.[\[6\]](#references)

For Frida/Objection-oriented repacking workflows, also check [Android Anti-Instrumentation & SSL Pinning Bypass](android-anti-instrumentation-and-ssl-pinning-bypass.html).
...
Or you can [**check below some Smali changes explained**](smali-changes.html#modifying-smali).
...
```
`jarsigner` still works for some quick tests, but for modern Android builds **`apksigner` is preferred** because it handles the newer APK signature schemes.[\[4\]](#references)
...
```
If the APK contains bundled native libraries (`lib/*.so`), Android now recommends using **`-P 16`** so the `.so` files are aligned for both 16 KiB and 4 KiB page-size devices:[\[5\]](#references)
...
- Add 2 to the number of declared variables: Ex: from
