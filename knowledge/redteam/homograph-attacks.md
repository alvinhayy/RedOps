...
source_url: https://hacktricks.wiki/generic-methodologies-and-resources/phishing-methodology/homograph-attacks.html
...
Because every glyph is identified internally by its **Unicode code point**, a single substituted character is enough to defeat naïve string comparisons (e.g., `"Παypal.com"` vs. `"Paypal.com"`).[\[1\]](#references)[\[3\]](#references)
...
4. Sender display name (e.g.,
5. **Redirect chain** – Victim is bounced through seemingly benign websites or URL shorteners before landing on the malicious host that harvests credentials / delivers malware.<sup>[\[1\]](#references)</sup>

## Unicode Ranges Commonly Abused

The following examples are Unicode blocks that contain characters commonly used to create cross-script look-alikes.[\[2\]](#references)[\[3\]](#references)
...
Internationalised Domain Names (IDNs) have a Unicode form and an ASCII-compatible **Punycode** form prefixed with `xn--`. Convert hostnames to the IDNA/Punycode form before allow-listing or comparing them, while retaining the Unicode form for display.[\[6\]](#references)
...
Tools such as **dnstwist** (`--fuzzers homoglyph`) or **urlcrazy** can enumerate visually-similar domain permutations and are useful for proactive takedown / monitoring.[\[4\]](#references)[\[5\]](#references)
...
These samples originate from Unit 42 research (July 2025) and illustrate how homograph abuse is combined with URL redirection and CAPTCHA evasion to bypass automated analysis.[\[1\]](#references)
