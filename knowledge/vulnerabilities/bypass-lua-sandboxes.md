...
source_url: https://hacktricks.wiki/generic-methodologies-and-resources/lua/bypass-lua-sandboxes/index.html
...
If the host application pushes scripts to clients and the VM exposes auto-run hooks (e.g., OnInit/OnLoad/OnEnter), place your payload there for drive-by compromise as soon as the script loads:[\[1\]](#references)
...
Lua’s loader API changed across versions: in Lua 5.1, `load` reads from a reader function and `loadstring` reads from a string; Lua 5.2’s `load` accepts either a string or a reader function, and `loadstring` is deprecated as its equivalent.[\[5\]](#references)[\[6\]](#references)
