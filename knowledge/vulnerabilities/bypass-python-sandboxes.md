...
source_url: https://hacktricks.wiki/generic-methodologies-and-resources/python/bypass-python-sandboxes/index.html
...
```
For more information about how pickle works check this: [https://checkoway.net/musings/pickle/](https://checkoway.net/musings/pickle/).[\[16\]](#references)
...
A different but very common bug is to **insert attacker-controlled data into a string and then evaluate that string as an f-string**. This is **not** Jinja/SSTI; the Python interpreter itself executes anything placed inside `{...}` during the second evaluation step:[\[10\]](#references)
...
- ReportLab/xhtml2pdf triple-bracket [[[…]]] expression evaluation → RCE (CVE-2023-33733). It abuses rl_safe_eval to reach function.**globals** and os.system from evaluated attributes (for example, font color) and returns a valid value to keep rendering stable.<sup>[\[7\]](#references)[\[8\]](#references)[\[9\]](#references)</sup>

[Reportlab Xhtml2pdf Triple Brackets Expression Evaluation Rce Cve 2023 33733](reportlab-xhtml2pdf-triple-brackets-expression-evaluation-rce-cve-2023-33733.html)
...
In [**this writeup**](https://blog.arkark.dev/2022/11/18/seccon-en/#misc-latexipy) UFT-7 is used to load and execute arbitrary python code inside an apparent sandbox:[\[11\]](#references)
...
### [RCE with](#rce-with-decorators) [decorators](https://docs.python.org/3/glossary.html#term-decorator)
...
```
#### [Crating objects with](#crating-objects-with-metaclasses) [metaclasses](https://docs.python.org/3/reference/datamodel.html#metaclasses)

The key thing that metaclasses allow us to do is **make an instance of a class, without calling the constructor** directly, by creating a new class with the target class as a metaclass.[\[15\]](#references)
...
However, **by default python imports a lot of modules in memory**. These modules may seem benign, but some of them are **also importing dangerous** functionalities inside of them that can be accessed to gain even **arbitrary code execution**.[\[4\]](#references)[\[5\]](#references)
...
If you **send** a **string** to python that is going to be **formatted**, you can use `{}` to access **python internal information.** You can use the previous examples to access globals or builtins for example.[\[14\]](#references)
...
From [here](https://www.cyberark.com/resources/threat-research-blog/anatomy-of-an-llm-rce): `().class.base.subclasses()[108].load_module('os').system('dir')`.[\[12\]](#references)

### From format to RCE loading libraries

According to the [**TypeMonkey chall from this writeup**](https://corgi.rip/posts/buckeye-writeups/) it’s possible to load arbitrary libraries from disk abusing the format string vulnerability in python.[\[13\]](#references)
...
If you want to **learn** about **python bytecode** in depth read this **awesome** post about the topic: **https://towardsdatascience.com/understanding-python-bytecode-e7edaae8734d**
...
Like in the following example, you **can access the code object** of that function, but just reading the disassemble you **don’t know how to calculate the flag** (*imagine a more complex `calc_flag` function*).[\[3\]](#references)
...
[Decompile compiled python binaries (exe, elf) - Retreive from .pyc](../../basic-forensic-methodology/specific-software-file-type-tricks/.pyc.html)
...
Therefore, checks like the following:[\[6\]](#references)
