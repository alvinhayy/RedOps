...
source_url: https://hacktricks.wiki/macos-hardening/macos-security-and-privilege-escalation/mac-os-architecture/macos-nvram.html
...
```
Read the effective setting with `csrutil status`; raw `nvram` output may use percent-encoded little-endian bytes. See [macOS SIP](../macos-security-protections/macos-sip.html) for the protection and bypass implications.
...
Do not use `bputil` policy-changing options during an audit. A normal macOS compromise should not be able to turn the above fields on silently: the downgrade path deliberately requires physical entry into paired 1TR and owner authentication.[\[4\]](#references)
...
## NVRAM and Apple silicon boot-policy audit
