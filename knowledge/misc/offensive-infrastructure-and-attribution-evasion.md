---
title: "Offensive Infrastructure and Attribution Evasion"
source: hacktricks.wiki
source_url: https://hacktricks.wiki/privacy/offensive-infrastructure-and-attribution-evasion.html
...
An **ORB network** is a managed proxy fleet used as an intermediate service. Mandiant divides them into provisioned networks of leased servers, non-provisioned networks of compromised routers/IoT, and hybrids. A mature topology has four logical roles:[\[2\]](#references)
...
MITRE distinguishes external proxies from **multi-hop proxies (T1090.003)**. The important property is not hop count but separation of knowledge and administration.[\[3\]](#references)
...
With classic **domain fronting (T1090.004)**, the TLS connection advertises an allowed front domain in SNI while the encrypted HTTP `Host` or HTTP/2 `:authority` requests a different back-end domain. A cooperating CDN routes on the inner value. A network observer without TLS decryption sees the front; the CDN sees both values and the origin. In domainless variants, SNI may be empty while another routing field selects the destination.[\[4\]](#references)
...
Commands arrive through a public, broadcast or append-only source while results leave through an unrelated channel after a delay. Examples of the primitive include web-service one-way communication and dead drops. Separation prevents a single flow from looking bidirectional and frustrates simple request/response correlation.[\[9\]](#references)
...
A quiet first stage performs inventory and only promotes selected systems to an unrelated second-stage channel. The second endpoint, protocol and process may share no infrastructure with the first. This limits exposure of capable infrastructure and is explicitly modeled as ATT&CK T1104.[\[10\]](#references)
...
Different hops translate HTTPS, QUIC, WebSocket, DNS, SSH or a message-queue API rather than transparently forwarding packets. Translation removes a single end-to-end protocol fingerprint but creates gateways with distinctive timing, buffering and semantic conversion. Protocol tunneling (T1572) can be combined with proxies and service impersonation.[\[11\]](#references)
...
Instead of beaconing, an implant monitors traffic already reaching a router/VPN and activates only on a magic value, source-port pattern or authenticated token. Normal traffic continues to the real service. ATT&CK calls this Traffic Signaling (T1205), with documented network-device and APT examples.[\[12\]](#references)
