...
source_url: https://hacktricks.wiki/generic-methodologies-and-resources/basic-forensic-methodology/file-integrity-monitoring.html
...
File Integrity Monitoring (FIM) is a critical security technique that protects IT environments and data by tracking changes in files. It usually combines:[\[1\]](#references)[\[3\]](#references)
...
The collection backend matters:[\[2\]](#references)[\[9\]](#references)
...
Example baseline + verification with AIDE:[\[4\]](#references)
...
aide --check
```
Example `osquery` FIM configuration focused on attacker persistence paths:[\[1\]](#references)
...
```
If you need **process attribution** instead of only path-level changes, prefer audit-backed telemetry such as `osquery` `process_file_events` or Wazuh `whodata` mode.[\[1\]](#references)[\[3\]](#references)[\[9\]](#references)

#### `io_uring`: syscall telemetry is not FIM

`io_uring`: syscall telemetry is not FIM
On modern Linux, watching `openat(2)`, `write(2)`, or other syscall entry points is **not equivalent to monitoring the resulting filesystem operation**. The 2025 **Curing** proof of concept queued file and network requests through `io_uring`, so products or policies attached only to the corresponding per-operation syscall entries lost process telemetry. In the same tests, a path-scoped FIM component still observed file modifications, showing that this is a **hook-placement blind spot**, not a permission bypass or a way to defeat every FIM backend.[\[10\]](#references)

When validating a sensor, modify the same canary through several paths: normal `write`, `mmap` + `msync`, `truncate`, `sendfile`/`copy_file_range`, atomic replacement, and `io_uring`. Check not only that the final hash drift is found, but also whether the event preserves the responsible process, container/cgroup, namespace-visible path, inode, and rename pair. A missing real-time event followed by a periodic-scan mismatch must be treated as **telemetry loss**, not as a routine unexplained change.[\[10\]](#references)[\[11\]](#references)

For eBPF-based monitoring, prefer common kernel enforcement points over a list of syscall-entry probes. For example, Tetragon’s file-access policy uses `security_file_permission` to cover ordinary I/O, `sendfile`, `copy_file_range`, AIO, and `io_uring`; it separately covers memory mappings with `security_mmap_file` and size changes with `security_path_truncate`. This also illustrates why one hook is rarely complete coverage.[\[11\]](#references)

### Windows

On Windows, FIM is stronger when you combine **change journals** with **high-signal process/file telemetry**:[\[6\]](#references)[\[7\]](#references)
...
Quick USN triage examples:[\[7\]](#references)
...
```
For deeper anti-forensic ideas around **timestamp manipulation**, **ADS abuse**, and **USN tampering**, check [Anti-Forensic Techniques](anti-forensic-techniques.html).
