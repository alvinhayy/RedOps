---
title: "Reproducible Privacy Testing"
source: hacktricks.wiki
source_url: https://hacktricks.wiki/privacy/reproducible-privacy-testing.html
...
```
1. Create a cleaned copy using the format-specific process in [Privacy-Preserving Communications and Sharing](privacy-preserving-communications-and-sharing.html) .
2. Compare metadata inventories:

```
exiftool -a -u -g1 ./original/file
exiftool -a -u -g1 ./clean/file
```
1. Render/open the copy in a disposable context. Check hidden content, attachments, links, forms, layers, thumbnails and visual identifiers.
2. Search only the staged copy for known canary author/email/path strings.
3. Hash the final output and have a second person verify the exact file being published.
