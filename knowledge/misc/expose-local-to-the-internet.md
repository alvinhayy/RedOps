...
source_url: https://hacktricks.wiki/generic-hacking/reverse-shells/expose-local-to-the-internet.html
...
Serveo’s documentation describes SSH forwarding for HTTP endpoints and private/public TCP forwarding; requesting a non-80/443 public TCP port (including port 0 for a random port) requires a registered user.[\[1\]](#references)
...
SocketXP’s getting-started guide documents `socketxp connect tcp://localhost:22` and `socketxp connect http://localhost:8080` for TCP and HTTP tunnels; the agent is authenticated with a portal token first.[\[2\]](#references)
...
ngrok’s CLI documents HTTP and TCP tunnels; its FAQ says free-tier TCP endpoints require a valid payment method and that the card is not charged.[\[3\]](#references)[\[4\]](#references)
...
The legacy Telebit.js CLI help documents `telebit http <port>` for HTTPS forwarding and `telebit tcp <local> [remote]` for raw TCP; availability depends on the deployment and relay.[\[5\]](#references)
...
LocalXpose’s current site documents `loclx tunnel http --to 3000`, lists HTTP/TLS/TCP/UDP support, and says the free plan covers personal/light commercial use while TCP tunneling is a paid-plan capability.[\[6\]](#references)[\[7\]](#references)
...
Expose documents `expose share` for HTTP/HTTPS local URLs and a PRO-only `expose share-port` command for TCP ports.[\[8\]](#references)[\[9\]](#references)
...
The official localtunnel repository describes exposing localhost for testing and documents the NPX command below.[\[10\]](#references)
...
Cloudflare’s current docs show unauthenticated “Quick” tunnels for local development, and the product overview lists HTTP, HTTPS, TCP, SSH, and RDP among supported published protocols.[\[11\]](#references)[\[12\]](#references)

For a locally managed named tunnel, Cloudflare documents the `tunnel login`, `create`, `route dns`, and `--config ... run ...` workflow.[\[13\]](#references)[\[14\]](#references)[\[17\]](#references)
...
./frpc -c frpc.toml
```
The current project documentation includes QUIC transport, token/OIDC authentication, bandwidth limits, health checks, and Go-template range mappings—consult the release matching your deployment before using any of those options.[\[20\]](#references)

## Pinggy (SSH-based)

Pinggy documents SSH reverse forwarding over port 443, so it can work in networks where outbound SSH on port 22 is blocked. Its free plan times out after 60 minutes and uses a new URL after reconnecting, while Pro adds persistent tunnels and custom domains.[\[21\]](#references)[\[22\]](#references)
