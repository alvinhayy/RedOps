...
source_url: https://hacktricks.wiki/crypto/public-key/rsa/index.html
...
Some broken big-integer generators leak structure directly into the public modulus: each limb contains only a small random subfield and the rest of the bits are `0`. In practice this appears as **regularly spaced zero blocks** across `n`, often aligned to 32-bit or 128-bit limbs.[\[1\]](#references)
...
This is more serious than a statistical bias: if both private factors `p` and `q` are short-sleeved, the modulus may become **easy to factor**.[\[1\]](#references)
