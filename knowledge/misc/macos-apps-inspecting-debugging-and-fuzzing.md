...
source_url: https://hacktricks.wiki/macos-hardening/macos-security-and-privilege-escalation/macos-apps-inspecting-debugging-and-fuzzing/index.html
...
Note that programs written in Objective-C **retain** their class declarations **when** **compiled** into [Mach-O binaries](../macos-files-folders-and-binaries/universal-binaries-and-mach-o-format.html). Such class declarations **include** the name and type of:
...
| **run (r)** | Start execution, which will continue until a breakpoint is hit or the process terminates. |
| **process launch --stop-at-entry** | Start execution and stop at the entry point. |
| **continue (c)** | Continue execution of the debugged process. |
| **nexti (n / ni)** | Execute the next instruction. This command skips over function calls. |
| **stepi (s / si)** | Execute the next instruction. Unlike the `nexti` command, this command steps into function calls. |
| **finish (f)** | Execute the remaining instructions in the current function (frame), return, and halt. |
| **control + c** | Pause execution. If the process has been run ( `r` ) or continued (`c` ), this causes it to halt wherever it is currently executing. |
| **breakpoint (b)** | `b main` # Any function called main ``b <binname>`main`` # Main function of the binary `b set -n main --shlib <lib_name>` # Main function of the indicated binary `breakpoint set -r '\[NSFileManager .*\]$'` # Any NSFileManager method `breakpoint set -r '\[NSFileManager contentsOfDirectoryAtPath:.*\]$'` `break set -r . -s libobjc.A.dylib` # Break in all functions of that library `b -a 0x0000000100004bd9` `br l` # Breakpoint list `br e/dis <num>` # Enable/disable breakpoint `breakpoint delete <num>` |
| **help** | `help breakpoint` # Get help for the breakpoint command `help memory write` # Get help for writing to memory |
| **reg** | `reg read` `reg read $rax` `reg read $rax --format <`[format](https://lldb.llvm.org/use/variable.html#type-format) > `reg write $rip 0x100035cc0` |
| **x/s <register/memory address>** | Display memory as a null-terminated string. |
| **x/i <register/memory address>** | Display memory as an assembly instruction. |
| **x/b <register/memory address>** | Display memory as a byte. |
| **print object (po)** | This prints the object referenced by the parameter. `po $raw` `{` `dnsChanger = {` `"affiliate" = "";` `"blacklist_dns" = ();` Most Apple Objective-C APIs or methods return objects and should therefore be displayed with the “print object” (`po` ) command. If`po` does not produce meaningful output, use`x/b` . |
| **memory** | `memory read 0x000....``memory read $x0+0xf2a``memory write 0x100600000 -s 4 0x41414141` # Write AAAA at that address`memory write -f s $rip+0x11f+7 "AAAA"` # Write AAAA at the address |
| **disassembly** | `dis` # Disassemble the current function `dis -n <funcname>` # Disassemble a function `dis -n <funcname> -b <basename>` # Disassemble a function `dis -c 6` # Disassemble six lines `dis -c 0x100003764 -e 0x100003768` # Disassemble from one address to the other `dis -p -c 4` # Start disassembling at the current address |
| **parray** | `parray 3 (char **)$x1` # Inspect an array of three components in the`x1` register |
| **image dump sections** | Print a map of the current process memory. |
| **image dump symtab <library>** | `image dump symtab CoreNLP` # Get the address of every symbol from CoreNLP |
