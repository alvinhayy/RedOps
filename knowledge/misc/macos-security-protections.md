...
source_url: https://hacktricks.wiki/macos-hardening/macos-security-and-privilege-escalation/macos-security-protections/index.html
...
[macOS Gatekeeper / Quarantine / XProtect](macos-gatekeeper.html)
...
[macOS Launch/Environment Constraints & Trust Cache](macos-launch-environment-constraints.html)
...
**macOS** now **alerts** every time a tool uses a well known **technique to persist code execution** (such as Login Items, Daemons…), so the user knows better **which software is persisting**.[\[3\]](#references)

This runs with a **daemon** located in `/System/Library/PrivateFrameworks/BackgroundTaskManagement.framework/Versions/A/Resources/backgroundtaskmanagementd` and the **agent** in `/System/Library/PrivateFrameworks/BackgroundTaskManagement.framework/Support/BackgroundTaskManagementAgent.app`[\[1\]](#references)

The way **`backgroundtaskmanagementd`** knows something is installed in a persistent folder is by **getting the FSEvents** and creating some **handlers** for those.[\[1\]](#references)

Moreover, there is a plist file that contains **well known applications** that frequently persists maintained by apple located in: `/System/Library/PrivateFrameworks/BackgroundTaskManagement.framework/Versions/A/Resources/attributions.plist`[\[3\]](#references)
...
It’s possible to **enumerate all** the configured background items running the Apple cli tool:[\[3\]](#references)

```
# The tool will always ask for the users password
sfltool dumpbtm
```
Moreover, it’s also possible to list this information with [**DumpBTM**](https://github.com/objective-see/DumpBTM).[\[2\]](#references)
...
./dumpBTM
```
This information is being stored in **`/private/var/db/com.apple.backgroundtaskmanagement/BackgroundItems-v4.btm`** and the Terminal needs FDA.[\[2\]](#references)

### Messing with BTM

When a new persistence is found an event of type **`ES_EVENT_TYPE_NOTIFY_BTM_LAUNCH_ITEM_ADD`**. So, any way to **prevent** this **event** from being sent or the **agent from alerting** the user will help an attacker to ***bypass*** BTM.[\[1\]](#references)

- **Resetting the database** : Running the following command resets the database (which should rebuild it from scratch). However, after doing this,**no new persistence alerts appear until the system is rebooted** .<sup>[\[1\]](#references)</sup>  - **root** is required.
