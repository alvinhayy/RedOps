...
source_url: https://hacktricks.wiki/hardware-physical-access/firmware-analysis/bootloader-testing.html
...
2. During boot, hit a known break key (often any key, 0, space, or a board-specific “magic” sequence) before
3.
...
```
8. Configure network and fetch a kernel/fit image from LAN:
9.
...
10. If env storage isn’t write-protected, you can persist control:
11.
...
14. If the platform claims secure/verified boot with FIT images, try both unsigned and tampered images:
15.
Standard Boot ( `bootstd` ),`extlinux` , and script bootflows
...
16. On modern U-Boot builds,

## [Network-boot surface (DHCP/PXE) and rogue servers](#network-boot-surface-dhcppxe-and-rogue-servers)
...
2. U-Boot’s legacy BOOTP/DHCP handling has had memory-safety issues. For example, CVE‑2024‑42040 describes memory disclosure via crafted DHCP responses that can leak bytes from U-Boot memory back on the wire.
3.
...
## [SoC ROM recovery modes that override normal boot](#soc-rom-recovery-modes-that-override-normal-boot)
...
## [UEFI/PC-class bootloaders: quick checks](#uefipc-class-bootloaders-quick-checks)

1. ESP tampering, rollback, and key-enrollment testing
...
1. Stale shim / SBAT / dbx revocation testing
...
- If the target uses dual-slot / A/B updates, review the anti-rollback and slot-desync techniques in the [firmware analysis overview](README.html) so you don’t miss updater-only trust gaps outside the bootloader itself.
