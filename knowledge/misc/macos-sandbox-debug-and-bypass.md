...
source_url: https://hacktricks.wiki/macos-hardening/macos-security-and-privilege-escalation/macos-security-protections/macos-sandbox/macos-sandbox-debug-and-bypass/index.html
...
Finally, the sandbox is activated with a call to **`__sandbox_ms`**, which calls **`__mac_syscall`**.[\[1\]](#references)[\[3\]](#references)
...
In the [**last examples of Word sandbox bypass**](macos-office-sandbox-bypasses.html#word-sandbox-bypass-via-login-items-and-.zshenv) can be appreciated how the **`open`** cli functionality could be abused to bypass the sandbox.
...
As indicated in [this writeup](https://jhftss.github.io/A-New-Era-of-macOS-Sandbox-Escapes/), the info about Mach services is stored in `/System/Library/xpc/launchd.plist`. It’s possible to find all the System and User Mach services by searching inside that file for `<string>System</string>` and `<string>User</string>`.[\[4\]](#references)
...
These Mach services were firstly abused to [escape from the sandbox in this writeup](https://jhftss.github.io/A-New-Era-of-macOS-Sandbox-Escapes/). By that time, **all the XPC services required** by an application and its framework were visible in the app’s PID domain (these are Mach Services with `ServiceType` as `Application`).[\[4\]](#references)
...
```
Several examples abusing this technique can be found in the [**original writeup**](https://jhftss.github.io/A-New-Era-of-macOS-Sandbox-Escapes/), however, the following are some summarized examples.[\[4\]](#references)
...
[**This research**](https://saagarjha.com/blog/2020/05/20/mac-app-store-sandbox-escape/) discovered 2 ways to bypass the Sandbox. Because the sandbox is applied from userland when the **libSystem** library is loaded. If a binary could avoid loading it, it would never get sandboxed:[\[2\]](#references)
...
As explained in the **[bonus of this writeup](https://jhftss.github.io/A-New-Era-of-macOS-Sandbox-Escapes/)** a sandbox restriction like:[\[4\]](#references)
...
`_libsecinit_initializer` to prevent the sandbox```
...
Sandbox Bypassed!
```
#### [Interpost `__mac_syscall` to prevent the Sandbox](#interpost-__mac_syscall-to-prevent-the-sandbox)

`__mac_syscall` to prevent the Sandbox```
// gcc -dynamiclib interpose.c -o interpose.dylib
#include <stdio.h>
#include <string.h>
// Forward Declaration
