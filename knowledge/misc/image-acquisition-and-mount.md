...
source_url: https://hacktricks.wiki/generic-methodologies-and-resources/basic-forensic-methodology/image-acquisition-and-mount.html
...
The AFF4 v1.0 specification, authored by Bradley L. Schatz and Michael I. Cohen, defines a forensic container with virtualized storage, arbitrary metadata, extensible compression and hashing, and high-throughput operation.[\[1\]](#references)
...
```
For filesystem-specific no-replay mounts, ext3/ext4 use `noload`, while XFS uses `norecovery` and requires read-only mode.[\[3\]](#references)[\[4\]](#references)
...
For a dirty ext3/ext4 filesystem, use `ro,noload` when journal replay must be prevented.[\[3\]](#references)
