...
source_url: https://hacktricks.wiki/stego/workflow/index.html
...
ls -lah target
```
If `file` and the extension disagree, investigate the signature instead of trusting the suffix. `file` is also heuristic and can be confused by malformed or polyglot input. Treat common formats as containers when appropriate (for example, OOXML documents are ZIP packages).[\[2\]](#references)
...
These catch high-frequency container-level cases: metadata payloads, appended bytes, and embedded files disguised by extension.[\[1\]](#references)[\[3\]](#references)
...
foremost -i file
```
Project repository: `korczis/foremost`.[\[4\]](#references)
...
For broader collections of steganography utilities and technique-specific resources, see the bundled stego-toolkit and 0xRick’s curated list.[\[1\]](#references)[\[7\]](#references)
