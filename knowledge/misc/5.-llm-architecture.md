...
source_url: https://hacktricks.wiki/AI/AI-llm-architecture/5.-llm-architecture.html
...
LLM architecture example from [https://github.com/rasbt/LLMs-from-scratch/blob/main/ch04/01_main-chapter-code/ch04.ipynb](https://github.com/rasbt/LLMs-from-scratch/blob/main/ch04/01_main-chapter-code/ch04.ipynb):[\[1\]](#references)
...
print(out)
```
Let’s explain it step by step:[\[2\]](#references)
...
- **Layer Details:**  - **First Linear Layer:** Expands the dimensionality from`emb_dim` to`4 * emb_dim` .
...
- **Key Components:**  - **Queries, Keys, Values:** Linear projections of the input, used to compute attention scores.
...
- **Components:**  - **`eps`:** A small constant (`1e-5` ) added to the variance to prevent division by zero during normalization.
...
- **Normalization Process:**  - **Compute Mean (`mean`):** Calculates the mean of the input`x` across the embedding dimension (`dim=-1` ), keeping the dimension for broadcasting (`keepdim=True` ).
...
1. **First Residual Path (Self-Attention):**  - **Input (`shortcut`):** Save the original input for the residual connection.
...
2. **Second Residual Path (FeedForward):**  - **Input (`shortcut`):** Save the updated input for the next residual connection.
...
- **Embedding Layers:**  - **Token Embeddings (`tok_emb`):** Converts token indices into embeddings. As reminder, these are the weights given to each dimension of each token in the vocabulary.
...
Having the GPT structure defined it’s possible to find out the number of parameters to train:[\[1\]](#references)
