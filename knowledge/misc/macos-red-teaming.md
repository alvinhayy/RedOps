...
source_url: https://hacktricks.wiki/macos-hardening/macos-red-teaming/index.html
...
However, to install an application in an enrolled device, you still need it to be signed by a developer account… however, upon MDM enrolment the **device adds the SSL cert of the MDM as a trusted CA**, so you can now sign anything.[\[4\]](#references)
...
JAMF can run **custom scripts** (scripts developed by the sysadmin), **native payloads** (local account creation, set EFI password, file/process monitoring…) and **MDM** (device configurations, device certificates…).[\[5\]](#references)
...
The **`jamf`** binary contained the secret to open the keychain which at the time of the discovery was **shared** among everybody and it was: **`jk23ucnq91jfu9aj`**.[\[5\]](#references)
...
[macOS Network Services & Protocols](../macos-security-and-privilege-escalation/macos-protocols.html)
...
For example, the info about user called *mark* is stored in */var/db/dslocal/nodes/Default/users/mark.plist* and the info about the group *admin* is in */var/db/dslocal/nodes/Default/groups/admin.plist*.

In addition to using the HasSession and AdminTo edges, **MacHound adds three new edges** to the Bloodhound database:[\[2\]](#references)
