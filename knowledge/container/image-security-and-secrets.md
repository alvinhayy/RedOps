...
source_url: https://hacktricks.wiki/linux-hardening/containers-namespaces/container-security/image-security-and-secrets.html
...
Container security starts before the workload is launched. The image determines which binaries, interpreters, libraries, startup scripts, and embedded configuration reach production. If the image is backdoored, stale, or built with secrets baked into it, the runtime hardening that follows is already operating on a compromised artifact. [\[1\]](#references)
...
One of the oldest mistakes in container build pipelines is embedding secrets directly into the image or passing them through build arguments or environment variables. Depending on the mechanism, those values may persist in image metadata or recoverable layers, appear in build logs, or become visible through inspection commands such as `docker inspect`. Build-time secrets should be mounted ephemerally during the build rather than copied into the image filesystem. [\[4\]](#references)
