...
source_url: https://hacktricks.wiki/linux-hardening/containers-namespaces/container-security/authorization-plugins.html
...
`docker exec` Adds Privilege After Container Creation
...
capsh --print
```
Once `CAP_SYS_ADMIN` or a similarly strong capability is present, many breakout techniques described in [capabilities.md](protections/capabilities.html) and [privileged-containers.md](privileged-containers.html) become reachable.
