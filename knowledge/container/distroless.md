...
source_url: https://hacktricks.wiki/linux-hardening/containers-namespaces/container-security/distroless.html
...
[Bypass FS protections: read-only / no-exec / Distroless](../../linux-basics/bypass-linux-restrictions/bypass-fs-protections-read-only-no-exec-distroless/index.html)
...
The key point is that distroless is an **image property**, not a runtime protection. Its value comes from reducing what is available inside the filesystem after compromise. Multi-stage builds are a common way to keep build tools out of the final image. [\[3\]](#references)
