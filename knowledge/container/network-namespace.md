...
source_url: https://hacktricks.wiki/linux-hardening/containers-namespaces/container-security/protections/namespaces/network-namespace.html
...
```
A historical example was the `containerd-shim` abstract-socket exposure bug, but the broader lesson is more important than the specific CVE: once a workload joins the host network namespace, abstract AF_UNIX services become part of the attack surface too.<sup>[\[3\]](#references)</sup> If those sockets appear runtime-related or administrative, pivot to [Runtime API And Daemon Exposure](../../runtime-api-and-daemon-exposure.html).
