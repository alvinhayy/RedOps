...
source_url: https://hacktricks.wiki/binary-exploitation/linux-kernel-exploitation/pixel-bigwave-bigo-job-timeout-uaf-kernel-write.html
...
- From the SELinux-confined **mediacodec** context,`/dev/bigwave` (Pixel AV1 hardware accelerator) is reachable. A backlog of jobs makes`BIGO_IOCX_PROCESS` hit its**16s wait_for_completion_timeout()** and return while the worker thread concurrently dequeues the same inline`job` structure.<sup>[\[1\]](#references)[\[2\]](#references)</sup>
- Closing the FD immediately frees `struct bigo_inst` (which embeds`struct bigo_job` ). The worker reconstructs`inst = container_of(job, ...)` and later uses freed fields such as**`job->regs`** inside`bigo_run_job()` , yielding a**Use-After-Free on the inline job/inst** .<sup>[\[1\]](#references)[\[2\]](#references)</sup>
- `bigo_pull_regs(core, job->regs)` performs`memcpy_fromio(regs, core->base, core->regs_size)` . By reclaiming the freed slab and overwriting`job->regs` , an attacker gets a**~2144-byte arbitrary kernel write** to a chosen address, with partial control of the bytes by pre-programming register values before the timeout.<sup>[\[1\]](#references)[\[2\]](#references)</sup>
- Tracked as **CVE-2025-36934** ; fixed in the**2026-01-05 Pixel/2025-12-01 ASB** builds.<sup>[\[3\]](#references)</sup>

## Attack surface mapping (SELinux → /dev reachability)

- Use tools like **DriverCartographer** to enumerate device nodes accessible from a given SELinux domain. Despite mediacodec’s constrained policy (software decoders should stay in an isolated context),`/dev/bigwave` remained reachable, exposing a large attack surface to post-media-RCE code.<sup>[\[1\]](#references)</sup>
...
```
- If userspace closes the FD after the timeout, `inst` /`job` are freed while the worker keeps using them → UAF. No synchronization ties FD lifetime to the worker thread’s job pointer.<sup>[\[1\]](#references)[\[2\]](#references)</sup>
...
5. **Data shaping:** Because registers are first programmed from user data (`bigo_push_regs` ), set them so the hardware does not execute, keeping the copied-back register image close to attacker-controlled bytes.<sup>[\[1\]](#references)</sup>
...
`/dev/vpu` `mmap()` → physical-memory R/W
Project Zero’s Pixel 10 follow-up replaced BigWave with another **mediacodec-reachable** driver: `/dev/vpu` for the **Chips&Media Wave677DV** decoder. The bug class is even shallower: the driver intends to expose only the VPU MMIO CSR window, but its `mmap` handler trusts the attacker-controlled VMA length.[\[4\]](#references)[\[5\]](#references)
...
Therefore, if `/dev/vpu` is reachable from a compromised app/service domain, a large `mmap()` does not stop at the register window: it keeps mapping the **contiguous physical pages after the VPU MMIO range** into userspace.[\[4\]](#references)
...
5. Read or overwrite kernel `.text` ,`.data` , credentials, function pointers, or build a more convenient arbitrary R/W primitive.<sup>[\[4\]](#references)</sup>
...
[Arm64 Static Linear Map Kaslr Bypass](arm64-static-linear-map-kaslr-bypass.html)

- Compared with the earlier BigWave UAF, this bug skips heap feng shui almost entirely: once the oversized mapping succeeds, the attacker gets **direct userspace access to kernel physical memory** .<sup>[\[4\]](#references)[\[5\]](#references)</sup>
