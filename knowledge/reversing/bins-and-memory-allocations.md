...
source_url: https://hacktricks.wiki/binary-exploitation/libc-heap/bins-and-memory-allocations.html
...
**When a thread frees** a chunk, **if it isn’t too big** to be allocated in the tcache and the respective tcache bin **isn’t full** (already 7 chunks), **it’ll be allocated in there**. If it cannot go to the tcache, it’ll need to wait for the heap lock to be able to perform the free operation globally.[\[6\]](#references)
...
There’s also an optimization, in this case, while having the heap lock, the thread **will fill his Tcache with heap chunks (7) of the requested size**, so in case it needs more, it’ll find them in Tcache.[\[6\]](#references)
...
In the following code it’s possible to see the **max bins** and **chunks per index**, the **`tcache_entry`** struct created to avoid double frees and **`tcache_perthread_struct`**, a struct that each thread uses to store the addresses to each index of the bin.[\[6\]](#references)
...
```
The function `__tcache_init` is the function that creates and allocates the space for the `tcache_perthread_struct` obj[\[6\]](#references)
...
```
Nine same-sized chunks are allocated and the first eight are freed. Seven **fill the tcache**, while the eighth reaches the unsorted bin because it is **too large for a fastbin**. The ninth remains allocated, preventing the eighth chunk from merging with the top chunk.
...
Small bins are faster than large bins but slower than fast bins.[\[5\]](#references)

Each bin of the 62 will have **chunks of the same size**: 16, 24, … (with a max size of 504 bytes in 32bits and 1024 in 64bits). This helps in the speed on finding the bin where a space should be allocated and inserting and removing of entries on these lists.[\[5\]](#references)
...
```
As in the unsorted-bin example, seven freed chunks **fill the tcache** and the eighth reaches the unsorted bin, while the allocated ninth chunk prevents top-chunk consolidation. Allocating the larger `0x110` request then causes the allocator to classify the unsorted chunk into its small bin.
...
Unlike small bins, which manage chunks of fixed sizes, each **large bin handle a range of chunk sizes**. This is more flexible, allowing the system to accommodate **various sizes** without needing a separate bin for each size.[\[5\]](#references)

In a memory allocator, large bins start where small bins end. The ranges for large bins grow progressively larger, meaning the first bin might cover chunks from 512 to 576 bytes, while the next covers 576 to 640 bytes. This pattern continues, with the largest bin containing all chunks above 1MB.[\[5\]](#references)
...
The pointer to the Top Chunk is stored in the `malloc_state` struct.[\[7\]](#references)
...
When malloc is used and a chunk is divided (from the unsorted bin or from the top chunk for example), the chunk created from the rest of the divided chunk is called Last Remainder and it’s pointer is stored in the `malloc_state` struct.[\[7\]](#references)
...
[Heap Functions Security Checks](heap-memory-functions/heap-functions-security-checks.html)
