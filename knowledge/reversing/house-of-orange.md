...
source_url: https://hacktricks.wiki/binary-exploitation/libc-heap/house-of-orange.html
...
- The canonical runnable example targets glibc 2.23.<sup>[\[1\]](#references)</sup>
- The same example is available with extensive inline commentary.<sup>[\[2\]](#references)</sup>
- This `_IO_list_all` /abort path was disrupted by glibc changes committed for glibc 2.26, so reproduce it with an older matching libc rather than assuming it works on current systems.<sup>[\[3\]](#references)</sup>
...
The following background and field layout correspond to the glibc 2.23 example.[\[1\]](#references)[\[2\]](#references)
...
The attack starts by moving the **top chunk** into the **unsorted bin**. In the glibc 2.23 example, this is achieved by calling `malloc` with a request larger than the corrupted top-chunk size but below the effective **`mp_.mmap_threshold`**, which would otherwise select an `mmap` allocation. The classic walkthrough uses the historical 128 KiB threshold as its planning value, but glibc can tune this threshold dynamically, so inspect the target rather than treating 128 KiB as universal. When corrupting the top size, ensure that **top-chunk address + size** reaches the required page boundary and that the **`PREV_INUSE`** bit remains set.[\[1\]](#references)

To get the top chunk inside the unsorted bin, allocate a chunk to create the top chunk, change the top chunk size (with an overflow in the allocated chunk) so that **top chunk + size** is page-aligned with the **prev_inuse** bit set. Then allocate a chunk larger than the new top chunk size. Note that `free` is never called to get the top chunk into the unsorted bin.[\[1\]](#references)[\[4\]](#references)
...
For the unsorted-bin write, set the old top chunk’s `bk` pointer to `_IO_list_all - 0x10`. During removal, glibc performs the equivalent of `victim->bk->fd = unsorted_chunks(av)`, so the unsorted-bin pointer is written into **`_IO_list_all`**.[\[1\]](#references)[\[2\]](#references)
...
The manipulation of the small bin gives control of the chunk’s forward pointer. Overlap the old top chunk with a fake **`_IO_FILE`** structure whose first bytes point to the command (for example, `/bin/sh`). Set `_IO_write_base` and `_IO_write_ptr` so the libc checks pass, and place a controlled jump-table pointer so the relevant virtual call resolves to `system` (or another useful target). These offsets and checks must match the target libc build.[\[1\]](#references)[\[2\]](#references)
...
- [3] [glibc `abort.c` diff changing abort stream handling](https://sourceware.org/git/?p=glibc.git;a=blobdiff;f=stdlib/abort.c;h=117a507ff88d862445551f2c07abb6e45a716b75;hp=19882f3e3dc1ab830431506329c94dcf1d7cc252;hb=91e7cf982d0104f0e71770f5ae8e3faf352dea9f;hpb=0c25125780083cbba22ed627756548efe282d1a0)
