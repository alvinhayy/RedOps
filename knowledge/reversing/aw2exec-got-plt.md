...
source_url: https://hacktricks.wiki/binary-exploitation/arbitrary-write-2-exec/aw2exec-got-plt.html
...
In a binary the GOT has the **addresses to the functions or** to the **PLT** section that will load the function address. The goal of this arbitrary write is to **override a GOT entry** of a function that is going to be executed later **with** the **address** of the PLT of the `system`**function** for example.[\[1\]](#references)
...
Some libc builds expose writable relocation targets, but this is not guaranteed: libc build flags, RELRO, symbol visibility, and version all matter. Inspect the exact loaded libc and resolve its ASLR base before choosing a target.
...
The referenced modern-libc notes document concrete libc GOT targets and version-specific call paths.[\[3\]](#references)
...
Another common technique is to overwrite a writable **`strlen`** GOT entry with **`system`**. If the program later calls that entry with controlled input, passing `"/bin/sh"` can spawn a shell.

In some particular libc builds, an internal `puts` path reaches `strlen`; overwriting that libc relocation can then turn `puts(user_input)` into `system(user_input)`. This call relationship is version/build-specific, so confirm it dynamically or in the target disassembly before relying on the chain.[\[3\]](#references)

## **One Gadget**

**One Gadget**

One-gadget constraints must hold at the overwritten call site; the referenced notes show how to evaluate them.[\[2\]](#references)
...
The HITCON training `lab13` writeup provides a complete example.[\[4\]](#references)
...
- [1] [Exploiting a GOT overwrite - ir0nstone’s Notes](https://ir0nstone.gitbook.io/notes/types/stack/got-overwrite/exploiting-a-got-overwrite)
- [2] [One Gadgets and Malloc Hook - ir0nstone’s Notes](https://ir0nstone.gitbook.io/notes/types/stack/one-gadgets-and-malloc-hook)
- [3] [nobodyisnobody/docs - Code execution on last libc: Targeting libc GOT entries](https://github.com/nobodyisnobody/docs/blob/main/code.execution.on.last.libc/README.md#1---targetting-libc-got-entries)
