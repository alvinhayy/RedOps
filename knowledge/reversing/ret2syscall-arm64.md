...
source_url: https://hacktricks.wiki/binary-exploitation/rop-return-oriented-programing/rop-syscall-execv/ret2syscall-arm64.html
...
```
Compile without a stack canary. The command does **not** disable PIE on toolchains that default to PIE; add `-fno-pie -no-pie` when a fixed executable base is required:
...
For Linux AArch64, `execve` is syscall number 221 (`0xdd`). The syscall ABI places the number in `x8` and the first three arguments in `x0`–`x2`:[\[1\]](#references)[\[2\]](#references)
...
Using ROPgadget, the original test system exposed the following libc gadgets. Offsets are build-specific and must be rediscovered against the target libc:[\[3\]](#references)
...
p.interactive()
```
The hard-coded libc base assumes ASLR is disabled. With ASLR enabled, obtain a libc disclosure and calculate the base before using these offsets. The chain also depends on the exact gadget side effects and stack layout shown above.

## References

- [1] [Linux kernel — generic syscall numbers (`__NR_execve` 221)](https://github.com/torvalds/linux/blob/master/include/uapi/asm-generic/unistd.h)
