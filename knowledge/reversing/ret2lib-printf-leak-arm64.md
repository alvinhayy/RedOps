...
source_url: https://hacktricks.wiki/binary-exploitation/rop-return-oriented-programing/ret2lib/ret2lib-printf-leak-arm64.html
...
```
- Recent toolchains may emit **PAC/BTI** instrumentation by default on some ARM64 targets. If you are building a lab binary for practice,**`-mbranch-protection=none`** keeps the classic ret2lib flow reproducible.<sup>[\[1\]](#references)</sup>
...
- **`x0`** to**`x7`** hold the first 8 function arguments, so a ret2libc chain must place the pointer to**`/bin/sh`** in**`x0`** before branching to**`system`** .<sup>[\[2\]](#references)</sup>
