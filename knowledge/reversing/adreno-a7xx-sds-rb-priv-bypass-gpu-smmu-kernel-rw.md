...
source_url: https://hacktricks.wiki/binary-exploitation/linux-kernel-exploitation/adreno-a7xx-sds-rb-priv-bypass-gpu-smmu-kernel-rw.html
...
- Bug: A7xx microcode kept masking $12 with 0x3 (2 bits) instead of 0x7 (3 bits). Since IB4 & 0x3 == 0, SDS was misidentified as IB0, allowing privileged packets from user-controlled SDS.<sup>[\[1\]](#references)</sup>
...
Goal: From SDS (misread as IB0) issue privileged CP packets to re-point the GPU SMMU to attacker-crafted page tables, then use GPU copy/write packets for arbitrary physical R/W. Finally, pivot to a fast CPU-side R/W via dirty pagetable.[\[1\]](#references)
...
- Read: CP_MEM_TO_MEM copies 4/8 bytes from target PA to a userspace-shared buffer (batch for larger reads)<sup>[\[3\]](#references)</sup>
...
- Construct an SDS buffer containing, in order:
  ...
  3. CP_SET_DRAW_STATE with flags to run-now<sup>[\[1\]](#references)</sup>

The exact packet encodings vary by firmware; use freedreno’s afuc/packet docs to assemble the words, and ensure the SDS submission path is taken by the driver.[\[2\]](#references)

## Finding Samsung kernel physbase under physical KASLR

Samsung randomizes the kernel physical base within a known region on Snapdragon devices. Brute-force the expected range and look for the first 16 bytes of _stext.[\[1\]](#references)
...
GPU R/W is slow and small-granularity. Pivot to a fast/stable primitive by corrupting your own process PTEs (“dirty pagetable”):[\[1\]](#references)[\[4\]](#references)
...
## Example dirty-pagetable pivot snippet
...
A local app with GPU access can execute privileged GPU packets, hijack the GPU SMMU, achieve arbitrary kernel physical/virtual R/W, disable SELinux and obtain root on affected Snapdragon A7xx devices (e.g., Samsung S23). Severity: High (kernel compromise).[\[1\]](#references)

### See also

[Pixel Bigwave Bigo Job Timeout Uaf Kernel Write](pixel-bigwave-bigo-job-timeout-uaf-kernel-write.html)
