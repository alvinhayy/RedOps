...
source_url: https://hacktricks.wiki/binary-exploitation/common-binary-protections-and-bypasses/stack-canaries/bf-forked-stack-canaries.html
...
Canary brute force is practical when a network service **forks a child for each connection** without replacing the process image. The children inherit the parent’s canary, so each new connection exposes an oracle for the same value. A service that calls `execve` after `fork`, regenerates the guard, rate-limits attempts, or lets a failed child terminate/restart the parent does not provide a stable oracle.[\[1\]](#references)[\[3\]](#references)
