...
source_url: https://hacktricks.wiki/binary-exploitation/rop-return-oriented-programing/ret2dlresolve.html
...
As explained in the page about [**GOT/PLT**](../arbitrary-write-2-exec/aw2exec-got-plt.html) and [**Relro**](../common-binary-protections-and-bypasses/relro.html), binaries without Full Relro will resolve symbols (like addresses to external libraries) the first time they are used. This resolution occurs calling the function **`_dl_runtime_resolve`**.
...
Check the second half of this video for a clear explanation of the technique:[\[1\]](#references)
...
1. Write fake structures in a writable location
...
From the [**pwntools documentation**](https://docs.pwntools.com/en/stable/rop/ret2dlresolve.html), this is how a **`ret2dlresolve`** attack look like:[\[7\]](#references)
...
You can find an **example of this technique here****containing a very good explanation of the final ROP chain**, but here is the final exploit used:[\[5\]](#references)
...
The following raw 0CTF `babystack` exploit shows the same technique without the pwntools helper: it stages forged relocation and symbol data in `.bss`, then invokes the resolver with the forged relocation offset.[\[3\]](#references)[\[6\]](#references)
...
- [7] [pwntools - ret2dlresolve documentation](https://docs.pwntools.com/en/stable/rop/ret2dlresolve.html)
