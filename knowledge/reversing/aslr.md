...
source_url: https://hacktricks.wiki/binary-exploitation/common-binary-protections-and-bypasses/aslr/index.html
...
- Another option for this in case the overflow is not that big and the exploit can be run locally is possible to
...
## Python exploit with given libc leak
...
- Read 8 bytes at that address (LE) using any kernel read (e.g., tracing-BPF helper calling `BPF_FUNC_probe_read_kernel` )<sup>[\[3\]](#references)</sup>
...
- Upstream arm64 removed linear-map randomization (commit `1db780bafa4c` ).<sup>[\[2\]](#references)</sup>
