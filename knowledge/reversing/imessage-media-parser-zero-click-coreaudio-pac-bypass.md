...
source_url: https://hacktricks.wiki/binary-exploitation/ios-exploiting/imessage-media-parser-zero-click-coreaudio-pac-bypass.html
...
## High-Level Chain Reported by the Research Repository

- Delivery vector: a malicious audio attachment (e.g., .amr / MP4 AAC) sent via iMessage/SMS.<sup>[\[1\]](#references)[\[2\]](#references)</sup>
...
- Post-exploitation: the repository claims that a BCM4387 hardware pivot permits Bluetooth-identity spoofing, triggers Auto-Unlock evaluation, and invokes CryptoTokenKit/SEP-backed ECDSA signing without an interactive prompt or exporting the key material.<sup>[\[1\]](#references)[\[2\]](#references)</sup>

## iMessage/BlastDoor attack surface notes

BlastDoor is a hardened service designed to parse untrusted message content. However, observed logs indicate paths where protections may be bypassed when messages are framed from a “known sender” and when additional filters (e.g., Blackhole) are relaxed:[\[1\]](#references)
...
- Yields code execution in the media parsing process. The write-up claims this escapes BlastDoor in the observed delivery path, enabling the next stage.<sup>[\[1\]](#references)[\[2\]](#references)</sup>
...
arm64e Pointer Authentication (PAC) impedes hijacking of return addresses and function pointers. Apple confirms that CVE-2025-31201 could let an attacker with arbitrary read/write bypass Pointer Authentication; the research repository describes how it believes that primitive fits into this chain.[\[2\]](#references)[\[3\]](#references)

Key idea:

- With arbitrary R/W, attackers can craft valid, re-signed pointers or pivot execution to PAC-tolerant paths. The so-called “RPAC path” enables control-flow under PAC constraints, turning a userland RCE into a reliable kernel exploit setup.<sup>[\[1\]](#references)[\[2\]](#references)</sup>
...
Once kernel is compromised, processes like identityservicesd can be impersonated and privileged cryptographic operations invoked via CryptoTokenKit without user prompts. Example logs:[\[1\]](#references)[\[2\]](#references)
...
- Use Secure Enclave–backed keys for unauthorized signing from a compromised context without exporting the key material, breaking trust models that assume only an intact authorized caller can request signatures.<sup>[\[1\]](#references)[\[2\]](#references)</sup>
- The repository reports device/token impersonation, AGX-framebuffer capture, HID surveillance, and system instability as observed or potential post-compromise effects; Apple does not confirm these chain-specific claims.<sup>[\[1\]](#references)[\[3\]](#references)</sup>
...
- Patch level: Apple fixed CVE-2025-31200 and CVE-2025-31201 in iOS/iPadOS 18.4.1; keep devices up to date.<sup>[\[3\]](#references)</sup>
...
- Apple lists the affected devices and the fixes in iOS/iPadOS 18.4.1, released April 16, 2025. The broader version range and full chain below are reported by the research repository.<sup>[\[1\]](#references)[\[2\]](#references)[\[3\]](#references)</sup>
...
- [3] [Apple - About the security content of iOS 18.4.1 and iPadOS 18.4.1](https://support.apple.com/en-euro/122282)
