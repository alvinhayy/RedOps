...
source_url: https://hacktricks.wiki/binary-exploitation/stack-overflow/uninitialized-variables.html
...
An automatic C variable that is not initialized has an **indeterminate value**. Evaluating that value can be undefined behavior, so the language does not guarantee that it contains a previous stack value, that two functions reuse the same address, or even that the compiler emits the apparent read. In vulnerable binaries, however, reused stack storage can expose stale data when the generated code copies or returns bytes that were never initialized.[\[4\]](#references)
...
- **Function 2: `useUninitializedVariable`** : Here, we declare another automatic variable`y` but do not assign a value to it. It is not automatically initialized to zero, but its value is indeterminate rather than a predictable copy of an earlier variable.

When these two functions run **sequentially**, a particular unoptimized build may reuse the same stack slot:

1. In `initializeVariable` ,`x` is assigned a value (`0x1234` ), which occupies a specific memory address.
2. In `useUninitializedVariable` ,`y` may occupy the same stack slot and stale bytes may appear in an observed build. This is a demonstration artifact, not behavior portable C code may rely on.
...
- **`demonstrateUninitializedVar` Function** : This function declares`uninitializedVar` without initializing it. Passing its value to`printf` invokes undefined behavior. In a simple unoptimized build the output may resemble stale stack data, while another compiler or optimization level may produce a different value, initialize it as a hardening measure, or optimize the code in an unexpected way.<sup>[\[4\]](#references)</sup>
...
```
## Mitigations and Compiler Options
...
- [3] [ARM64 Reversing and Exploitation Part 6: Exploiting an Uninitialized Stack Variable Vulnerability](https://8ksec.io/arm64-reversing-and-exploitation-part-6-exploiting-an-uninitialized-stack-variable-vulnerability/)
- [4] [SEI CERT C - EXP33-C: Do not read uninitialized memory](https://wiki.sei.cmu.edu/confluence/display/c/EXP33-C.+Do+not+read+uninitialized+memory)
