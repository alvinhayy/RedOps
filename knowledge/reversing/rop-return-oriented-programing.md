...
source_url: https://hacktricks.wiki/binary-exploitation/rop-return-oriented-programing/index.html
...
- Uses the **System V AMD64 ABI** calling convention on Unix-like systems, where the**first six integer or pointer arguments are passed in the registers `RDI`, `RSI`, `RDX`, `RCX`, `R8`, and `R9`** . Additional arguments are passed on the stack. The return value is placed in`RAX` .<sup>[\[1\]](#references)</sup>
...
When only a small overwrite is available, a chain can also use a stable `vsyscall` address as a `ret`-like stepping stone before a partial return-address overwrite; the Hack.lu `stackstuff` challenge is a concrete example.[\[2\]](#references)

## ROP chain in ARM64

Regarding **ARM64 basics and calling conventions**, check the following page for this information. Practical ARM64 chains include using `mprotect` to make a controlled region executable and longer real-world iOS chains assembled from available gadgets.[\[3\]](#references)[\[4\]](#references)

[!DANGER]
When jumping to a function with ROP on **ARM64**, you may need to skip its frame-setup instructions. Otherwise, the prologue can save the current frame state over controlled stack data and trap the chain in an unintended loop. For a common prologue this may mean entering at the second instruction or later, but verify the exact instruction boundary for the target function rather than assuming that the second instruction is always correct.

### Finding gadgets in system Dylds

The system libraries are compiled into a single file called **dyld_shared_cache_arm64**. This file contains the system libraries in a compressed format. To download it from the mobile device, use:
...
```
Then, you can use one of these tools to extract the libraries from the `dyld_shared_cache_arm64` file:
...
```
If preserving `fd` is hard, calling `open()` multiple times can make it feasible to **guess the descriptor** used for `/proc/self/mem`. On ARM Thumb targets, remember to **set the low bit** when branching (`addr | 1`).[\[5\]](#references)[\[6\]](#references)
...
- [1] [Exploiting Calling Conventions - ir0nstone’s Notes](https://ir0nstone.gitbook.io/notes/types/stack/return-oriented-programming/exploiting-calling-conventions)
- [2] [Hack.lu CTF 2015 - stackstuff writeup](https://guyinatuxedo.github.io/15-partial_overwrite/hacklu15_stackstuff/index.html)  - 64 bit, Pie and nx enabled, no canary, overwrite RIP with a `vsyscall` address with the sole purpose or return to the next address in the stack which will be a partial overwrite of the address to get the part of the function that leaks the flag
- 64 bit, Pie and nx enabled, no canary, overwrite RIP with a
- [3] [ARM64 Reversing and Exploitation Part 4: Using mprotect to Bypass NX Protection - 8kSec Blogs](https://8ksec.io/arm64-reversing-and-exploitation-part-4-using-mprotect-to-bypass-nx-protection-8ksec-blogs/)  - arm64, no ASLR, ROP gadget to make stack executable and jump to shellcode in stack
