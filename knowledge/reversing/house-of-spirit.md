...
source_url: https://hacktricks.wiki/binary-exploitation/libc-heap/house-of-spirit.html
...
```
The example above demonstrates the classic fastbin variant.[\[1\]](#references)
...
**The code from** **guyinatuxedo****is great to understand the attack.** Although this schema from the code summarises it pretty good:[\[3\]](#references)
...
- On modern glibc the **tcache fast-path** calls`tcache_put` before validating the next chunk size/`prev_inuse` , so only the current fake chunk has to look sane.<sup>[\[2\]](#references)</sup>
- Requirements:
...
**CTF**[**https://guyinatuxedo.github.io/39-house_of_spirit/hacklu14_oreo/index.html**](https://guyinatuxedo.github.io/39-house_of_spirit/hacklu14_oreo/index.html)
 ...
 Next time`scanf` is called, we can send the input`"/bin/sh"` and get a shell.<sup>[\[4\]](#references)</sup>
-
