...
source_url: https://hacktricks.wiki/binary-exploitation/libc-heap/heap-memory-functions/unlink.html
...
The diagram below illustrates the unlink pointer updates.[\[2\]](#references)

### Security Checks

- Check if the indicated size of the chunk is the same as the prev_size indicated in the next chunk
- Check that `P->fd->bk == P` and`P->bk->fd == P` .
- If the chunk is not small, check that `P->fd_nextsize->bk_nextsize == P` and`P->bk_nextsize->fd_nextsize == P`

### Leaks

Unlinking a chunk does not erase its pointer fields. If the freed chunk remains readable, those fields can disclose useful addresses.[\[1\]](#references)
