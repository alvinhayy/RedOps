...
source_url: https://hacktricks.wiki/binary-exploitation/libc-heap/heap-memory-functions/malloc-and-sysmalloc.html
fetched_at: 2026-09-20T09:50:19Z
license: unspecified
category: reversing
---

## [Allocation Order Summary](#allocation-order-summary--1) <sup>[\[1\]](#references)</sup>

[\[1\]](#references)

(Security checks are not described in this summary, and some cases are omitted for brevity.)
...
2. `_int_malloc` :
  ...
  5. Check the unsorted bin, use the first chunk with enough space
    1. If the selected chunk is larger, split it, return the requested part, and add the remainder to the unsorted bin
  ...
  6. Check the large bin in the index of the requested size
    1. Starting with the first chunk larger than the requested size, return a suitable chunk and place any remainder in the unsorted bin
  7. Check the large bins from the next indexes until the end
    1. Starting with the next larger index, split the first suitable chunk and add the remainder to the unsorted bin
...
The public `malloc` entry point calls `__libc_malloc`. This function first checks the tcache for a chunk of the requested size. If none is available, a single-threaded process calls `_int_malloc` in the main arena; otherwise it calls `_int_malloc` in the thread’s selected arena.[\[1\]](#references)
...
```
The returned pointer is passed through `tag_new_usable`, as shown in the code:
...
This is the function that allocates memory using the other bins and top chunk.[\[1\]](#references)
...
The first check determines whether the requested size belongs to a small bin. If it does, the allocator obtains the corresponding **small-bin index** and checks for an **available chunk**.

Then, a security check is performed checking:

- if `victim->bk->fd = victim` . To see that both chunks are correctly linked.

The chunk then receives the `inuse` bit, is unlinked from the doubly linked list, and receives the non-main-arena bit when appropriate.
...
While removing these chunks, adjacent free chunks are **unlinked and merged** before the resulting chunk is placed in the **unsorted bin**.
...
- If not `victim->bck->fd == victim` or not`victim->fd == av` (arena):`malloc(): unsorted double linked list corrupted`  - Because the traversal always examines the last chunk, its `fd` must point to the arena structure.
- Because the traversal always examines the last chunk, its
...
```
If this succeeds, return the chunk; otherwise, continue through the allocation path.
...
Security checks verify that the large bin’s two doubly linked orderings have not been corrupted:
...
If the remainder of the selected chunk is large enough to form a chunk, add it to the unsorted bin and update `last_remainder`.

A security check is performed when adding the remainder to the unsorted bin:
...
The remainder of the split chunk is added to the unsorted bin, `last_remainder` is updated, and the same security check is performed:
...
If arena is null or the requested size is too big (and there are mmaps left permitted) use `sysmalloc_mmap` to allocate space and return it.[\[1\]](#references)
...
It starts by obtaining the old top-chunk information and checking the following invariants:
...
If the preceding call returned `MORECORE_FAILURE`, try again using `sysmalloc_mmap_fallback`.
...
Finish the allocation by updating the arena information.
