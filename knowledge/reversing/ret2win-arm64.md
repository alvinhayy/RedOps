...
source_url: https://hacktricks.wiki/binary-exploitation/stack-overflow/ret2win/ret2win-arm64.html
...
This is really an **off-by-2 / 2-byte partial overwrite** of the saved PC on the stack. Instead of overwriting the whole return address we only replace the **last 2 bytes** with `0x06c4`.

A **true off-by-one** only works when the saved `x30` and `win()` differ in the **lowest byte only**. For example, if the saved return ends in `...06a4` and `win()` is `...06c4`, `p8(0xc4)` would be enough. In the binary shown here the low **two** bytes change, so `p16()` is the correct primitive.
...
```
If you do find an `arm64e` slice, check [this page about Mach-O slices and `arm64e`](../../../macos-hardening/macos-security-and-privilege-escalation/macos-files-folders-and-binaries/universal-binaries-and-mach-o-format.html).
...
```
- This produces the offset `72` . Placing the address of`win()` there executes the function and opens a shell when ASLR is disabled.
...
# Replace 0x61616173 with the 4 first bytes from the value of x30
```
- This second binary also produces an offset of `72` . Placing the resolved address of`win()` there executes the function and opens a shell; unlike the first example, this PIE-relative address is calculated from the leaked`leak_anchor()` address.
...
- Current GCC/Clang toolchains support `-mbranch-protection=standard` , which enables the common PAC/BTI hardening profile. For labs, keep using`-mbranch-protection=none` so your saved-`x30` overwrite behaves like a classic ret2win.<sup>[\[1\]](#references)</sup>
- If the binary is compiled with AArch64 Branch Protection, you may see `paciasp` /`autiasp` or`bti c` emitted in function prologues/epilogues. Some hardened entries use`paciasp` /`pacibsp` as the landing instruction instead of a separate`bti c` , so do not grep only for`bti` .<sup>[\[2\]](#references)</sup> In that case:
...
- Returning to an address that is not a valid BTI landing pad may raise a
