...
source_url: https://hacktricks.wiki/binary-exploitation/libc-heap/heap-memory-functions/free.html
...
2. If the chunk was mmapped, unmap it and finish
3. Call `_int_free` :
...
- Check pointer tag
- If the chunk is mmapped, call `munmap` and finish
...
```
-
For a full tcache poisoning walkthrough (and its limits under safe-linking), see: {{#ref}} ../tcache-bin-attack.md {{#endref}}
...
```
This is also useful when reproducing older write-ups that assume a chunk larger than `0x410`/`0x420` will immediately reach the unsorted path.

Related reading within HackTricks:

-
First-fit/unsorted behaviour and overlap tricks: {{#ref}} ../use-after-free/first-fit.md {{#endref}}
-
Double-free primitives and modern checks: {{#ref}} ../double-free.md {{#endref}}
...
[WWW2Exec - __malloc_hook & __free_hook](../../arbitrary-write-2-exec/aw2exec-__malloc_hook.html)
