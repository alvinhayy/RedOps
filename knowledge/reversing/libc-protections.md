...
source_url: https://hacktricks.wiki/binary-exploitation/common-binary-protections-and-bypasses/libc-protections.html
...
glibc `malloc` aligns chunks to `MALLOC_ALIGNMENT`, commonly 8 bytes on 32-bit targets and 16 bytes on 64-bit targets. Therefore, a valid chunk address and size must be a multiple of the target’s alignment; there is no requirement that a 32-bit chunk address specifically end in hexadecimal `8`. Allocator checks reject misaligned pointers before consuming them from the affected bins.[\[1\]](#references)[\[4\]](#references)

### Alignment Security Benefits

The enforcement of chunk alignment in 64-bit systems significantly enhances Malloc’s security by **limiting the placement of fake chunks to only 1 out of every 16 addresses**. This complicates exploitation efforts, especially in scenarios where the user has limited control over input values, making attacks more complex and harder to execute successfully.[\[1\]](#references)

- **Fastbin Attack on `__malloc_hook`**

Alignment checks complicate classic fastbin allocations near `__malloc_hook`: the attacker must find an address where the forged chunk header and returned pointer satisfy the allocator’s alignment and size checks. They do not make every hook-targeting attack impossible by themselves.
...
**Safe-Linking** protects the singly linked `fd`/`next` pointers in fastbins and tcache. It raises the cost of attacks that forge or partially overwrite a freelist pointer without knowing its storage address.[\[1\]](#references)[\[4\]](#references)
...
The shift discards the 12-bit offset within a typical 4 KiB page and mixes the storage location’s page number into the pointer. Its entropy comes from ASLR; it is an encoding and alignment check, not cryptographic protection.[\[4\]](#references)
...
When the exact storage address `L` is known, decoding is the same XOR: `P = New_Ptr XOR (L >> 12)`. If `L` is not known, the relationship between a heap pointer and its storage location can sometimes be recovered iteratively or from another heap leak.[\[1\]](#references)

### Safe-Linking Security Benefits

Pointer mangling aims to **prevent partial and full pointer overwrites in heap** management, a significant enhancement in security. This feature impacts exploit techniques in several ways:[\[1\]](#references)

1. **Prevention of Byte-wise Relative Overwrites** : Previously, attackers could change part of a pointer to**redirect heap chunks to different locations without knowing exact addresses** , a technique evident in the leakless**House of Roman** exploit. With Safe-Linking, such relative overwrites generally require a heap-address disclosure, an algebraic recovery technique, or brute force.
...
With Safe-Linking enabled, a leaked encoded pointer can be decoded directly when the exact address of the field that stores it is known:[\[2\]](#references)[\[4\]](#references)
...
```
This restores the `fd` value and can enable classic tcache/fastbin poisoning. The low 12-bit page offset is **not** what must be brute-forced—the encoding shifts it away. When the storage address is unknown, exploitation instead needs enough information to recover its page number, an iterative demangling method based on heap-layout relationships, or brute force over the remaining unknown ASLR bits.[\[1\]](#references)[\[2\]](#references)
...
For a better explanation of the process [**check the original post from here**](https://maxwelldulin.com/BlogPost?post=5445977088).[\[1\]](#references)
...
Pointer Guard is a glibc mitigation for selected stored code pointers, including pointers used by `atexit()` handlers. On x86-64 glibc, the `PTR_MANGLE` operation XORs a pointer with the thread-local guard commonly addressed at `fs:0x30`, then rotates it left by 17 bits. Other architectures and implementations can use a different operation or guard location.[\[5\]](#references)
...
3. **Exploiting Known Plaintexts:**  - **Identifying Fixed Function Pointers:** By examining glibc source code or initialized function pointer tables (like`__libc_pthread_functions` ), an attacker can find predictable function pointers.
