...
source_url: https://hacktricks.wiki/binary-exploitation/common-binary-protections-and-bypasses/pie/bypassing-canary-and-pie.html
...
To **bypass PIE**, first recover a code address. If the service exposes no direct leak but preserves its randomized layout across crashes, it may be possible to brute-force the saved RBP and RIP byte by byte.

For example, if a binary is protected using both a **canary** and **PIE**, you can start brute-forcing the canary, then the **next** 8 Bytes (x64) will be the saved **RBP** and the **next** 8 Bytes will be the saved **RIP.**[\[1\]](#references)[\[2\]](#references)

This approach assumes that the saved return address points into the main executable, as is often the case when the vulnerable function belongs to that executable.

This technique is especially useful when **each failed probe kills only the current worker and does not rerandomize the parent state** (for example, a `fork()`-per-connection server or a service that respawns workers without `execve()`). If you first need to brute-force the canary in that scenario, check [BF Forked & Threaded Stack Canaries](../stack-canaries/bf-forked-stack-canaries.html).
...
The **reliable** calculation is to subtract that static offset from the leaked runtime address:[\[2\]](#references)
...
- **Introduce a short delay between probes** : sending requests too quickly can leave many workers/processes around, fill memory, or accumulate`TIME_WAIT` sockets, creating false positives unrelated to the guessed byte.<sup>[\[2\]](#references)</sup>
...
- **Validate candidates more than once** : a wrong`RIP` can still return into valid code and print output. Requiring the same candidate to succeed several times, or validating it with a known stop gadget as in[BROP](../../rop-return-oriented-programing/brop-blind-return-oriented-programming.html) , reduces false positives.
