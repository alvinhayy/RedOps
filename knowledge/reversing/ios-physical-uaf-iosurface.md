...
source_url: https://hacktricks.wiki/binary-exploitation/ios-exploiting/ios-physical-uaf-iosurface.html
...
## iOS Exploit Mitigations <sup>\[\\[4\\]\](#references)</sup>
...
This section summarizes Alfie C.G.’s kernel-exploitation writeup. Additional implementations and notes are available in the `kfd` repository.[\[1\]](#references)[\[2\]](#references)
...
3. An entry here represents a smaller region of virtual memory, specifically
4. **L3 Page Table (Level 3)** :
...
5. This is the finest level, where each entry maps a single
...
2. The kernel checks the L1 page table entry corresponding to this virtual address. If it has a
3. **L2 Table** :
...
4. The kernel checks the L2 page table for a more detailed mapping. If this entry points to an
5. **L3 Table** :
  - The kernel looks up the final L3 entry, which points to the **physical address** of the actual memory page.
6. The kernel looks up the final L3 entry, which points to the
...
This means the process can access **pages of kernel memory**, which could contain sensitive data or structures, potentially allowing an attacker to **manipulate kernel memory**.[\[1\]](#references)[\[2\]](#references)
...
4. When they find an IOSurface object on a freed page, they can use it to **read and write kernel memory** .<sup>[\[1\]](#references)</sup><sup>[\[2\]](#references)</sup>

More info about this in [https://github.com/felix-pb/kfd/tree/main/writeups](https://github.com/felix-pb/kfd/tree/main/writeups)[\[1\]](#references)[\[3\]](#references)
...
After achieving control over an IOSurface object in kernel memory (mapped to a freed physical page accessible from userspace), we can use it for **arbitrary kernel read and write operations**.[\[1\]](#references)
...
By overwriting these pointers, we redirect them to arbitrary addresses in kernel memory, enabling read/write capabilities.[\[1\]](#references)
...
With these primitives, the exploit provides controlled **32-bit reads** and **64-bit writes** to kernel memory. Further jailbreak steps could involve more stable read/write primitives, which may require bypassing additional protections (e.g., PPL on newer arm64e devices).[\[1\]](#references)[\[2\]](#references)

## References

- [1] [kfd - kernel exploits for the iOS 15/16 kernel](https://github.com/felix-pb/kfd)
- [2] [A step-by-step guide to writing an iOS kernel exploit](https://alfiecg.uk/2024/09/24/Kernel-exploit.html)
- [3] [felix-pb/kfd](https://github.com/felix-pb/kfd/tree/main/writeups)
- [4] [Apple Platform Security - Apple SoC security](https://support.apple.com/guide/security/apple-soc-security-sec87716a080/web)
