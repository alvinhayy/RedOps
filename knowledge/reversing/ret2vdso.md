...
source_url: https://hacktricks.wiki/binary-exploitation/rop-return-oriented-programing/ret2vdso.html
...
The vDSO base address is passed in the auxiliary vector as `AT_SYSINFO_EHDR`, so if you can read `/proc/<pid>/auxv` (or call `getauxval` in a helper process), you can recover the base without relying on a memory leak. See [Auxiliary Vector (auxv) and vDSO](../basic-stack-binary-exploitation-methodology/elf-tricks.html) for practical ways to obtain it.

Once you have the base, treat the vDSO like a normal ELF DSO (`linux-vdso.so.1`): dump the mapping and use `readelf -Ws`/`objdump -d` (or the kernel reference parser `tools/testing/selftests/vDSO/parse_vdso.c`) to resolve exported symbols and look for gadgets. On x86 32-bit the vDSO commonly exports `__kernel_vsyscall`, `__kernel_sigreturn`, and `__kernel_rt_sigreturn`; on x86_64 typical exports include `__vdso_clock_gettime`, `__vdso_gettimeofday`, and `__vdso_time`. Because the vDSO uses symbol versioning, match the expected version when resolving symbols.[\[1\]](#references)[\[2\]](#references)

The *Maze of Mist* example shows how to dump the vDSO mapping and transfer it to the host:[\[3\]](#references)
...
On affected 32-bit compatibility configurations, `CONFIG_COMPAT_VDSO` could map the vDSO at a fixed address, making these gadgets usable without first defeating ASLR. Verify the target kernel and mapping rather than assuming current kernels behave this way.[\[4\]](#references)
...
- [2] [Linux kernel vDSO ABI documentation](https://www.kernel.org/doc/Documentation/ABI/stable/vdso)
- [3] [Maze of Mist - HTB Cyber Apocalypse](https://7rocky.github.io/en/ctf/other/htb-cyber-apocalypse/maze-of-mist/)
