...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/cgi.html
...
**CGI is an interface, not a language**: real targets may expose legacy **Perl**, **sh**, **Python**, or compiled programs behind `*.cgi`. CGI defines how the server passes request metadata and body data to those programs.[\[7\]](#references)
...
For authorized CGI enumeration, Nikto’s `-C all` option tests every CGI directory.[\[10\]](#references)
...
- If you suspect Apache-specific CGI weirdness (source disclosure via absolute paths, local redirects, handler confusion), check [Apache](apache.html) .
- If the stack is really **FastCGI/PHP-FPM** instead of classic CGI, check[9000 - Pentesting FastCGI](../9000-pentesting-fastcgi.html) .

## **ShellShock**

**ShellShock**

**Shellshock** (CVE-2014-6271 and related flaws) affected Bash’s handling of function definitions imported through environment variables. CGI maps request headers into environment variables, so a vulnerable Bash-backed CGI script could execute attacker-supplied trailing commands.[\[8\]](#references)

Exploiting this behavior may cause the **page to return an error**.
...
According to RFC 3875, extra path data after the CGI script path is exposed to the application as **`PATH_INFO`**, and a server may derive **`PATH_TRANSLATED`** from it.<sup>[\[7\]](#references)</sup> In practice, many CGI handlers either:
...
Many embedded web UIs multiplex dozens of privileged actions behind a single CGI endpoint (for example, `/cgi-bin/cstecgi.cgi`) and use a selector parameter such as `topicurl=<handler>` to route the request to an internal function.[\[2\]](#references)
...
To exploit this vulnerability, access a PHP file without a conventional query parameter (especially without `=`).
