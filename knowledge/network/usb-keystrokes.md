...
source_url: https://hacktricks.wiki/generic-methodologies-and-resources/basic-forensic-methodology/pcap-inspection/usb-keystrokes.html
...
The standard boot keyboard input report is structured as follows.[\[8\]](#references)[\[9\]](#references)
...
On busy captures, identify the HID keyboard before dumping any reports. A reliable starting point is the interface descriptor response:[\[3\]](#references)[\[8\]](#references)

```
usb.transfer_type == 0x02 && usb.endpoint_address.direction == 1 && usb.bDescriptorType == 4 && usb.bInterfaceClass == 3
```
The HID class defines these interface values:[\[8\]](#references)
...
The classic extraction pattern—dump `usb.capdata`, drop idle reports, and map usage IDs—appears in the original 2017 analysis and its walkthrough.[\[1\]](#references)[\[2\]](#references)

The `ctf-usb-keyboard-parser` repository automates the classic tshark + sed pipeline:[\[5\]](#references)

```
tshark -r ./usb.pcap -Y 'usb.capdata && usb.data_len == 8' -T fields -e usb.capdata | sed 's/../:&/g2' > keystrokes.txt
python3 usbkeyboard.py ./keystrokes.txt
```
On newer captures, prefer Wireshark’s decoded `usbhid.data` field and fall back to `usb.capdata`; write one payload per report to a per-device file:[\[7\]](#references)[\[10\]](#references)[\[11\]](#references)
...
```
Those per-device files can be fed to a decoder after normalizing the hex format it expects. If the capture came from BLE keyboards tunneled over GATT, filter on `btatt.value && frame.len == 20` and dump the hex payloads before decoding.[\[7\]](#references)

### When the report is not the classic 8-byte boot report

A non-boot interface or a report ID can change the payload layout, so do not assume every keyboard report matches `modifier,reserved,key1..key6`.[\[8\]](#references)[\[11\]](#references)
...
USB boot keyboards send reports at the idle rate even when there is no new key event, so captures may contain repeated reports before the release event. A practical decoder should:[\[3\]](#references)[\[8\]](#references)
