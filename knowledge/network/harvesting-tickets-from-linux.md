...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-kerberos-88/harvesting-tickets-from-linux.html
...
Linux Kerberos implementations support several credential-cache types, including **files**, **directories**, **kernel keyrings**, **KCM**, and **process memory**. The `default_ccache_name` setting in `/etc/krb5.conf` indicates the configured default; an implementation or distribution may choose a different default when the setting is absent.[\[5\]](#references)
...
- `MEMORY:unique_id` for process-local caches created by libraries (`gssproxy` ,`sshd` , etc.).<sup>[\[2\]](#references)[\[5\]](#references)</sup>
...
```
The combination of `klist`, `keyctl`, and `/proc` inspection quickly reveals whether credentials live in files, keyrings, or KCM so you can pick the right dumping technique.[\[4\]](#references)[\[5\]](#references)

### Extracting Credentials

The 2017 paper, [**Kerberos Credential Thievery (GNU/Linux)**](https://web.archive.org/web/20210721113019/https://www.delaat.net/rp/2016-2017/p97/report.pdf), outlines methods for extracting credentials from keyrings and processes, emphasizing the Linux kernel’s keyring mechanism for managing and storing keys.[\[4\]](#references)
...
$ keyctl pipe <serial> > /tmp/key_blob     # extract one key payload
```
The extracted key payload is not necessarily a complete MIT ccache: KEYRING collections store a cache as several components. Reconstruct the components with a tool such as `tickey`/`hercules.sh`, validate the resulting cache with `klist`, and then convert it with `kerbtool` or Impacket’s `ticketConverter.py` when `.kirbi` interoperability is required. Repeat the extraction for every relevant serial and principal.[\[1\]](#references)[\[3\]](#references)[\[4\]](#references)[\[6\]](#references)
...
SSSD’s Kerberos Cache Manager (kcm) proxies credential storage through `/var/run/kcm/kcmsock` (or `/run/.heim_org.h5l.kcm-socket`) and persists encrypted blobs inside `/var/lib/sss/secrets/` together with `.secrets.mkey`.<sup>[\[2\]](#references)</sup> Attack flow:
...
Building on the principles of the **hercules.sh script**, the [**tickey**](https://github.com/TarlogicSecurity/tickey) tool is specifically designed for extracting tickets from keyrings, executed via `/tmp/tickey -i`. It enumerates kernel keyrings, reconstructs the serialized ccaches, and writes MIT-compatible cache files you can immediately feed to `klist`, `impacket-*`, or `kerberoast` tooling.[\[1\]](#references)

#### Kerbtool

[**kerbtool**](https://github.com/jfjallid/kerbtool) is a modern Go utility that runs natively on Linux and can parse, convert, and request Kerberos tickets. Two handy use cases when harvesting from Linux boxes:[\[3\]](#references)
...
- [5] [MIT Kerberos - Credential cache types](https://web.mit.edu/kerberos/krb5-latest/doc/basic/ccache_def.html)
- [6] [Fortra Impacket - `ticketConverter.py`](https://github.com/fortra/impacket/blob/master/examples/ticketConverter.py)
