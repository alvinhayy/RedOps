...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/roundcube.html
...
Affected versions (per vendor/NVD):[\[1\]](#references)[\[2\]](#references)
...
- The _from parameter in program/actions/settings/upload.php is not validated, enabling injection of attacker‑controlled data that Roundcube later unserializes, leading to gadget chain execution and remote code execution in the web context (post‑auth).<sup>[\[3\]](#references)</sup>

Quick exploitation

- Requirements: valid Roundcube credentials and a reachable UI URL (e.g., http://mail.target.tld)
- Public PoC automates session handling, gadget crafting and upload flow<sup>[\[4\]](#references)</sup>
...
Roundcube ships a CLI that uses the same rcmail->decrypt() logic and the configured des_key:[\[5\]](#references)
