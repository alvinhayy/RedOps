...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/perl-tricks.html
...
- `IPC::Open3` with one command scalar can inherit Perl’s single-string shell handling. Pass the program and arguments as separate list elements when no shell is required.<sup>[\[5\]](#references)</sup>
- An explicit shell such as `system('/bin/sh', '-c', $attacker_string)` , which is dangerous even though it uses list form.

Minimal vulnerable shape observed in the wild:[\[1\]](#references)
...
```
Key reachability considerations in `mod_perl`:[\[1\]](#references)
...
### Exploitation workflow <sup>\[\\[1\\]\](#references)</sup>
...
- Do not build shell strings. Prefer argument-vector execution: `system('/usr/bin/curl', '--silent', '--', $safe_url)` .<sup>[\[4\]](#references)</sup>
...
Real-world case: Dell UnityVSA pre-auth RCE (CVE-2025-36604)[\[1\]](#references)[\[2\]](#references)[\[3\]](#references)
