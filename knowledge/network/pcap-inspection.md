...
source_url: https://hacktricks.wiki/generic-methodologies-and-resources/basic-forensic-methodology/pcap-inspection/index.html
...
Pcap analysis from the browser.[\[5\]](#references)

### Xplico Framework

[**Xplico**](https://github.com/xplico/xplico) is a Unix-like network-forensics tool that decodes PCAP files and can extract email over POP/IMAP/SMTP, HTTP contents, SIP VoIP calls, FTP data, and TFTP data.[\[6\]](#references)
...
Like Xplico, [**NetworkMiner**](https://www.netresec.com/?page=NetworkMiner) parses PCAP traffic to extract artifacts such as files, images, email, and passwords, and it aggregates host information; its free edition is primarily for Windows.[\[7\]](#references)

### NetWitness Investigator

You can download **NetWitness Investigator from here****(It works in Windows)**.

The vendor describes the freeware as an interactive network-session analysis tool for malicious-activity triage and currently presents access through a contact form.[\[8\]](#references)

### [BruteShark](https://github.com/odedshimon/BruteShark)

BruteShark’s documented modules can parse credentials from HTTP, FTP, Telnet, IMAP, and SMTP, export Kerberos, NTLM, CRAM-MD5, and HTTP-Digest authentication hashes for Hashcat, map network nodes and users, extract DNS queries, rebuild TCP/UDP sessions, and carve files.[\[9\]](#references)

### Capinfos

Wireshark’s `capinfos` prints a long report for a capture file by default.[\[10\]](#references)
...
[File/Data Carving & Recovery Tools](../partitions-file-systems-carving/file-data-carving-recovery-tools.html)

### Capturing credentials

You can use [PCredz](https://github.com/lgandx/PCredz) to parse credentials from a stored PCAP file or a live interface.[\[12\]](#references)
...
Suricata’s `-r` option replays a PCAP in offline mode; in this example, `-k none` disables checksum checks, `-v` increases logging, and `-l` selects the log directory.[\[13\]](#references)
...
[**YaraPCAP**](https://github.com/kevthehermit/YaraPcap) processes HTTP streams from PCAP files, optionally decompresses gzip streams, scans extracted files with YARA, writes `report.txt`, and can save matching files to a directory.[\[14\]](#references)
...
[Zeek](https://docs.zeek.org/en/master/about.html) is a passive, open-source network traffic analyzer used as a Network Security Monitor (NSM) and for broader traffic analysis, including performance measurement and troubleshooting.[\[15\]](#references)

Zeek generates structured logs rather than PCAP files, so use log-analysis tools such as `zeek-cut` to inspect those logs.[\[15\]](#references)[\[16\]](#references)

### Connections Info

The examples below use `zeek-cut` to select named fields from TSV logs, then standard Unix tools to rank and count connections; RITA can also ingest Zeek logs for long-connection, beaconing, and DNS-tunneling analysis.[\[16\]](#references)[\[17\]](#references)
