...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/drupal/index.html
...
```
- **Node** : Drupal content entities of the node type represent items such as articles and pages. Canonical routes commonly use`/node/<nodeid>` , although aliases can hide that route.<sup>[\[2\]](#references)</sup>
...
Drupal installs define the following broad roles by default; permissions and any additional roles are site-configurable:[\[3\]](#references)
...
Exposed configuration-synchronization files may reveal enabled modules and service settings.[\[5\]](#references)
...
```
`droopescan` fingerprints CMS versions, plug-ins, and themes; validate its findings manually because content filtering and customized paths can cause false results.[\[4\]](#references)
...
- **Drupalwned** is a Drupal exploitation script whose documented chains can elevate XSS into RCE or other critical impact.<sup>[\[1\]](#references)[\[6\]](#references)</sup> Its documented targets include Drupal 7.x through 10.x, and it supports:
