...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/laravel.html
...
Read information about this here: [https://stitcher.io/blog/unsafe-sql-functions-in-laravel](https://stitcher.io/blog/unsafe-sql-functions-in-laravel)[\[13\]](#references)
...
Therefore **any attacker that knows the 32-byte secret `APP_KEY` can craft an encrypted PHP serialized object and gain RCE via magic methods (`__wakeup`, `__destruct`, …)**.[\[1\]](#references)[\[2\]](#references)
...
[laravel-crypto-killer](https://github.com/synacktiv/laravel-crypto-killer) automates the process and adds a convenient **brute-force** mode:[\[3\]](#references)
...
The exploitation workflow is always:[\[1\]](#references)[\[2\]](#references)

1. Obtain or brute-force the 32-byte `APP_KEY` .
2. Build a gadget chain with **PHPGGC** (for example`Laravel/RCE13` ,`Laravel/RCE9` or`Laravel/RCE15` ).<sup>[\[4\]](#references)</sup>
...
Below are concise one-liners demonstrating the full attack path for each real-world CVE mentioned above:[\[1\]](#references)[\[2\]](#references)
...
Because every fresh Laravel response sets at least one encrypted cookie (`XSRF-TOKEN` and usually `laravel_session`), **public internet scanners (Shodan, Censys, …) leak millions of ciphertexts** that can be attacked offline.[\[1\]](#references)

Key findings of the research published by Synacktiv (2024-2025):[\[1\]](#references)[\[2\]](#references)
...
The private Go tool **nounours** pushes AES-CBC/GCM brute-force throughput to about 1.5 billion attempts per second, reducing full-dataset cracking to under two minutes.[\[1\]](#references)[\[2\]](#references)

## CVE-2024-52301 – HTTP argv/env override → auth bypass

When PHP’s `register_argc_argv=On` (typical on many distros), PHP exposes an `argv` array for HTTP requests derived from the query string. Recent Laravel versions parsed these “CLI-like” args and honored `--env=<value>` at runtime. This allows flipping the framework environment for the current HTTP request just by appending it to any URL:[\[6\]](#references)[\[7\]](#references)

-
Quick check:
  - Visit `https://target/?--env=local` or any string and look for environment-dependent changes (debug banners, footers, verbose errors). If the string is reflected, the override is working.<sup>[\[8\]](#references)</sup>
- Visit
-
Impact example (business logic trusting a special env):
  - If the app contains branches like `if (app()->environment('preprod')) { /* bypass auth */ }` , you can authenticate without valid creds by sending the login POST to:
    - `POST /login?--env=preprod`<sup>[\[8\]](#references)</sup>
...
Minimal exploitation flow (Burp):[\[8\]](#references)
...
- Affected: Laravel 11.9.0–11.35.1 with `APP_DEBUG=true` (either globally or forced via misconfigured env overrides like CVE-2024-52301).<sup>[\[10\]](#references)</sup>
- Primitive: every uncaught exception rendered by Whoops echoes parts of the request/route **without HTML encoding** , so injecting`<img src>` /`<script>` in a route or request parameter yields stored-on-response XSS before authentication.<sup>[\[10\]](#references)</sup>
...
## Decrypt/encrypt cookies helper (Python)
...
Details of the deserialization vulnerability are available in the [WithSecure write-up](https://labs.withsecure.com/archive/laravel-cookie-forgery-decryption-and-rce/).[\[5\]](#references)
...
This Ignition debug-mode deserialization-to-RCE chain is documented with exploit material in the Ambionics repository.[\[14\]](#references)
