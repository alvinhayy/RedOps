...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/aem-adobe-experience-cloud.html
...
- `</script>` comment at the bottom of HTML.
...
| `/adminui/debug` | **AEM Forms on JEE** Struts dev-mode OGNL evaluator | On misconfigured Forms installs (CVE-2025-54253) this endpoint executes unauthenticated OGNL → RCE. <sup>[\[2\]](#references)[\[3\]](#references)</sup> |
...
7. **AEM Forms Struts devMode left enabled** –`/adminui/debug?expression=` evaluates OGNL without auth (CVE-2025-54253) leading to unauth RCE; paired XXE in Forms submission (CVE-2025-54254) allows file read.<sup>[\[2\]](#references)[\[3\]](#references)</sup>

## [4. Recent vulnerabilities (service-pack cadence)](#4-recent-vulnerabilities-service-pack-cadence)

| Quarter | CVE / Bulletin | Affected | Impact |
|---|---|---|---|
| Dec 2025 | **APSB25-115** , CVE-2025-64537/64539 | 6.5.24 & earlier, Cloud 2025.12 | Multiple critical/stored XSS → code execution via author UI. <sup>[\[1\]](#references)</sup> |
| Sep 2025 | APSB25-90 | 6.5.23 & earlier | Security feature bypass chain (Dispatcher auth checker) – upgrade to 6.5.24/Cloud 2025.12. |
| Aug 2025 | **CVE-2025-54253 / 54254** (AEM Forms JEE) | Forms 6.5.23.0 and earlier | DevMode OGNL RCE + XXE file read, unauthenticated. <sup>[\[2\]](#references)[\[3\]](#references)</sup> |
...
## [5. Exploitation snippets](#5-exploitation-snippets)

### [5.1 RCE via dispatcher bypass + JSP upload](#51-rce-via-dispatcher-bypass--jsp-upload)
...
### [5.2 SSRF to RCE (historical < 6.3)](#52-ssrf-to-rce-historical--63)

`/libs/mcm/salesforce/customer.html;%0aa.css?checkType=authorize&authorization_url=http://127.0.0.1:4502/system/console``aem_ssrf2rce.py` from **aem-hacker** automates the full chain.

### [5.3 OGNL RCE on AEM Forms JEE (CVE-2025-54253)](#53-ognl-rce-on-aem-forms-jee-cve-2025-54253)
...
```
If vulnerable, the HTTP body contains the command output.[\[2\]](#references)

### [5.4 QueryBuilder hash disclosure (encoded slash bypass)](#54-querybuilder-hash-disclosure-encoded-slash-bypass)
...
- [1] [Adobe Security Bulletin APSB25-115 – Security updates for Adobe Experience Manager (Dec 9, 2025)](https://helpx.adobe.com/security/products/experience-manager/apsb25-115.html)
- [2] [Struts Devmode in 2025? Critical Pre-Auth Vulnerabilities in Adobe Experience Manager Forms](https://slcyber.io/assetnote-security-research-center/struts-devmode-in-2025-critical-pre-auth-vulnerabilities-in-adobe-experience-manager-forms)
- [3] [BleepingComputer – Adobe issues emergency fixes for AEM Forms zero-days (Aug 5, 2025)](https://www.bleepingcomputer.com/news/security/adobe-issues-emergency-fixes-for-aem-forms-zero-days-after-pocs-released/)
