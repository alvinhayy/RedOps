...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/wsgi.html
...
[SSRF (Server Side Request Forgery)](../../pentesting-web/ssrf-server-side-request-forgery/index.html)
...
#### [`UWSGI_SCRIPT` - Script Loading](#uwsgi_script---script-loading)
...
#### [`UWSGI_SETENV` - Environment Variable Manipulation](#uwsgi_setenv---environment-variable-manipulation)
...
#### [`UWSGI_CHDIR` - Directory Change](#uwsgi_chdir---directory-change)
...
This is possible because many deployments use a non-HTTP uwsgi socket internally; the reverse proxy (nginx/Apache) translates client HTTP into the uwsgi param bag. With SSRF+gopher you can directly craft the uwsgi binary packet and set dangerous variables like `UWSGI_FILE`.[\[2\]](#references)
...
For standard requests `modifier1` is 0. The body contains uwsgi params such as `SERVER_PROTOCOL`, `REQUEST_METHOD`, `PATH_INFO`, `UWSGI_FILE`, etc. See the official protocol spec for full details.[\[4\]](#references)
...
- [1] [uWSGI Magic Variables Documentation](https://uwsgi-docs.readthedocs.io/en/latest/Vars.html)
- [2] [IOI SaveData CTF Writeup](https://bugculture.io/writeups/web/ioi-savedata)
