...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/jira.html
...
- Affects Confluence Data Center/Server 8.0.x–8.5.3 & 8.4.5. Vulnerable Velocity template `text-inline.vm` allows OGNL evaluation without authentication.<sup>[\[1\]](#references)</sup>
...
- Publicly reachable Confluence Data Center/Server 8.0.0–8.5.1 allows flipping `setupComplete` and re‑running`/setup/setupadministrator.action` to create a new admin account.<sup>[\[2\]](#references)</sup>
...
- A Confluence administrator can upload a crafted language definition through **Configure Code Macro** ; affected versions pass tainted content to the Rhino script engine, leading to RCE.<sup>[\[6\]](#references)</sup>
...
As indicated in this **blog**<sup>[\[4\]](#references)</sup>, in the documentation about [Plugin modules ↗](https://developer.atlassian.com/server/framework/atlassian-sdk/plugin-modules/) it’s possible to check the different types of plugins, like:
...
[**This post**](https://cyllective.com/blog/posts/atlassian-malicious-plugin) describes different (malicious) actions that could perform a malicious Jira plugin.<sup>[\[5\]](#references)</sup> You can find [**code example in this repo**](https://github.com/cyllective/malfluence).

These are some of the actions a malicious plugin could perform:

- **Hiding Plugins from Admins** : A malicious plugin can inject front-end JavaScript that hides the plugin from administrators.
- **Exfiltrating Attachments and Pages** : A plugin can access and exfiltrate Confluence data available to its execution context.
- **Stealing Session Tokens** : A plugin can add an endpoint that echoes request headers and JavaScript that sends session data to it.
- **Command Execution** : A malicious server-side plugin can execute operating-system commands.
