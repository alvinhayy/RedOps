...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/telerik-ui-aspnet-ajax-unsafe-reflection-webresource-axd.html
...
- Telerik UI for ASP.NET AJAX versions 2011.2.712 through 2025.1.218 (inclusive) are vulnerable.<sup>[\[1\]](#references)</sup>
- Fixed in 2025.1.416 (released 2025-04-29). Patch immediately or remove/lock down the handler.<sup>[\[1\]](#references)</sup>
...
- If `type=rau` is reachable, use the classic major-version brute force from older RAU tooling to recover the exact`Telerik.Web.UI` assembly version.
- Compare the recovered version against the vulnerable range (`2011.2.712` to`2025.1.218` ) and the fixed build (`2025.1.416+` ).
- Treat `type=rau` absence as inconclusive.`iec` may still be exposed even when`rau` is disabled or filtered.
...
The Image Editor cache download flow constructs an instance of a type supplied in prtype and only later casts it to ICacheImageProvider and validates the download key. The constructor has already run when validation fails.[\[2\]](#references)
...
Unsafe constructor execution unlocks many target‑specific gadgets and chains.<sup>[\[2\]](#references)</sup> Hunt for:
...
- Some types delete fixed-path files in finalizers. Combined with link-following or predictable paths this can enable local privilege escalation in certain environments.<sup>[\[4\]](#references)</sup>

## Example pre‑auth RCE chain (Sitecore XP)

- Step 1 – Pre‑auth: Trigger a type whose static/instance ctor registers an insecure AssemblyResolve handler (e.g., Sitecore’s FolderControlSource in ControlFactory).<sup>[\[5\]](#references)</sup>
...
[IIS - Internet Information Services](iis-internet-information-services.html)
