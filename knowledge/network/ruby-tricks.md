...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/ruby-tricks.html
...
As explained in [this article](https://www.offsec.com/blog/cve-2024-46986/), uploading a `.rb` file into sensitive directories such as `config/initializers/` can lead to remote code execution (RCE) in Ruby on Rails applications.[\[14\]](#references)
...
When an application uses Active Storage with `image_processing` + `mini_magick`, and passes untrusted parameters to image transformation methods, Rails versions prior to 7.1.5.2 / 7.2.2.2 / 8.0.2.1 could allow command injection because some transformation methods were mistakenly allowed by default.[\[1\]](#references)
...
## [Rack::Static LFI / path traversal (CVE-2025-27610)](#rackstatic-lfi--path-traversal-cve-2025-27610)

If the target stack uses Rack middleware directly or via frameworks, versions of `rack` prior to 2.2.13, 3.0.14, and 3.1.12 allow Local File Inclusion via `Rack::Static` when `:root` is unset/misconfigured. Encoded traversal in `PATH_INFO` can expose files under the process working directory or an unexpected root.[\[5\]](#references)
...
`Content-Type` parsing ReDoS (CVE-2024-25126)
Rack versions before `3.0.9.1` and `2.2.8.1` spent quadratic time splitting crafted `Content-Type` headers. A header containing tens of thousands of leading spaces can occupy a Puma/Unicorn worker and cause denial of service or request-queue starvation.[\[10\]](#references)
...
The REXML gem < 3.3.9 (Ruby 3.1 and earlier) catastrophically backtracks when parsing hex numeric character references containing long digit runs (e.g., `�x41;`). Any XML processed by REXML or libraries that wrap it (SOAP/XML API clients, SAML, SVG uploads) can be abused for CPU exhaustion.<sup>[[15]](#references)</sup>
...
## [CGI cookie parsing / escapeElement ReDoS (CVE-2025-27219 & CVE-2025-27220)](#cgi-cookie-parsing--escapeelement-redos-cve-2025-27219--cve-2025-27220)

Apps using the `cgi` gem (default in many Rack stacks) can be frozen with a single malicious header:[\[11\]](#references)
...
## [Basecamp `google_sign_in` open redirect chain (CVE-2025-57821)](#basecamp-google_sign_in-open-redirect-chain-cve-2025-57821)

`google_sign_in` open redirect chain (CVE-2025-57821)
The `google_sign_in` gem before 1.3.0 performed an incomplete same-origin check on its persisted post-authentication redirect URL. A malformed URL could pass that check and redirect the user to another origin, potentially exposing authentication information such as a token.[\[16\]](#references)
...
## [Rails nested mass assignment via `permit!` on a sub-object](#rails-nested-mass-assignment-via-permit-on-a-sub-object)

`permit!` on a sub-object
...
This is still **mass assignment**, but in Rails apps it often hides in “safe-looking” strong-parameter code because the dangerous call is `permit!` on a nested object rather than direct `params[:user]` binding.[\[2\]](#references)[\[3\]](#references)

## [Camaleon CMS admin panel → MinIO / S3 pivot](#camaleon-cms-admin-panel--minio--s3-pivot)

If Rails/CMS admin access exposes **filesystem/storage settings**, treat it as a credential-recovery surface.<sup>[\[2\]](#references)</sup> In Camaleon deployments using S3-compatible storage, the admin UI may reveal:
...
## [Camaleon private-media download path traversal on S3/AWS backends](#camaleon-private-media-download-path-traversal-on-s3aws-backends)

Camaleon’s private-media download flow has had traversal issues where the controller prepends a fixed prefix and forwards attacker-controlled input to the uploader backend:[\[2\]](#references)[\[4\]](#references)
...
## [Forging/decrypting Rails cookies when `secret_key_base` is leaked](#forgingdecrypting-rails-cookies-when-secret_key_base-is-leaked)

`secret_key_base` is leaked
Rails encrypts and signs cookies using keys derived from `secret_key_base`. If that value leaks (e.g., in a repo, logs, or misconfigured credentials), you can usually decrypt, modify, and re-encrypt cookies. This often leads to authz bypass if the app stores roles, user IDs, or feature flags in cookies.

If you only have an authenticated file-read primitive, try to recover `config/master.key` first and then decrypt `config/credentials.yml.enc` to extract `secret_key_base`. In many apps this is the shortest path from LFI/path traversal to cookie decryption and session forgery.[\[2\]](#references)
...
```
## [Forging Rails signed IDs / SGIDs when `secret_key_base` is leaked](#forging-rails-signed-ids--sgids-when-secret_key_base-is-leaked)
...
- `ActiveRecord::SignedId`**does not expire by default** . Signed Global IDs in Rails**expire after 1 month by default** unless the app overrides`Rails.application.config.global_id.expires_in` .<sup>[\[13\]](#references)</sup>
...
```
## [See also (Ruby/Rails-specific vulns)](#see-also-rubyrails-specific-vulns)
...
## [Log Injection → RCE via Ruby `load` and `Pathname.cleanpath` smuggling](#log-injection--rce-via-ruby-load-and-pathnamecleanpath-smuggling)

`load` and `Pathname.cleanpath` smuggling
...
You can often achieve remote code execution by poisoning the log and then coercing the app to `load` the log file. Key primitives:[\[6\]](#references)

- Ruby `load` evaluates the target file content as Ruby regardless of file extension. Any readable text file whose contents parse as Ruby will be executed.<sup>[\[9\]](#references)</sup>
- `Pathname#cleanpath` collapses`.` and`..` segments without hitting the filesystem, enabling path smuggling: attacker-controlled junk can be prepended for logging while the cleaned path still resolves to the intended file to execute (e.g.,`../logs/error.log` ).<sup>[\[7\]](#references)</sup>

### [Real-world poison source: `Rack::CommonLogger` (CVE-2025-25184)](#real-world-poison-source-rackcommonlogger-cve-2025-25184)

`Rack::CommonLogger` (CVE-2025-25184)
...
### [Minimal vulnerable pattern](#minimal-vulnerable-pattern)
...
```
### [Why the log can contain valid Ruby](#why-the-log-can-contain-valid-ruby)

`Logger` writes prefix lines like:[\[8\]](#references)
...
```
### [Smuggling a single string that both logs code and resolves to the log path](#smuggling-a-single-string-that-both-logs-code-and-resolves-to-the-log-path)
...
### [End-to-end exploitation](#end-to-end-exploitation)
