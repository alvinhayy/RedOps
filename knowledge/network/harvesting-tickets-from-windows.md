...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-kerberos-88/harvesting-tickets-from-windows.html
...
klist sessions
```
`klist` doesn’t export tickets, but it quickly confirms whether the current logon session already has a **TGT**, which **service tickets** are cached, and whether it is worth escalating before touching LSASS.[\[1\]](#references)

### Mimikatz

Mimikatz is still the fastest option when you can read LSASS, and it is also useful against an offline **LSASS minidump**.[\[1\]](#references)
...
sekurlsa::tickets /export
```
`sekurlsa::ekeys` is especially useful in modern **AES-first** domains because it returns Kerberos key material you can reuse in [over-pass-the-hash / pass-the-key](../../windows-hardening/active-directory-methodology/over-pass-the-hash-pass-the-key.html) workflows even when full ticket extraction is inconvenient.

### Rubeus

Rubeus is usually the best first choice because it lets you **triage before dumping** and can work without elevation for some actions.[\[2\]](#references)
...
```
`createnetonly` prints the new LUID when it spawns the sacrificial session; inject into that session to keep your original TGT untouched. Ticket injection/replay and cross-platform conversion are covered in [Pass the Ticket](../../windows-hardening/active-directory-methodology/pass-the-ticket.html).
...
On recent Windows 11 / Server 2025 estates, **LSASS protected process** and **Credential Guard** make old live-LSASS ticket dumping much less predictable. In practice:[\[3\]](#references)
...
- if you need the protection internals and common bypass considerations, read [Windows credentials protections](../../windows-hardening/stealing-credentials/credentials-protections.html) .
...
- [3] [Microsoft Learn — Credential Guard overview](https://learn.microsoft.com/en-us/windows/security/identity-protection/credential-guard/)
