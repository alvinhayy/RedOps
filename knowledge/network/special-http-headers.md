...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/special-http-headers.html
...
[HTTP Request Smuggling / HTTP Desync Attack](../../pentesting-web/http-request-smuggling/index.html)

## The Expect header

An HTTP/1.1 client can send `Expect: 100-continue`; the server may answer with `HTTP/1.1 100 Continue` before the client sends the request body. Differences in how frontends and backends handle the expectation and body can expose desynchronization bugs.[\[8\]](#references)[\[10\]](#references)

Interesting observed results of `Expect: 100-continue` testing include:[\[10\]](#references)
...
[Cache Poisoning and Cache Deception](../../pentesting-web/cache-deception/index.html)

**Browser and legacy cache headers**:[\[9\]](#references)
...
- **`If-Match`** requires a current entity-tag match, while**`If-None-Match`** requires no match. For`GET` /`HEAD` , a failed`If-None-Match` condition normally returns`304` ; for other methods it returns`412` .<sup>[\[8\]](#references)</sup>  - Entity-tag generation is implementation-specific. For example, `W/"37-eL2g8DEyqntYlaLp5XLInBWsjWI"` is a weak validator from a particular framework; its syntax alone does not prove that the value is SHA-1 or that`37` represents a byte count.
...
- **`Range`** : Requests part of a representation. For example,`Range: bytes=80-100` asks for bytes 80 through 100 and can produce`206 Partial Content` . Removing`Accept-Encoding` often makes byte offsets easier to reason about.<sup>[\[8\]](#references)</sup>  - If an attacker can inject a `Range` request header, a partial response may isolate reflected JavaScript or other bytes that were harmless in the complete representation. Exploitability depends on browser behavior, content type, and intermediary caching.
...
This pattern was useful in SolarWinds **Serv-U** (`<= 15.5.4.108`), where `POST` + body + `Content-Encoding: deflate` reached an unsafe in-memory deflate decompressor and reliably crashed the process, while the hotfix added a generic `415` gate for requests carrying a body plus any non-empty `Content-Encoding` header.[\[6\]](#references)[\[7\]](#references)
...
```
This means the file named “filename.jpg” is intended to be downloaded and saved.[\[2\]](#references)
...
This header prevents MIME type sniffing, a practice that could lead to XSS vulnerabilities. It ensures that browsers respect the MIME types specified by the server.[\[3\]](#references)
...
To combat clickjacking, this header restricts how documents can be embedded in `<frame>`, `<iframe>`, `<embed>`, or `<object>` tags, recommending all documents to specify their embedding permissions explicitly.[\[3\]](#references)
...
CORP is crucial for specifying which resources can be loaded by websites, mitigating cross-site leaks. CORS, on the other hand, allows for a more flexible cross-origin resource sharing mechanism, relaxing the same-origin policy under certain conditions.[\[3\]](#references)
...
COEP and COOP are essential for enabling cross-origin isolation, significantly reducing the risk of Spectre-like attacks. They control the loading of cross-origin resources and the interaction with cross-origin windows, respectively.[\[3\]](#references)
...
Lastly, HSTS is a security feature that forces browsers to communicate with servers only over secure HTTPS connections, thereby enhancing privacy and security.[\[3\]](#references)
...
HTTP field names are **case-insensitive** (RFC 9110 §5.1). Nevertheless, custom middleware, security filters, or business logic sometimes compare the literal header name without normalizing its case. If a filter is case-sensitive but the downstream consumer is compliant and case-insensitive, an attacker may bypass the filter with different capitalization.[\[1\]](#references)[\[8\]](#references)
...
- [1] [CVE-2025-27636 – RCE in Apache Camel via header casing bypass (OffSec blog)](https://www.offsec.com/blog/cve-2025-27636/)
- [2] [MDN Web Docs - Content-Disposition](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Disposition)
- [3] [MDN Web Docs - HTTP headers reference](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers)
- [4] [web.dev - Security headers quick reference](https://web.dev/security-headers/)
