...
source_url: https://hacktricks.wiki/network-services-pentesting/pentesting-web/tomcat/index.html
...
Probe for the standard **`/manager`** and **`/host-manager`** applications, while allowing for nonstandard context paths or removal. Modern default configurations do not grant remote management access merely because the application exists.[\[2\]](#references)[\[5\]](#references)
...
In `mod_jk` versions before 1.2.23, CVE-2007-1860 allowed crafted, possibly double-encoded traversal through a prefix `JkMount` to reach protected pages.[\[4\]](#references)
...
In some [**vulnerable reverse-proxy mappings**](https://www.acunetix.com/vulnerabilities/web/tomcat-path-traversal-via-reverse-proxy-mapping/), path-parameter and normalization differences can expose protected Tomcat directories with a segment such as `/..;/`.[\[6\]](#references)
...
### [Bind and reverse shell with](#bind-and-reverse-shell-with-tomcatwardeployerpy) [tomcatWarDeployer.py](https://github.com/mgeeky/tomcatWarDeployer)
...
```
### [Using](#using-clusterd) [Clusterd](https://github.com/hatRiot/clusterd)
