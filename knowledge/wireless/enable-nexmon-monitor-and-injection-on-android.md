...
source_url: https://hacktricks.wiki/generic-methodologies-and-resources/pentesting-wifi/enable-nexmon-monitor-and-injection-on-android.html
...
Hijacker can toggle monitor mode automatically before running `airodump`, `wifite`, etc.  In **Settings -> Advanced** add the following entries (edit the library path if your module differs):[\[6\]](#references)
...
`nexutil` flags mean?
The Kali Galaxy S10 guide uses `nexutil -s0x613 -i -v2` as its monitor-mode setup command.<sup>[\[6\]](#references)</sup> In the official parser, `-s` selects a custom set-command/ioctl, `-i` marks the custom-command value as an integer, and `-v2` supplies the value `2`.[\[8\]](#references)

The parser does not define the firmware-specific meaning of custom command `0x613`; do not describe it as `FCAP_FRAME_INJECTION` without verifying the exact S10 patch source.[\[8\]](#references)
...
After running *Enable monitor mode* you should see the interface in monitor state and be able to capture raw frames with:[\[6\]](#references)
...
nexutil -m0 && svc wifi enable
```
For passive monitor mode, prefer the upstream-documented `nexutil -m2` flow; do not assume that omitting the S10-specific `-s0x613` custom command has the same effect on every patched firmware.[\[2\]](#references)
...
`libnexmon` inside Kali NetHunter / chroot
Stock user-space tools in Kali do not know about NexMon, but you can force them to use it via `LD_PRELOAD`.[\[6\]](#references)
...
With patched firmware and the appropriate preload helper, user-space tools can use the internal interface in place of an external adapter.[\[2\]](#references)[\[6\]](#references)

## When the ROM is newer than the published patch

If `nexutil -V` reports a newer `bcm4375b1` firmware than the patch you flashed, the problem is usually **firmware-porting**, not Hijacker itself.[\[5\]](#references)[\[7\]](#references)
